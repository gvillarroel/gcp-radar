...
Evidence snippets:
- How driver nodes help you run concurrents job Managed Service for Apache Spark starts a job driver process on a Managed Service for Apache Spark cluster master node for each job.
- You can add {ROLE} == 'Driver' in an initialization script to have the script perform actions for a driver node group in node selection .
