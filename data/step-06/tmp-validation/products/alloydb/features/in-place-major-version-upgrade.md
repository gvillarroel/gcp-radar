...
generated_at: "2026-04-14T00:15:29.110Z"
...
Evidence snippets:
- You can upgrade a cluster by performing an in-place major version upgrade or by migrating the cluster's data to a new cluster .
...
- You only need to do this once per directory. terraform init Optional: To use the latest Google provider version, include the -upgrade option: terraform init -upgrade Apply the changes Note: Terraform samples typically assume that the required APIs are enabled in your Google Cloud project.
