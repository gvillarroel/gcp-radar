...
generated_at: "2026-04-14T00:15:29.199Z"
...
- JSON representation { "hostPort" : string , "referenceId" : string , "sourceType" : enum ( MigrationSourceType ) } Fields hostPort string Output only.
- Place holder for the external source identifier(e.g DMS job name) that created the cluster. sourceType enum ( MigrationSourceType ) Output only.
...
- AuthApiKey ( weaviate api key ), ) Chroma from langchain chroma import Chroma chromadb client = Chroma ( collection name = chromadb collection name , embedding function = embeddings service , persist directory = chromadb path , ) Qdrant from qdrant client import QdrantClient qdrant client = QdrantClient ( path = qdrant path ) Milvus milvus client = MilvusClient ( uri = milvus uri ) Get all the data from the database.
