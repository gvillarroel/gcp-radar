...
feature_name: "BigQuery materialized views over Iceberg tables"
feature_slug: "bigquery-materialized-views-over-iceberg-tables"
...
keywords:
  - "MV over Iceberg"
  - "materialized view over Iceberg"
...
# BigQuery materialized views over Iceberg tables
...
BigQuery materialized views can now reference Iceberg tables directly, removing the need to migrate that data into BigQuery-managed storage.

## Extended Definition

BigQuery can create materialized views that reference Apache Iceberg external tables directly, so you can query Iceberg data from a materialized view without first migrating that data into BigQuery-managed storage. Materialized views are defined in GoogleSQL through CREATE MATERIALIZED VIEW statements with a SELECT query and associated materialized-view options.

## Evidence Summary

The release notes explicitly state Iceberg external table support in materialized views, and the DDL reference page documents the standard CREATE MATERIALIZED VIEW syntax and options used to define such views.
