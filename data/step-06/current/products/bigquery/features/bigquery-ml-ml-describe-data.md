...
feature_name: "BigQuery ML ML.DESCRIBE_DATA"
feature_slug: "bigquery-ml-ml-describe-data"
...
BigQuery ML now includes ML.DESCRIBE_DATA to compute descriptive statistics for training or serving datasets.

## Extended Definition

ML.DESCRIBE_DATA is a BigQuery ML SQL function listed in Google Cloud’s BigQuery ML end-to-end model journey documentation as part of the workflow functions used for model-related operations. The documentation places it alongside other model functions across many model types (for example linear/logistic regression, boosted trees, random forest, AutoML, k-means, PCA, and autoencoders), indicating it is a standard BigQuery ML function intended for broad model monitoring/use-case workflows. The provided excerpt does not define its exact output fields or semantics, so only its role as an available model-function is directly evidenced.

## Evidence Summary

The page confirms the presence of ML.DESCRIBE_DATA in BigQuery ML model journey function tables, but the excerpt does not include a standalone definition or detailed behavior of that function.
