...
Allows creating a BigQuery ML remote model that references Vertex AI textembedding-gecko text embedding models.
...
The cited pages define how BigQuery ML defines remote models in `CREATE MODEL` with `REMOTE WITH CONNECTION` and endpoint/model-identifier options for Vertex AI, with examples including Vertex AI multimodal embedding and Gemini remote models.
...
- Re-rank rationale: The document covers the remote model workflow in BigQuery ML, but does not describe support for the textembedding-gecko model.
