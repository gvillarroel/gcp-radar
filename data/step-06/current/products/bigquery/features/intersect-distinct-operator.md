...
feature_name: "INTERSECT DISTINCT operator"
feature_slug: "intersect-distinct-operator"
...
- WITH NumbersTable AS ( SELECT 1 AS one digit , 10 AS two digit UNION ALL SELECT 2 , 20 UNION ALL SELECT 3 , 30 ) SELECT one digit , two digit FROM NumbersTable INTERSECT DISTINCT STRICT CORRESPONDING SELECT 10 AS two digit , 1 AS one digit ; / -----------+-----------+ one digit two digit +-----------+-----------+ 1 10 +-----------+----------- / For more syntax examples with the BY NAME modifier, see the UNION set operator.
