...
feature_name: "BigQuery LOAD DATA regional destination support for US and US-EAST-4"
feature_slug: "bigquery-load-data-regional-destination-support-for-us-and-us-east-4"
...
keywords:
  - "US-EAST-4 destination"
  - "us-east4"
  - "LOAD DATA destination location"
  - "US multi-region destination"
  - "LOAD DATA location setting"
  - "regional destination support"
  - "US regions"
---

# BigQuery LOAD DATA regional destination support for US and US-EAST-4
...
BigQuery added support for transferring data with LOAD DATA into US multi-region and US-EAST-4 locations.

## Extended Definition

The provided Google Cloud documentation confirms that BigQuery supports operations in specific regions including US multi-region (`us`) and `us-east4` (Northern Virginia), as part of its documented location availability. It does not directly specify LOAD DATA behavior in the excerpt, so only a cautious inference can be made that regional destination options for BigQuery features can include those locations.

## Evidence Summary

The locations page provides BigQuery-supported regions for multiple feature categories (including US and us-east4), but it does not explicitly document LOAD DATA destination support in the provided text.
