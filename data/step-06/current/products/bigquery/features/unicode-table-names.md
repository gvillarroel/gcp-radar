...
feature_name: "Unicode table names"
feature_slug: "unicode-table-names"
...
- The command uses the --append table flag to append the query results to the destination table. bq query \ --append table \ --use legacy sql = false \ --destination table my-other-project:mydataset.mytable \ 'SELECT name, number FROM bigquery-public-data.usa names.usa 1910 current WHERE gender = "M" ORDER BY number DESC' The output for each of these examples looks like the following.
