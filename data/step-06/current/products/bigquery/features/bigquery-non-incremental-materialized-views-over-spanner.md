...
keywords:
  - "BigQuery Spanner materialization"
  - "non-incremental MV"
  - "Spanner MV"
...
Adds support for creating non-incremental materialized views over Spanner data in BigQuery.

## Extended Definition

BigQuery supports non-incremental materialized views over Spanner data, which periodically cache query results to improve query performance. The feature is defined using `CREATE MATERIALIZED VIEW ... AS SELECT ...` in GoogleSQL, and can use the materialized-view option `allow non incremental definition` (Boolean, disabled by default) to support a broader set of SQL queries for these views.

## Evidence Summary

The release notes state the new BigQuery non-incremental materialized views over Spanner, while the DDL reference documents the `CREATE MATERIALIZED VIEW` syntax and the `allow non incremental definition` option controlling expanded query support.
