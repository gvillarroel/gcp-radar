...
feature_name: "ERROR function"
feature_slug: "error-function"
...
BigQuery standard SQL includes the ERROR function for returning errors.
...
- If the file set spec type is NEW LINE DELIMITED MANIFEST , then each line in the file is interpreted as a URI that points to a data file. ignore unknown values BOOL If true , ignore extra values that are not represented in the table schema, without returning an error.
