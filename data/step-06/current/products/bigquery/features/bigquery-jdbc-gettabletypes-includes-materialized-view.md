...
feature_name: "BigQuery JDBC getTableTypes includes MATERIALIZED_VIEW"
feature_slug: "bigquery-jdbc-gettabletypes-includes-materialized-view"
...
keywords:
  ...
  - "JDBC table type metadata"
  - "metadata table types"
  - "MATERIALIZED_VIEW"
  - "table types"
---

# BigQuery JDBC getTableTypes includes MATERIALIZED_VIEW
...
The BigQuery JDBC driver's getTableTypes function now includes MATERIALIZED_VIEW in the returned table types.

## Extended Definition

The feature is the inclusion of a `MATERIALIZED_VIEW` table-type value when BigQuery JDBC metadata APIs enumerate table types via `DatabaseMetaData.getTableTypes`, representing materialized views as a distinct table category. Official BigQuery docs confirm materialized views are represented as table-like objects in schema/DDL metadata contexts, but they do not explicitly confirm this exact JDBC method output in the provided excerpts.

## Evidence Summary

The cited pages establish BigQuery materialized views as documented table-capable resources, but they do not directly state that `DatabaseMetaData.getTableTypes` returns `MATERIALIZED_VIEW`.
