...
feature_name: "BigQuery Omni Azure workload identity federation"
feature_slug: "bigquery-omni-azure-workload-identity-federation"
...
keywords:
  - "no client secrets"
  - "Azure tenant application"
  - "BigQuery Omni WIF"
  - "Omni WIF"
  - "Azure workload identity federation"
  - "identity federation for Omni"
  - "Omni connection"
  - "BigQuery Omni"
---

# BigQuery Omni Azure workload identity federation
...
BigQuery Omni now supports Azure workload identity federation for Omni connections, allowing Google access to Azure tenant applications without managing application client secrets.

## Extended Definition

BigQuery Omni is documented as a BigQuery capability with cross-cloud capabilities and a dedicated service identity represented by the IAM role `roles/bigqueryomni.serviceAgent`. The IAM documentation also notes that there are no separate IAM permissions for the BigQuery Omni service itself. The provided excerpts do not provide evidence for Azure workload identity federation, Azure tenant applications, or secret-free authentication behavior for Omni.

## Evidence Summary

These pages confirm Omni cross-cloud usage and IAM artifacts (service agent role and permission model), but not the specific Azure workload identity federation feature described.
