...
generated_at: "2026-04-10T13:25:38.630Z"
product_name: "BigQuery"
product_slug: "bigquery"
feature_name: "ARRAY input feature type for BigQuery ML models"
feature_slug: "array-input-feature-type-for-bigquery-ml-models"
...
# ARRAY input feature type for BigQuery ML models
...
BigQuery ML now allows ARRAY to be used as an input feature type for other models.

## Extended Definition

In BigQuery ML, the `CREATE MODEL` statement defines model feature inputs through an `INPUT (field name, field type)` clause as part of the model training query syntax. This feature is the ability to use array-typed fields as model input feature columns, while the provided excerpts confirm that input feature columns have documented supported data types but do not show the complete type list that includes ARRAY explicitly.

## Evidence Summary

The CREATE MODEL syntax page confirms the `INPUT` field-type based feature declaration and that supported input-feature data types are documented, while the journey page situates `CREATE MODEL` as the core model-creation path for BigQuery ML.
...
- Objectives This tutorial guides you through completing the following tasks: Creating a time series model to forecast site traffic by using the CREATE MODEL statement .
- The auto arima option of the CREATE MODEL statement defaults to TRUE , so the auto.ARIMA algorithm automatically tunes the hyperparameters in the model.
