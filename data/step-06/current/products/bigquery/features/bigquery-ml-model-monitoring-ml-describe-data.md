...
feature_name: "BigQuery ML model monitoring: ML.DESCRIBE_DATA"
feature_slug: "bigquery-ml-model-monitoring-ml-describe-data"
...
keywords:
  - "BQML DESCRIBE_DATA"
  - "BQML model monitoring"
  - "serving dataset statistics"
...
# BigQuery ML model monitoring: ML.DESCRIBE_DATA
...
BigQuery ML introduces ML.DESCRIBE_DATA to compute descriptive statistics for training or serving datasets.

## Extended Definition

The BigQuery ML feature `ML.DESCRIBE DATA` (shown in the docs as `ML.DESCRIBE DATA`) is an official BigQuery ML statement listed under the **Model monitoring** category. In the BigQuery ML end-to-end user journey reference table, it is available across multiple model types, indicating it is part of standard SQL monitoring workflows for model pipelines.

## Evidence Summary

The cited page identifies `ML.DESCRIBE DATA` as a model monitoring function in BigQuery ML’s official statement/function matrix across many model families.
