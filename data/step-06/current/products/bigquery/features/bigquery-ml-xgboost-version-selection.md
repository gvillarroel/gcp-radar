...
feature_name: "BigQuery ML XGBoost version selection"
feature_slug: "bigquery-ml-xgboost-version-selection"
...
# BigQuery ML XGBoost version selection
...
BigQuery ML lets users specify the XGBoost version used during model training.
...
- The table name can contain the following: Up to 1,024 characters Letters (upper or lower case), numbers, and underscores OPTIONS(table option list) : Lets you specify additional table creation options such as a label and an expiration time. source table name : The name of the source table.
