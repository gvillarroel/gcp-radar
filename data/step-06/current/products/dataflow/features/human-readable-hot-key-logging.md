...
- Causes: Cloud KMS key Permission 'cloudkms.cryptoKeyVersions.useToEncrypt' denied on resource 'projects/<project-id>/locations/<gcp-region>/keyRings/<key-ring-name>/cryptoKeys/<key-name>' (or it may not exist). cannot be validated.
...
Evidence snippets:
- May 14, 2021 Feature You can now enable logging of human-readable hot keys.
...
- Version latest keyboard arrow down 0.11.0 (latest) 0.10.0 0.9.0 0.8.17 0.7.0 0.6.2 0.5.5 0.4.1 0.3.1 0.2.0 0.1.5 SnapshotsV1Beta3AsyncClient ( , credentials : typing .
- Normally, each value must be of type str , but for metadata keys ending with the suffix -bin , the corresponding values must be of type bytes .
- Normally, each value must be of type str , but for metadata keys ending with the suffix -bin , the corresponding values must be of type bytes .
