...
feature_name: "$in aggregation operator"
feature_slug: "in-aggregation-operator"
...
Supports the $in aggregation operator.
...
- You can't combine array-contains with array-contains-any in the same disjunction. in , not-in , and array-contains-any Use the in operator to combine up to 30 equality ( == ) clauses on the same field with a logical OR .
...
- To view the time it took to process a DATA READ or DATA WRITE request, see the processing duration field within the metadata object of an AuditLog .
