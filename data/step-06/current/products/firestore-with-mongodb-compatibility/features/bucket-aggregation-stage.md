...
generated_at: "2026-04-12T12:16:03.676Z"
product_name: "Firestore with MongoDB compatibility"
product_slug: "firestore-with-mongodb-compatibility"
feature_name: "$bucket aggregation stage"
feature_slug: "bucket-aggregation-stage"
...
source_links:
  ...
  - "https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/migrate-configure-resources"
keywords:
  - "bucket"
  - "aggregation"
  - "stage"
...
Supports the $bucket aggregation stage.
...
- [https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/migrate-configure-resources](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/migrate-configure-resources)
...
Evidence snippets:
- Dataflow template parameters: GCS BUCKET TEMPLATE PATH : A sub-path in the GCS BUCKET ROOT PATH directory where a Dataflow template will be staged.
...
- GCS BUCKET ROOT PATH : The name of the top-level directory in the Cloud Storage bucket for placing the intermediate data during the migration.
...
- As an alternative, you can create these files in any text editor and substitute the values manually. echo " $( cat <<EOF includeObjects: databases: - database: ${ MONGODB DATABASE NAME } EOF ) " > mongo source config.yaml echo " $( cat <<EOF path: "/ ${ GCS BUCKET SUB PATH } " avroFileFormat: {} EOF ) " > gcs dst config.yaml The previous example configures the full contents of $MONGODB DATABASE NAME for migration.
...
- However, the data format and the data placement within the Cloud Storage bucket structure are a property of the stream.

### "Configure resources for migration \_|\_ Firestore with MongoDB compatibility\

- URL: [https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/migrate-configure-resources](https://docs.cloud.google.com/firestore/mongodb-compatibility/docs/migrate-configure-resources)
- Source ID: `site-iam-reference`
- Final score: 83
- Re-rank relevance: MODERATE
- Re-rank rationale: Fast mode kept the lexical match without page-level LLM reranking.
...
Evidence snippets:
- Create a new Cloud Storage bucket with the chosen name and in the chosen region by following instructions provided on the Create a bucket page in the Cloud Storage documentation.
- Create a Cloud Storage bucket Create a new Cloud Storage bucket in the following way: Choose a Cloud Storage region where the migration pipeline will run.
- At this stage, you do the following: Install command-line tools required for running commands at later stages of the migration process.
- Choose the name for this bucket.
