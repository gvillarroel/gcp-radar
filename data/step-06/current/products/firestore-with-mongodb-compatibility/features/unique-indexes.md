...
feature_name: "Unique indexes"
feature_slug: "unique-indexes"
...
Firestore with MongoDB compatibility supports unique indexes.
...
- After you start to create an index, Firestore with MongoDB compatibility assigns the operation a unique name.
...
Evidence snippets:
- Firestore with MongoDB compatibility does not automatically create an index on id , but it ensures values of id are unique within a collection.
- Read concern Firestore with MongoDB compatibility supports the snapshot , majority , and linearizable read concerns.
- For a breakdown of supported features depending on MongoDB version, see: Supported features: 8.0 Supported features: 7.0 Supported features: 6.0 Supported features: 5.0 Connections and databases Each connection is limited to a single Firestore with MongoDB compatibility database.
- Home Documentation Databases Firestore with MongoDB compatibility Guides Send feedback Stay organized with collections Save and categorize content based on your preferences.
