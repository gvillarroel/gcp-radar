...
generated_at: "2026-04-14T16:48:38.051Z"
...
In Google Distributed Cloud (software only) for VMware, the Connect Gateway feature enables cluster access by using a kubeconfig/context and running kubectl commands through the gateway. The documentation explicitly confirms read-only kubectl use (for example, `kubectl get pods -A` and `kubectl get namespaces`) and shows that RBAC/cluster-admin access for gateway connectivity is managed via gcloud commands (`generate-gateway-rbac` and cluster update for admin users). The provided excerpts do not provide evidence for `kubectl attach`, `kubectl cp`, `kubectl exec`, or `gcloud beta container fleet memberships get-credentials` specifically.

## Evidence Summary

The cited pages confirm Connect Gateway-based kubectl access patterns and required RBAC/grant setup, but they do not explicitly validate the specific `cp/attach/exec` command support or the exact `get-credentials` command variant in the feature summary.
