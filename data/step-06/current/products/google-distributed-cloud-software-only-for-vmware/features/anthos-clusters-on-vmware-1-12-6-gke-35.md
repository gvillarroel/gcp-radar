...
generated_at: "2026-04-14T16:48:38.126Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.12.6-gke.35"
feature_slug: "anthos-clusters-on-vmware-1-12-6-gke-35"
...
# Anthos clusters on VMware 1.12.6-gke.35
...
Anthos clusters on VMware 1.12.6-gke.35 is available and runs on Kubernetes v1.23.16-gke.2400.

## Extended Definition

Anthos clusters on VMware in Google Distributed Cloud (software-only) refers to versioned Anthos user-cluster workloads running in a VMware-based deployment, managed through gcloud commands that accept and apply explicit cluster version values (for example, via `--version` when creating a cluster). The documented workflow shows that operators must query supported versions with `gcloud container vmware clusters query-version-config` and ensure the admin cluster has the required platform version installed before user-cluster creation or upgrade. The exact feature label `1.12.6-gke.35` is provided in your feature record but is not directly evidenced in the excerpts shown.

## Evidence Summary

These pages show that Google Distributed Cloud (software-only) for VMware supports explicit version-based creation and upgrade of user clusters, and that version availability/install prerequisites are managed through admin-cluster version configuration and query commands.
