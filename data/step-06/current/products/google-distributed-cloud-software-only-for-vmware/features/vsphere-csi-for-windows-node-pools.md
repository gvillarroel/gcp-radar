...
generated_at: "2026-04-14T16:48:38.117Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "vSphere CSI for Windows node pools"
feature_slug: "vsphere-csi-for-windows-node-pools"
...
# vSphere CSI for Windows node pools
...
Anthos clusters on VMware adds GA support for vSphere CSI on Windows node pools.

## Extended Definition

The provided excerpts do not include evidence describing a dedicated "vSphere CSI" feature for Windows node pools in Google Distributed Cloud (software only) for VMware. The only supported fact is that Windows node pools are not eligible for skip-version node-pool upgrades, while Ubuntu and COS node pools are, indicating an operational limitation affecting Windows node pools rather than confirming CSI behavior.

## Evidence Summary

The cited upgrade page documents a Windows-node-pool limitation for skip-version upgrades, and the cluster creation page confirms user-cluster/node-pool workflows without mentioning CSI-on-Windows specifics.
...
- Home Technology areas Google Distributed Cloud Documentation GDC for VMware Guides Send feedback Skip a version when upgrading node pools Stay organized with collections Save and categorize content based on your preferences.
