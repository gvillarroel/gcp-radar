...
generated_at: "2026-04-14T16:48:38.146Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.11.3-gke.45"
feature_slug: "anthos-clusters-on-vmware-1-11-3-gke-45"
...
# Anthos clusters on VMware 1.11.3-gke.45
...
Anthos clusters on VMware version 1.11.3-gke.45 is available and runs on Kubernetes 1.22.8-gke.204.

## Extended Definition

Anthos clusters on VMware (software-only) are managed through a version-based user-cluster lifecycle: you discover available cluster versions via `gcloud container vmware clusters query-version-config` and then create or upgrade clusters with a selected version string (for example values like `1.16.3-gke.45`). The docs also state that the target Anthos version must be available on the admin cluster before a user-cluster upgrade or creation can proceed. The supplied excerpts do not directly confirm the exact version `1.11.3-gke.45` or its stated Kubernetes mapping to `1.22.8-gke.204`, so that specific pair is not evidenced here.

## Evidence Summary

The pages provide operational evidence that version-aware creation/upgrades use version listings from `query-version-config` and require admin-cluster readiness, while the Windows guidance contributes only a general note about patch timing.
