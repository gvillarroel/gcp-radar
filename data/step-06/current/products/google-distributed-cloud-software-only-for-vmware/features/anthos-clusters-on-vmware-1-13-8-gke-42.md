...
generated_at: "2026-04-14T16:48:38.100Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.13.8-gke.42"
feature_slug: "anthos-clusters-on-vmware-1-13-8-gke-42"
...
# Anthos clusters on VMware 1.13.8-gke.42
...
Anthos clusters on VMware version 1.13.8-gke.42 is available and runs on Kubernetes 1.24.11-gke.1200.

## Extended Definition

Anthos clusters on VMware 1.13.8-gke.42 refers to a versioned Anthos VMware cluster release used by Google Distributed Cloud (software-only) for VMware, specified via the `--version` flag during cluster creation or via upgrade workflows. In this platform, cluster creation and upgrades rely on versions returned by `gcloud container vmware clusters query-version-config`, and the selected version must be available on the admin cluster before user cluster creation or upgrade. However, the provided excerpts do not explicitly confirm that `1.13.8-gke.42` is listed as available in those version lists.

## Evidence Summary

The pages show that VMware cluster versions are managed as explicit release identifiers discovered with `query-version-config`, and that an admin-cluster-available version is required before creating or upgrading user clusters.
