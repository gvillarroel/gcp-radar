...
generated_at: "2026-04-14T16:48:38.145Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.10.7-gke.15"
feature_slug: "anthos-clusters-on-vmware-1-10-7-gke-15"
...
# Anthos clusters on VMware 1.10.7-gke.15
...
Anthos clusters on VMware version 1.10.7-gke.15 is available and runs on Kubernetes 1.21.14-gke.2100.

## Extended Definition

For Google Distributed Cloud (software only) for VMware, this feature refers to version-managed operations for Anthos user clusters: users list supported versions and create user clusters with a specified Anthos/GKE release using the gcloud CLI and Terraform. In the upgrade flow, the docs show querying available versions with `gcloud container vmware clusters query-version-config` and preparing the admin cluster by installing the target release via `gcloud container vmware admin-clusters update --required-platform-version` before upgrading a user cluster. The provided excerpts do not explicitly confirm the specific release `1.10.7-gke.15` or its Kubernetes base `1.21.14-gke.2100`, so that exact pairing is not directly evidenced here.

## Evidence Summary

The create and upgrading guides document how Anthos for VMware user cluster versioning works (query available versions, pass a target `--version`, and stage versions in the admin cluster), but do not mention version `1.10.7-gke.15` or Kubernetes `1.21.14-gke.2100` in the provided excerpts.
