...
generated_at: "2026-04-14T16:48:38.181Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "MetalLB bundled software load balancer"
feature_slug: "metallb-bundled-software-load-balancer"
...
# MetalLB bundled software load balancer
...
MetalLB is available as an additional bundled software load balancer alongside Seesaw.

## Extended Definition

MetalLB bundled is a built-in load balancer option for Google Distributed Cloud (software only) for VMware, available alongside the manual third‑party load balancing option when configuring admin and user clusters. When enabled, MetalLB allocates external IPs for Services of type LoadBalancer from configured address pools, and can be deployed with cluster nodes using either static addresses or DHCP in supported configuration examples. During cluster creation, Google Cloud console, gcloud CLI, or Terraform workflows require the same load balancer kind to be used for both admin and user clusters.

## Evidence Summary

The cited pages define MetalLB as a bundled GDC load balancer choice, explain how it is configured and how it assigns Service type LoadBalancer addresses, and specify required consistency of load balancer type across admin/user clusters.
...
- Re-rank rationale: This page is explicitly about configuring bundled load balancing with MetalLB and includes concrete MetalLB control-plane and user-cluster configuration details.
...
- Final score: 170
- Re-rank relevance: STRONG
- Re-rank rationale: The page explicitly says MetalLB is bundled with Google Distributed Cloud for VMware and describes how it is configured and used for LoadBalancer services.
