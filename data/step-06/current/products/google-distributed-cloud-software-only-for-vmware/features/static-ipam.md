...
generated_at: "2026-04-14T16:48:38.322Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Static IPAM"
feature_slug: "static-ipam"
latest_feature_date: "2019-02-07"
deprecation_date: ""
...
Static IPAM lets you provide nodeip blocks in YAML files during installation to assign static node IP addresses.

## Extended Definition

In Google Distributed Cloud (software only) for VMware, Static IPAM is the configuration mode for node networking where cluster nodes receive fixed IP addresses instead of DHCP-assigned ones. Setting `network.ipMode.type` to `static` requires a provided IP block definition (such as a YAML file referenced by `ipBlockFilePath` or equivalent static IP block input during cluster creation), which includes static IPs plus optional hostnames and is used to assign addresses for control plane and worker nodes.

## Evidence Summary

The cited Google docs explicitly show `ipMode: static`, require an IP block path for static allocation, and provide examples of static IP blocks used to assign node and control-plane addresses during user cluster configuration and creation.
...
- The control-plane nodes for your user cluster must get their IP addresses from a list of static addresses that you provide in the IP block file.
- From a list of static IP addresses that you provide in the IP block file.
- Fill in your user cluster configuration file If you used gkeadm to create your admin workstation, then gkeadm generated a template for your user cluster configuration file named user-cluster.yaml .
