...
generated_at: "2026-04-14T16:48:38.128Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.13.5-gke.27"
feature_slug: "anthos-clusters-on-vmware-1-13-5-gke-27"
...
# Anthos clusters on VMware 1.13.5-gke.27
...
Anthos clusters on VMware 1.13.5-gke.27 is available and runs on Kubernetes 1.24.9-gke.2500.

## Extended Definition

Anthos clusters on VMware uses versioned Anthos releases when creating and upgrading user clusters in a Google Distributed Cloud (software only) for VMware deployment, with the target release passed in commands (for example via `--version` in create examples) and version availability controlled on the admin cluster. The workflow in the docs shows obtaining allowed versions with `gcloud container vmware clusters query-version-config` and installing a desired release into the admin cluster first using `--required-platform-version` before user-cluster creation or upgrade. The provided excerpts do not explicitly confirm that version `1.13.5-gke.27` or Kubernetes `1.24.9-gke.2500` is available.

## Evidence Summary

These official pages define the supported VMware deployment context and the documented CLI flows for querying and applying available Anthos versions during VMware user-cluster creation and upgrade.
