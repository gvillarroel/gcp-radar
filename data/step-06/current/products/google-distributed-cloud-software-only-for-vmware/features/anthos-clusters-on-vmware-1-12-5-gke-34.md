...
generated_at: "2026-04-14T16:48:38.130Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.12.5-gke.34"
feature_slug: "anthos-clusters-on-vmware-1-12-5-gke-34"
...
# Anthos clusters on VMware 1.12.5-gke.34
...
Anthos clusters on VMware 1.12.5-gke.34 is available and runs on Kubernetes 1.23.15-gke.2400.

## Extended Definition

In Google Distributed Cloud (software only) for VMware, this feature refers to a specific Anthos on VMware user-cluster release identifier that is selected when creating or upgrading a user cluster (for example via the `--version` flag), and only from versions that are made available by the admin cluster. Administrators must query available versions from the admin cluster before creating or upgrading a user cluster and may then apply an allowed release, but the provided excerpts list examples such as `1.16.3-gke.45` and `1.34.200-gke.68`, not `1.12.5-gke.34`. Therefore, evidence for that exact release and its Kubernetes mapping (`1.23.15-gke.2400`) is weak in the provided excerpts.

## Evidence Summary

The cited Google Cloud docs provide evidence that Anthos on VMware user-cluster lifecycle actions use admin-cluster–scoped version queries for creation/upgrade and support versioned releases, but they do not explicitly include the requested `1.12.5-gke.34` or `1.23.15-gke.2400` values.
