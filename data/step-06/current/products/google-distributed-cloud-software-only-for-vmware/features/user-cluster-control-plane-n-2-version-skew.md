...
generated_at: "2026-04-14T16:48:38.060Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "user cluster control plane n+2 version skew"
feature_slug: "user-cluster-control-plane-n-2-version-skew"
...
# user cluster control plane n+2 version skew
...
A user cluster control plane can be two minor versions newer than its node pools and admin cluster; A user cluster control plane can be two minor versions newer than its node pools and admin cluster.

## Extended Definition

For Google Distributed Cloud (software only) for VMware, user cluster control-plane/node-pool version skew allows the user cluster control plane to be up to 2 minor versions ahead of its node pools (for example, control-plane 1.n can pair with node pools at 1.n, 1.n-1, or 1.n-2), and in this state the control plane is not upgraded to the next minor version until all node pools are at 1.n or 1.n-1. In supported versions 1.31 and higher, the admin cluster itself can be up to 2 minor versions higher than its managed user clusters.

## Evidence Summary

The upgrade-overview documentation explicitly documents the user-cluster control-plane-to-node-pool skew limits and the 1.31+ admin-to-user cluster skew limit, while the upgrading page links to and reinforces version-skew constraints during upgrades.
...
- Version skew not allowed : When upgrading from a non-advanced to an advanced cluster, you cannot upgrade the control plane and node pools separately.
