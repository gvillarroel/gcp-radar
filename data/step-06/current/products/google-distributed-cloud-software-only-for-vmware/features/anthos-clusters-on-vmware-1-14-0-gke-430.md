...
generated_at: "2026-04-14T16:48:38.132Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.14.0-gke.430"
feature_slug: "anthos-clusters-on-vmware-1-14-0-gke-430"
...
# Anthos clusters on VMware 1.14.0-gke.430
...
Anthos clusters on VMware 1.14.0-gke.430 is available and runs on Kubernetes 1.25.5-gke.100.

## Extended Definition

Anthos clusters on VMware (software-only) are Google Distributed Cloud user clusters that run in your own VMware vSphere infrastructure and are created or upgraded via `gcloud container vmware clusters` workflows. The feature includes version-based operations: administrators can list available cluster versions with `query-version-config`, and a selected version must be available/installed on the admin cluster before user cluster creation or upgrade. The provided excerpts only show example Anthos versions such as `1.16.3-gke.45` and `1.34.200-gke.68`, so evidence for `1.14.0-gke.430` or Kubernetes `1.25.5-gke.100` is not present.

## Evidence Summary

These Google docs pages establish the VMware software-only deployment model, the supported user-cluster lifecycle commands, and version-query/install prerequisites, but do not substantiate the specific version and Kubernetes mapping in the requested feature label.
