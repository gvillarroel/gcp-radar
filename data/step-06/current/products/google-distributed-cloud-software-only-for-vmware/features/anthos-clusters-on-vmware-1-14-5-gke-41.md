...
generated_at: "2026-04-14T16:48:38.096Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.14.5-gke.41"
feature_slug: "anthos-clusters-on-vmware-1-14-5-gke-41"
...
# Anthos clusters on VMware 1.14.5-gke.41
...
Anthos clusters on VMware version 1.14.5-gke.41 is available and runs on Kubernetes 1.25.8-gke.1500.

## Extended Definition

In Google Distributed Cloud (software only) for VMware, a cluster release such as `1.14.5-gke.41` is a version identifier used to choose a specific Anthos cluster software build for user clusters via `--version` during create/upgrade operations. The docs show that version identifiers follow the `x.y.z-gke.NNN` pattern and must be present in the admin cluster’s available version set, which is obtained with `gcloud container vmware clusters query-version-config` before user-cluster creation or upgrade. The exact version `1.14.5-gke.41` and its Kubernetes mapping to `1.25.8-gke.1500` are not directly evidenced in the provided excerpts.

## Evidence Summary

The pages provide official command patterns for querying available Anthos on VMware versions and applying a selected `-gke` version when creating or upgrading user clusters, plus confirmation that this is a software-only offering.
