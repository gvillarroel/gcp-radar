...
generated_at: "2026-04-14T16:48:38.093Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.14.6-gke.23"
feature_slug: "anthos-clusters-on-vmware-1-14-6-gke-23"
...
# Anthos clusters on VMware 1.14.6-gke.23
...
Anthos clusters on VMware version 1.14.6-gke.23 is available and runs on Kubernetes 1.25.10-gke.1200.

## Extended Definition

This feature refers to a specific supported release stream for Anthos user clusters on VMware, where cluster software is identified by version strings in the `1.x-gke.y` format (versioned Kubernetes-based releases). The official VMware docs describe this as part of creating and upgrading clusters by querying version availability, then creating/upgrading user clusters only with versions that are available and synchronized to the admin cluster; however, the provided excerpts do not explicitly list version `1.14.6-gke.23` or `Kubernetes 1.25.10-gke.1200`.

## Evidence Summary

These pages document where Anthos clusters on VMware are available, that release/version availability is handled via `query-version-config`, and that upgrades/installations require admin-cluster-supported versions, but not the exact target version and Kubernetes base version from the feature metadata.
