...
generated_at: "2026-04-14T16:48:38.092Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.15.2-gke.44"
feature_slug: "anthos-clusters-on-vmware-1-15-2-gke-44"
...
# Anthos clusters on VMware 1.15.2-gke.44
...
Anthos clusters on VMware version 1.15.2-gke.44 is available and runs on Kubernetes 1.26.2-gke.1001.

## Extended Definition

Anthos clusters on VMware (software-only) on Google Distributed Cloud defines user-cluster lifecycle operations for creating and upgrading clusters by selecting an available Anthos release and applying it during cluster create/upgrade commands. Version usage is driven by `query-version-config` outputs, and the admin cluster must first have the target version installed with `--required-platform-version` before user-cluster creation or upgrade. The provided excerpts do not contain evidence for the specific pair `1.15.2-gke.44` and `1.26.2-gke.1001`, so that exact version-to-Kubernetes mapping is not verified from these sources.

## Evidence Summary

These pages collectively document how to query available Anthos for VMware versions and require admin-cluster version readiness for user-cluster create/upgrade, but they do not explicitly mention `1.15.2-gke.44` or Kubernetes `1.26.2-gke.1001`.
