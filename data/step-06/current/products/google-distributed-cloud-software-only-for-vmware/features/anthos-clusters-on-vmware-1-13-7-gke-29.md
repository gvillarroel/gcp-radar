...
generated_at: "2026-04-14T16:48:38.122Z"
product_name: "Google Distributed Cloud (software only) for VMware"
product_slug: "google-distributed-cloud-software-only-for-vmware"
feature_name: "Anthos clusters on VMware 1.13.7-gke.29"
feature_slug: "anthos-clusters-on-vmware-1-13-7-gke-29"
...
# Anthos clusters on VMware 1.13.7-gke.29
...
Anthos clusters on VMware 1.13.7-gke.29 is available and runs on Kubernetes 1.24.11-gke.1200.

## Extended Definition

Anthos clusters on VMware in Google Distributed Cloud (software only) are user clusters that are created and upgraded through `gcloud container vmware clusters` commands for on-prem environments tied to an admin cluster. Versioned software releases used by user clusters are discovered with `gcloud container vmware clusters query-version-config`, and an Anthos version must be available on the admin cluster before creating or upgrading a user cluster. The provided excerpts do not provide evidence for the specific release `1.13.7-gke.29` or its stated Kubernetes base `1.24.11-gke.1200`, so that exact mapping is unverified here.

## Evidence Summary

The pages confirm the VMware-specific Anthos software context and show how to provision and upgrade user clusters using version-config discovery and admin-cluster version readiness checks.
