...
generated_at: "2026-04-14T18:41:05.995Z"
product_name: "Cloud Deployment Manager"
product_slug: "cloud-deployment-manager"
feature_name: "--update-policy flag"
feature_slug: "update-policy-flag"
...
Evidence snippets:
- Currently, setting LOG FAIL CLOSED mode will have no effect, but still exists because there is active work being done to support it (b/115874152). policy.rules[].logConfigs[]. cloudAudit nested object Cloud audit options. policy.rules[].logConfigs[].cloudAudit. logName string The log name to populate in the Cloud Audit Record. policy.rules[].logConfigs[].cloudAudit. authorizationLoggingOptions nested object Information used by the Cloud Audit Logging pipeline. policy.rules[].logConfigs[].cloudAudit.authorizationLoggingOptions. permissionType string The type of the permission that was checked. policy. etag bytes etag is used for optimistic concurrency control as a way to help prevent simultaneous updates of a policy from overwriting each other.
