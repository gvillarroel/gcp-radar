...
- You can use these metrics to analyze the performance of your Bare Metal Solution infrastructure and troubleshoot issues.
...
- For example, some of the virtualized infrastructure services that you use instead of hardware on Google Cloud include Compute Engine instances and persistent disks, Virtual Private Cloud networks and firewalls, and Cloud Storage buckets for mass storage.
...
- Verify that SAP Host Agent is receiving metrics To verify that the infrastructure metrics are collected by Google Cloud's Agent for SAP and sent correctly to the SAP Host Agent, follow these steps: In your SAP system, enter transaction ST06 .
