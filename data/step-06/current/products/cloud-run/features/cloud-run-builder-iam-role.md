...
- Grant the Cloud Run Builder role to the build service account on your project: gcloud projects add-iam-policy-binding PROJECT ID \ --member = serviceAccount: BUILD SERVICE ACCOUNT EMAIL \ --role = roles/run.builder Replace: PROJECT ID with your Google Cloud project ID.
...
- Enable the Cloud Run Admin API and Cloud Build APIs: Roles required to enable APIs To enable APIs, you need the Service Usage Admin IAM role ( roles/serviceusage.serviceUsageAdmin ), which contains the serviceusage.services.enable permission.
- Granting the Cloud Run builder role takes a couple of minutes to propagate .
