...
feature_name: "Cloud Storage FUSE support for H4D"
feature_slug: "cloud-storage-fuse-support-for-h4d"
...
# Cloud Storage FUSE support for H4D
...
Cluster Toolkit lets H4D machine types use Cloud Storage FUSE.
...
- H4D machine types are optimized for tightly-coupled applications that scale across multiple nodes.
...
- Overview of configuration steps This page shows how to configure an H4D cluster for your MPI applications by completing the following steps: Create a cluster using Cluster Toolkit.
- Then use the following command: Example for Open MPI mpirun -np TOTAL PROCESSES --hostfile HOST FILE --map-by ppr: PROCESSES PER NODE :node ./ YOUR APPLICATION Intel MPI Create a host file that lists the network names of the H4D instances in the cluster.
