...
feature_name: "K8sStorageClass constraint template"
feature_slug: "k8sstorageclass-constraint-template"
...
keywords:
  - "Kubernetes storage class policy"
  - "storage class constraint"
  - "K8sStorageClass constraint"
  - "K8sStorageClass template"
...
The Constraint Template Library added the K8sStorageClass template (available in Anthos Config Management 1.12.1+).
...
Evidence snippets:
- 1.15.1 Change The constraint template library's K8sStorageClass template now supports an allowed list of storage classes using the new allowedStorageClasses parameter.
- 1.13.0 Feature The constraint template library includes a new template: K8sStorageClass .
