...
- For optimal performance, Apigee recommends using no more than 3,000 API proxy basepaths per Apigee environment or environment group.
- Consider performance when using key/value maps as this information is stored in the Cassandra database.
- Import libraries to the Apigee organization or environment and do not include these in the JAR file.
...
- Common use cases Interacting with Apigee-managed data : A no target proxy is useful in cases where you only need to interact with Apigee-managed data, such as data stored in a key-value map (KVM) or the Apigee cache .
