...
Evidence snippets:
- Because this rule can match a large number of events, it is recommended that you refine the rule to match on context entities that have specific intelligence.
...
Evidence snippets:
- A rule exclusion defines the criteria used to exclude an event from being evaluated by the rule set, or by specific rules in the rule set.
