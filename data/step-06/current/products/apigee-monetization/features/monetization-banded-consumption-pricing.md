...
feature_name: "Monetization Banded Consumption Pricing"
feature_slug: "monetization-banded-consumption-pricing"
...
- Rate plan Define the billing configuration, fees, and charges that are incurred for the use of APIs offered in a monetized API product bundle.
...
- Here, 1,000 per minute --> <Rate>1000pm</Rate> <!-- Specifies where to get the user's prompt from the request --> <UserPromptSource>{jsonPath('$.contents[-1].parts[-1].text',request.content,true)}</UserPromptSource> </PromptTokenLimit> Where: <Rate> : The rate at which to limit tokens. <UserPromptSource> : Specifies where to get the user's prompt from the request.
