...
feature_name: "Product image editing"
feature_slug: "product-image-editing"
...
An editing feature that keeps a primary product while changing its background or position; Product image editing lets you preserve a primary product while changing its background or position.
...
- Prompt: A poster with the text "Summerland" in bold font as a title, underneath this text is the slogan "Summer never felt so good" Guide Placement : While Imagen can attempt to position text as directed, you should expect occasional variations.
...
- The values can be one of: reposition - the default behavior in the GPS pipeline fixed - keeps the product in the same position as in the position as in the input image.
- JSON representation { "bufferZones" : [ { object ( BufferZone ) } ] , "editMode" : string , "maskDilation" : number , "guidanceScale" : integer , "productPosition" : string , "maskMode" : { object ( MaskMode ) } , "baseSteps" : integer , "backend" : string , "semanticFilterConfig" : { object ( SemanticFilterConfig ) } , "alternateInitConfig" : { object ( AlternateInitConfig ) } , "outpaintingConfig" : { object ( OutpaintingProcessingConfig ) } } MaskMode Fields maskType string The type of mask to generate from the provided input image.
