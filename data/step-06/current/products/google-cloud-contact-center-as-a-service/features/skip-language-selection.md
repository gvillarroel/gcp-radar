...
- We have introduced the Skip Cascade Group Timer setting, which allows you to bypass the cascade timer settings and route a call or chat to the next available agent, regardless of the cascade group.
- You can use this functionality to route incoming calls to appropriate destinations based on IVR menu selections or queue routing rules.
- Instead, callers get the default language and are routed to the default queue.
