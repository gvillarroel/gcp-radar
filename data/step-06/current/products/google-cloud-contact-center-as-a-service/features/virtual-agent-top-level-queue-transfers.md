...
feature_name: "Virtual agent top-level queue transfers"
feature_slug: "virtual-agent-top-level-queue-transfers"
...
# Virtual agent top-level queue transfers
...
A virtual agent can now transfer consumers to a top-level queue and navigate active subqueue and leaf queue options.
...
- For example, the virtual agent understood what the consumer was requesting and sent them to the proper queue/agent.
