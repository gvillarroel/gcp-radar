...
App Engine can use Cloud CDN when fronted by External HTTP(S) Load Balancing through serverless NEGs.
...
- When migrating traffic to the new version, the global external Application Load Balancer might try to send traffic to instances that aren't fully ready to receive requests, resulting in 503 errors.
