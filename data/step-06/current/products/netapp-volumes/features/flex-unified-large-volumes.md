...
feature_name: "Flex Unified large volumes"
feature_slug: "flex-unified-large-volumes"
...
NetApp Volumes offers preview file-only large volumes with NFS and SMB support for massive datasets.
...
Evidence snippets:
- File volume access with large volumes enabled is also in Preview.
- Flex File Standard Premium Extreme For the Flex Unified and Flex File service level, you can choose between zonal or regional availability.
- Optional: for all Flex Unified pools or regions which support Flex zonal custom performance, complete the following steps in the Performance section: Enter the throughput for the storage pool in the Throughput field.
