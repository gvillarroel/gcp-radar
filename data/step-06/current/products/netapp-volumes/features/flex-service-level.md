...
feature_name: "Flex service level"
feature_slug: "flex-service-level"
...
The Flex service level provides independent provisioning of capacity and performance with zonal pools.
...
- Custom performance pools : independent provisioning of capacity and performance with zonal pools in selected regions.
- Performance : independent provisioning of capacity and performance with zonal pools in selected regions.
...
Evidence snippets:
- The hot tier pricing depends on the type of service level: For the Premium and Extreme service levels: the hot tier size is calculated as the total pool capacity minus the cold tier size and is charged at the pool's hot tier pricing.
- For the Flex service level: the allocated hot tier capacity is charged at the pool's hot tier pricing.
- For Flex service levels, the command shows the size of the hot tier and the cold tier.
...
Evidence snippets:
- The following table provides a few examples of volume capacity versus maximum inodes for volumes: Volume capacity in GiB Inode limit for Standard, Premium, and Extreme service levels Inode limit for Flex service level 100 21.2 million 3.1 million 512 21.2 million 15.9 million 1024 31.8 million 31.8 million 2048 63.7 million 63.7 million 4096 127.5 million 127.5 million 5200 161.8 million 161.8 million For more information about monitoring inode usage, see Methods for monitoring volume usage .
...
- Backup limits Resource Limit Number of backups per volume 1,000 Total number of scheduled backups (daily, weekly, and monthly) 1,000 Maximum number of concurrent backups per Flex storage pool 10 Storage pool limits Resource Limit Minimum pool capacity Flex Unified and Flex File service level : 1 TiB.
