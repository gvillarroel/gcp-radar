...
feature_name: "Large capacity volumes"
feature_slug: "large-capacity-volumes"
...
- Cloud Monitoring Cloud Monitoring receives auto-tiering statistics for Premium and Extreme volumes, which can be visualized using metrics explorer or dashboards.
...
Evidence snippets:
- CMEK migration not blocked CMEK migration isn't blocked by the Google Cloud console and is allowed even when large capacity volumes are present.
- If you try CMEK migration when large capacity volumes are configured, the attempt will fail.
- If you receive a quota error during a Stop operation, compare the quota rules on both the source and destination volumes and then update any missing quota rules on the destination to match the source volume settings.
