...
Cloud SQL offers committed use discounts for regional database instance usage over 1- or 3-year terms.
...
- Home Documentation Databases Cloud SQL SQL Server Guides Send feedback Clone instances Stay organized with collections Save and categorize content based on your preferences.
- The new instance has the same settings such as database flags, connectivity options, machine type, and storage and memory settings.
- For a regional instance, this zone replaces the primary zone, but the secondary zone remains the same as the source instance.
