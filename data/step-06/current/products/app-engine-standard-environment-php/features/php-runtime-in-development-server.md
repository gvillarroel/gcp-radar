...
- To set a variable in a user's session, you can use the following code: session start(); $ SESSION['Foo'] = 'Bar'; The following code will print Bar , on a subsequent request by the same user: session start(); print $ SESSION['Foo']; // prints Bar For longer-lived sessions, use an alternative storage service such as Cloud SQL .
