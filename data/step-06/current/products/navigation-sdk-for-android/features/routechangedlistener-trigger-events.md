...
generated_at: "2026-04-12T12:18:00.686Z"
product_name: "Navigation SDK for Android"
product_slug: "navigation-sdk-for-android"
feature_name: "RouteChangedListener trigger events"
feature_slug: "routechangedlistener-trigger-events"
...
Evidence snippets:
- Navigator.RouteChangedListener provides the onRouteChanged() callback, triggered when the route changes.
...
Evidence snippets:
- After calculating directions, the SupportNavigationFragment displays a polyline representing the route on the map, and a marker at the destination.
...
- Unregistering the service To stop receiving navigation updates, unregister the service from the Navigation SDK. navigator . unregisterServiceForNavUpdates (); Understand navigation states Use NavInfo.getNavState() to get the current state of navigation, which is one of the following: Enroute - The ENROUTE state means that guided navigation is active and the user is on the provided route.
- Populate the feed display Now that you have set up your turn-by-turn service, this section covers the visual and text elements you can use to populate the guidance cards for the turn-by-turn feed.
