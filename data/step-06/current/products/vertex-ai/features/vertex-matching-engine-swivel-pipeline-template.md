...
feature_name: "Vertex Matching Engine Swivel pipeline template"
feature_slug: "vertex-matching-engine-swivel-pipeline-template"
...
# Vertex Matching Engine Swivel pipeline template
...
The Swivel pipeline template was introduced in preview to support creating embeddings for use with Vertex Matching Engine.
...
- REPO NAME = "mlops" Create a repo in the artifact registry ! gcloud artifacts repositories create $REPO NAME --location=$REGION --repository-format=KFP Upload the compiled pipeline to the repository. from kfp.registry import RegistryClient host = f "https:// { REGION } -kfp.pkg.dev/ { PROJECT ID } / { REPO NAME } " client = RegistryClient ( host = host ) TEMPLATE NAME , VERSION NAME = client . upload pipeline ( file name = PIPELINE FILE , tags = [ "v1" , "latest" ], extra headers = { "description" : "This is an example pipeline template." }) TEMPLATE URI = f "https:// { REGION } -kfp.pkg.dev/ { PROJECT ID } / { REPO NAME } / { TEMPLATE NAME } /latest" In the Google Cloud console, verify that your template appears in Pipeline Templates .
...
- Within a task, you can build on the on-demand compute capabilities of Vertex AI with Kubernetes to scalably execute your code, or delegate your workload to another execution engine, such as BigQuery, Dataflow, or Managed Service for Apache Spark.
