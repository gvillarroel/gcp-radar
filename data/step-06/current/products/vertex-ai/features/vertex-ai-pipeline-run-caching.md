...
feature_name: "Vertex AI pipeline run caching"
feature_slug: "vertex-ai-pipeline-run-caching"
...
# Vertex AI pipeline run caching
...
Vertex AI added the enable_caching option in the Google Cloud console to configure pipeline run caching.
...
Evidence snippets:
- PipelineJob ( display name = "triggered custom regression evaluation" , template path = TEMPLATE URI , parameter values = parameters , pipeline root = BUCKET URI , enable caching = False ) Run the pipeline job job . run () The job takes about 30 minutes to complete.
