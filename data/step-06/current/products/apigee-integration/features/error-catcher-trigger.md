...
Introduces an Error Catcher trigger to handle failures in a trigger, task, or edge condition.
...
- Attach the error catcher Once you have created your custom error catcher logic, you can use it to handle the errors or execution failures at both, the task and event levels.
...
Evidence snippets:
- Following is a sample edge condition configured using the isApproved variable: $ TASK ID isApproved$ = TRUE Error handling strategy An error handling strategy for a task specifies the action to take if the task fails due to a temporary error .
...
- What's next Learn about all tasks and triggers .
