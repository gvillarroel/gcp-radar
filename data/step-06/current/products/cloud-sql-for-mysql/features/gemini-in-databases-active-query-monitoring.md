...
feature_name: "Gemini in Databases active query monitoring"
feature_slug: "gemini-in-databases-active-query-monitoring"
...
# Gemini in Databases active query monitoring
...
Gemini in Databases can monitor and troubleshoot active queries in your database.
...
Evidence snippets:
- Home Documentation Databases Cloud SQL MySQL Guides Send feedback Troubleshoot Stay organized with collections Save and categorize content based on your preferences.
...
- Things to try: Check the replication metrics for your replica instance in the Cloud Monitoring section of the Google Cloud console.
- External primary Issue Troubleshooting Lost connection to MySQL server during query when dumping table .
...
- Whether canceling the out-of-memory (OOM) session is enabled. activeQueryEnabled boolean Output only.
- Use the "cloudsql.googleapis.com/database/disk/bytes used" metric in Cloud Monitoring API instead.
