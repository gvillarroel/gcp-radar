...
feature_name: "Regional availability in europe-west10"
feature_slug: "regional-availability-in-europe-west10"
...
# Regional availability in europe-west10
...
Cloud SQL for MySQL is available in the europe-west10 region.
...
- This implies that that if your workload is not a streaming workload and is latency sensitive, it can't reach the IOPS limit as regional persistent disk with SSD has higher latency than a zonal persistent disk with SSD.
- This is because of the synchronous replication of the data across multiple zones involved in a regional persistent disk to provide multiple copies of data across the zones in a region.
...
- You are returned to the instance page for the primary instance while the instance is updated. gcloud Create the regional instance: gcloud sql instances create REGIONAL INSTANCE NAME \ --availability-type = REGIONAL \ --database-version = DATABASE VERSION \ --tier = MACHINE TYPE \ --enable-bin-log For a complete list of available parameters, see the gcloud sql instances create reference page.
