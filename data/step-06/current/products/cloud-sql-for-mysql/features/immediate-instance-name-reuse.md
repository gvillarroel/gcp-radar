...
- In the Delete backup window, enter the name of deleted instance, and then click Delete . gcloud To delete a retained backup, use the gcloud sql backups delete command: gcloud sql backups delete BACKUP NAME Replace the following: BACKUP NAME : the name of your retained backup.
...
- This field can exist even if a standalone instance doesn't have a DR replica yet or the DR replica is deleted. failoverDrReplicaName string Optional.
