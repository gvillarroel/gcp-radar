...
generated_at: "2026-04-14T13:43:35.737Z"
product_name: "Cloud Asset Inventory"
product_slug: "cloud-asset-inventory"
feature_name: "SearchAllResources searchable field: state"
feature_slug: "searchallresources-searchable-field-state"
...
# SearchAllResources searchable field: state
...
Cloud Asset Inventory now supports `state` as a searchable field in the SearchAllResources API.
...
- For more information, see Set up authentication for a local development environment . require "google/cloud/asset" scope = 'SCOPE OF THE QUERY' query = 'QUERY STATEMENT' asset types = 'AN ARRAY OF ASSET TYPES TO SEARCH FOR' page size = 'SIZE OF EACH RESULT PAGE' page token = 'TOKEN PRODUCED BY THE PRECEDING CALL' order by = 'FIELDS TO SORT THE RESULTS' asset service = Google :: Cloud :: Asset . asset service response = asset service . search all resources ( scope : scope , query : query , asset types : asset types , page size : page size , page token : page token , order by : order by ) Do things with the response response . page . each do resource puts resource end Additional search examples The following code samples show specific search queries for both gcloud and REST to help you to construct your own searches.
