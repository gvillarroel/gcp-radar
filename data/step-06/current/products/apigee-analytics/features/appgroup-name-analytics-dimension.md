...
generated_at: "2026-04-14T17:34:53.935Z"
product_name: "Apigee Analytics"
product_slug: "apigee-analytics"
feature_name: "AppGroup Name analytics dimension"
feature_slug: "appgroup-name-analytics-dimension"
...
# AppGroup Name analytics dimension
...
The AppGroup Name analytics dimension lets custom reports and report jobs group metrics by AppGroup; The AppGroup Name analytics dimension lets custom reports and report jobs group metrics by AppGroup.
...
Evidence snippets:
- Analyze API message content using custom analytics .--> Dimension Name to use in the Apigee API Description Access Token access token App end user's OAuth access token.
- You'll see the UI names when you create and manage custom reports .
- Note that if you have a relatively low number of transactions over the time period, the average number of transactions per second could appear to be zero in UI custom reports if the number is smaller than two decimal places.
- Metrics Following are the API metrics you can retrieve in custom reports and Apigee API calls.
...
- Prepare the environment Now, lets prepare the environment for our custom curation.
