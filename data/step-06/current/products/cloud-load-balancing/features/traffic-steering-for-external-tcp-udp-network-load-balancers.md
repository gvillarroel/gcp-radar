...
feature_name: "Traffic steering for external TCP/UDP network load balancers"
feature_slug: "traffic-steering-for-external-tcp-udp-network-load-balancers"
...
# Traffic steering for external TCP/UDP network load balancers
...
External TCP/UDP network load balancer forwarding rules can direct traffic from specific source IP ranges to specific backend services or target instances.
...
- What's next Learn how to configure a regional external passthrough Network Load Balancer and distribute traffic in Set up a regional external passthrough Network Load Balancer .
...
- Sample log format for an external client to VM flows The following diagram shows the inbound and outbound traffic for an external client (203.0.113.7), regional external passthrough Network Load Balancer (198.51.100.99), and backend instance (10.240.0.2).
