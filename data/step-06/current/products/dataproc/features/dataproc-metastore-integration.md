...
- Create a Dataproc Metastore service The following instructions demonstrate how to create a Dataproc Metastore service that you can migrate your SQL dump file to.
...
Evidence snippets:
- Run the following add-iam-policy-binding command to grant a Dataproc Metastore predefined role to an IAM principal (user account or service account). gcloud projects add-iam-policy-binding PROJECT ID \ --member= PRINCIPAL \ --role= METASTORE ROLE Replace the following: PROJECT ID : The ID of the project you want to enable Metastore access to.
...
- This page describes how to grant a Google Cloud user account or service account access to basic Dataproc Metastore resources in a project.
