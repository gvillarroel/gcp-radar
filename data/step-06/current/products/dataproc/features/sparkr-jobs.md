...
Evidence snippets:
- Change New sub-minor versions of Cloud Dataproc images - 1.0.83, 1.1.74, 1.2.38 Change the jdbc connect string passed to beeline when submitting Hive jobs to high availability clusters through the Cloud Dataproc Jobs API.
- This feature allows you to submit SparkR jobs on a Cloud Dataproc cluster using the gcloud command-line tool, the Google Cloud Platform Console, or the Cloud Dataproc API.
- This feature allows you to submit SparkR jobs on a Cloud Dataproc cluster using the gcloud command-line tool, the Google Cloud Platform Console, or the Cloud Dataproc API.
- March 02, 2021 Feature Added the --cluster-labels flag to gcloud dataproc jobs submit to allow submitting jobs to a cluster that matches specified cluster labels.
