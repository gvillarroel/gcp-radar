...
- Strategy: BackupAllRestoreAll This is the simplest strategy and backs up all the component's volumes at backup time and restores them all from their volume backups at restore time.
- There are three strategies available: BackupAllRestoreAll - backup the volumes associated with all instances of the component and restore them all from the backups.
...
Evidence snippets:
- BACKUP PLAN : the source of backups that can be restored with this restore plan: projects/ PROJECT ID /locations/ BACKUP LOCATION /backupPlans/ BACKUP PLAN NAME Ensure that the backup plan is in the same project as the new restore plan.
...
- Create a restore plan You can create a restore plan for your backups by using the Google Cloud CLI or the Google Cloud console.
