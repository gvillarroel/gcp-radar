...
Administrators can add custom fields to the Close Case dialog.
...
- Command: python -m detect.v1alpha.get alert \ --project id=$PROJECT ID \ --project instance=$PROJECT INSTANCE \ --credentials file=$CREDENTIALS FILE \ --alert id=$ALERT ID Expected Result: The JSON output contains a feedbackSummary object showing "status": "CLOSED" and your specified comment (for example "automated cleanup").
...
- Customization: If you need to change the verdict (for example to FALSE POSITIVE ) or the comment, you can: Directly edit the DEFAULT FEEDBACK dictionary in the Python file.
