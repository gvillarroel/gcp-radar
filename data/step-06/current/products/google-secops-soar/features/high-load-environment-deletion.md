...
feature_name: "High-load environment deletion"
feature_slug: "high-load-environment-deletion"
...
# High-load environment deletion
...
Environments with heavy loads can now be deleted directly from the platform.
...
- Office 365 - Sharepoint and OneDrive : Detects malicious, suspicious, and high-risk activities in Office 365 Sharepoint and OneDrive, including malware uploads, anonymous file sharing, and searches for credentials and financial data.
...
- For example, you may want to match on the confidence score that Mandiant assigned to the indicator, whether it was seen in a breached environment or the malware family associated with the indicator.
...
- Action results Script result Script result name Value options Example is success True or False is success:False JSON result { "#0-1B09DN3B0D2011E985730AS799BFE5BC" : "obLD1AACAAQAAAAAAAAAAAAABdwAAAABV+kUZQAHyFMAAAXqAAAF6gr3GgnfOwobLz7Y6wgARQAF3Dd3QABnBvvXVduqw6wfLg8MmgG7xmc2dMr3EdxQEAD+OgAAABcDAwdVAAAAAAAAAASEw70Ys0kQbz8wdaj1lsHAAA==" } Get Vulnerability Reports Get environment vulnerability report files.
...
- If a playbook is associated with a specific environment group, its scope automatically updates if the environment group's scope changes.
