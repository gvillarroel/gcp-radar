...
feature_name: "Multiple BGPLoadBalancer resources"
feature_slug: "multiple-bgploadbalancer-resources"
...
- A subject alternative name (SAN) is a feature of SSL certificates that lets you define the domain names and subdomains on which you want a certificate to be valid.
- Don't modify the top-level control plane node pool resources directly.
- This specification declares and controls critical cluster resources.
...
- Google Distributed Cloud lets you specify the number of gratuitous ARP (GARP) messages to send at a time after a control plane node transitions to the role of the master server.
...
- These resources are used by the Kubernetes control plane to manage the IP address ranges for your services.
...
- Define a variable for the control plane VIP address.
