...
feature_name: "Multiple-key rate limiting"
feature_slug: "multiple-key-rate-limiting"
...
- Rate limiting based on multiple keys You can also throttle traffic based on multiple rate limiting keys by using the enforce-on-key-configs flag.
...
- To learn more about internal service security policies, see Configure rate limiting with Cloud Armor in the Cloud Service Mesh documentation.
...
Evidence snippets:
- Google Cloud Armor and reCAPTCHA provide tools to help you evaluate and act on incoming requests that might be from automated clients. reCAPTCHA uses advanced risk analysis techniques to distinguish between human users and automated clients. reCAPTCHA assesses the user based on the configuration of the reCAPTCHA WAF site keys.
- In addition, you can identify clients for rate limiting based on reCAPTCHA exemption cookies or tokens, to throttle requests or ban clients that reuse or abuse the same cookie or token at a rate exceeding the user-configured threshold.
