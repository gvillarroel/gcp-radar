...
feature_name: "DSN DataPath writemode options"
feature_slug: "dsn-datapath-writemode-options"
...
# DSN DataPath writemode options
...
DSN DataPaths can be configured with writemode values OLD, MOD, or SHR.
...
Evidence snippets:
- For all other DataPaths , the default output format is ORC . --parallelism = COUNT (Optional) Specify the number of processing threads.
- For all other DataPaths , the default output format is ORC . --parallelism = COUNT (Optional) Specify the number of processing threads.
...
- The certificate trust store can be stored in the following formats: PKCS12 (.p12): May contain many certificates and requires a password JKS (.jks): May contain many certificates and requires a password PEM (.pem): May contain many certificates encoded as base64 Binary (.der): Contains one certificate per file If the trustStore system property is the path to a directory, the directory is scanned and all certificates with the .pem and .der format found in the directory are added as trusted certificates.
- Cloud Storage objects take precedence over local DSN when this is set. #export GCSGDGURI="gs://[BUCKET]/[PREFIX]" Uncomment the line below to set a default output bucket for the gszutil command. #export GCSOUTURI="gs://[BUCKET]/[PREFIX]" Mainframe Connector gRPC service Uncomment and edit the lines below to set the Hostname or IP Address and port of the gRPC data set transcoding service.
