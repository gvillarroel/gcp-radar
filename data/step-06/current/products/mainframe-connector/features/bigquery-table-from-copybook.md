...
feature_name: "BigQuery table from copybook"
feature_slug: "bigquery-table-from-copybook"
...
A BigQuery table can be created from a copybook.
...
- Export BigQuery table into Mainframe dataset You can export BigQuery table into mainframe dataset by creating a YAML file that executes a SQL read from the QUERY DD file , and exports the resulting dataset to Cloud Storage as a binary file, as follows.
...
- If you want to move very large volumes of data (around 500 GB per day or more) from your mainframe to Google Cloud, and don't want to use your mainframe for this effort, you can use a cloud-enabled Virtual Tape Library (VTL) solution to transfer the data to a Cloud Storage bucket.
...
- If you don't provide a value, Mainframe Connector reads from DD COPYBOOK . --connections = NUMBER (Optional) Specify the number of connections that can made to the remote receiver.
