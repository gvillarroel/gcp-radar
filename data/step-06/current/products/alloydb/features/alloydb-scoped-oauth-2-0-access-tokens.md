...
- If the data api access configuration isn't enabled on an instance, you can enable it using the curl command to update the value of the field dataApiAccess to ENABLED : curl -X PATCH \ -H "Authorization: Bearer $(gcloud auth print-access-token)" \ -H "Content-Type: application/json" \ https://alloydb.googleapis.com/v1alpha/projects/ PROJECT ID /locations/ LOCATION /clusters/ CLUSTER ID /instances/ INSTANCE ID ?updateMask=dataApiAccess \ -d '{ "dataApiAccess": "ENABLED", }' Replace the following: PROJECT ID : The ID of your Google Cloud project.
- POST /mcp HTTP/1.1 Host: alloydb.googleapis.com Content-Type: application/json { "id": USER SPECIFIED ID , "jsonrpc": "2.0", "method": "tools/list", } Execute SQL To execute SQL statements, follow these steps: Set the data api access instance setting on the AlloyDB instance to the value ALLOW DATA API ACCESS .
