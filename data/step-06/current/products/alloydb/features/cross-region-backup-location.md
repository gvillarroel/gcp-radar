...
- To list all the backups in a backup vault for your cluster, run the following command: gcloud backup-dr backups list \ --data-source = D DATA SOURCE \ --project = WORKLOAD PROJECT ID \ --location = REGION \ --backup-vault = BACKUP VAULT NAME Replace the following: DATA SOURCE : the data-source for your AlloyDB cluster.
