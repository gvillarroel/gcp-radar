...
- Use the gcloud alloydb instances update command to change the machine type of the primary instance. gcloud alloydb instances update INSTANCE ID \ --cpu-count = CPU COUNT \ --machine-type = MACHINE TYPE \ --region = REGION ID \ --cluster = CLUSTER ID \ --project = PROJECT ID Replace the following: INSTANCE ID : The ID of the instance that you are updating.
