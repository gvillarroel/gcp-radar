...
feature_name: "Private Service Connect consumer HTTP(S) controls for managed service access"
feature_slug: "private-service-connect-consumer-http-s-controls-for-managed-service-access"
...
- Configure DNS for on-premises systems If you've implemented the DNS configuration using Cloud DNS, you'll need to configure on-premises systems so that they can make queries to your Cloud DNS managed private zones: Create an inbound server policy in the VPC network to which your on-premises network connects.
...
- Access endpoints from hybrid networks Clients in networks that are connected to Google Cloud with VLAN attachments for Cloud Interconnect or Cloud VPN tunnels can reach Private Service Connect endpoints.
...
- As the service receives calls for different APIs, it uses Private Service Connect interfaces to initiate connections to consumer VPC networks.
- Google APIs and services that are reachable from the consumer VPC network through Private Google Access or VPC Service Controls .
