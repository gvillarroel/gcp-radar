...
- Log filtering When you enable VPC Flow Logs, you can set a filter based on both base and metadata fields that only preserves logs that match the filter.
...
- Note: It is possible to enable VPC Flow Logs for the Private Service Connect subnet.
- Logging You can enable VPC Flow Logs on the subnets that contain the backend VMs.
