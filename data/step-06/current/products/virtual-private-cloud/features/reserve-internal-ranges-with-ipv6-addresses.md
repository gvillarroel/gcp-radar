...
Evidence snippets:
- Internal ranges for migration The internal range is used to reserve the CIDR range that is used in the producer subnet so that when the producer subnet is deleted, the CIDR range can't be used for another purpose.
