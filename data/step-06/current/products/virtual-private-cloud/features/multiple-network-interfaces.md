...
feature_name: "Multiple Network Interfaces"
feature_slug: "multiple-network-interfaces"
latest_feature_date: "2017-08-18"
deprecation_date: ""
...
Multiple Network Interfaces allows a VM instance to attach more than one virtual network interface, each connected to a different VPC network; Multiple Network Interfaces enables a VM instance to use more than one virtual network interface, with each interface attached to a different VPC network.
...
Evidence snippets:
- For more examples of how to create instances with multiple network interfaces, see Create VMs with multiple network interfaces . gcloud compute instance-templates create INSTANCE TEMPLATE NAME \ --network-interface=network= NETWORK A ,subnet= SUBNET A \ --network-interface=network= NETWORK B ,subnet= SUBNET B ,vlan= VLAN ID Replace the following: INSTANCE TEMPLATE NAME : a name for the new instance template.
- For more examples of how to create instances with multiple network interfaces, see Create VMs with multiple network interfaces .
...
Evidence snippets:
- Console If you specify a VM instance with multiple network interfaces as the source or destination of a test, Connectivity Tests prompts you to select a network interface from a list.
...
- Limitations Live data plane analysis might not cover all possible networking paths: If the destination endpoint is a Google Cloud load balancer with multiple backends, each live data plane analysis probe is sent to a random backend.
