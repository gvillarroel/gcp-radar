...
feature_name: "Autonomous Database private network access"
feature_slug: "autonomous-database-private-network-access"
...
- Select a Client subnet . (Optional) In the Advanced network settings section, you can provide a Private IP address and a Hostname prefix . (Optional) To enable public access, select the Allow public access checkbox and create access endpoints.
...
- Configure single-region database connectivity Single-region connectivity can be configured using the same methods as you would use for a database not protected by Autonomous Data Guard.
