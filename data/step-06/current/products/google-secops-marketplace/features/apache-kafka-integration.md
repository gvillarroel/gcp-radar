...
feature_name: "Apache Kafka integration"
feature_slug: "apache-kafka-integration"
...
Evidence snippets:
- Therefore, to update a list of brokers to add a new broker, specify all existing brokers and the new broker. kafka settings.tls settings.certificate string Optional The path and certificate filename.
...
- Reason: {0}''.format(error.Stacktrace) General Authorize Security Group Ingress Adds the specified ingress rule to a security group.
...
- Error is {0}".format(exception.stacktrace) General Create a User Description Create a new IAM user for your AWS account.
