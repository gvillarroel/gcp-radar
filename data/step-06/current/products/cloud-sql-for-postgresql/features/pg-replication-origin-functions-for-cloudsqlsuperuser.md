...
Evidence snippets:
- Error message: GRANT stderr: ERROR: must be member of role ROLE NAME This error message appears if you try to import a SQL dump file that's uploaded in Cloud Storage to a Cloud SQL database, and the import job has run for about four days.
...
- To resolve this issue, complete the following steps: Create the ROLE NAME role in the destination database where you're importing the SQL dump file.
