...
Evidence snippets:
- Depending on underlying disk size, storage shrink operations might accrue considerable downtime.
...
- The downtime experienced when shrinking the storage of an instance is comparable to the downtime experienced when restoring a read replica.
...
- A slot that's not actively used to track data causes PostgreSQL to hold onto WAL segments indefinitely, causing the disk space to grow indefinitely.
- Long-running uncommitted transactions on the primary instance can cause read replica creation to fail.
