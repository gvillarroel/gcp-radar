...
- Note that when autovacuum runs, up to autovacuum max workers times this memory may be allocated, so be careful not to set the default value too high.
- When looking for signs of memory exhaustion, you should primarily use the usage metric.
- If the metrics appear high, the instance might be low in memory.
