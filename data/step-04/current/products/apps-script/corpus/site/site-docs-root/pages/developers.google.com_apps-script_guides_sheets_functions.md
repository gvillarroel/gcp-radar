---
title: "Custom Functions in Google Sheets \_|\_ Apps Script \_|\_ Google for Developers"
...
Calculate sale price of discounted items (quickstart)
...
Apps Script .
Delete any code in the script editor. For the DOUBLE function shown
...
Create
or open a spreadsheet in Sheets.
At the top, click Add-ons > Get add-ons .
Once the Google Workspace Marketplace
opens, click the search box in the top right corner.
Type "custom function" and press Enter.
If you find a custom function add-on you're interested in, click Install
to install it.
A dialog might tell you that the add-on requires authorization. If so,
read the notice carefully, then click Allow .
The add-on becomes available in the spreadsheet. To use the add-on in a
...
Type an equals sign ( = ) followed by the function name and any input value —
for example, =DOUBLE(A1) — and press Enter.
The cell momentarily displays Loading... , then returns the result.
...
working with them can be complicated .
...
JSDoc @customfunction tag, as in the DOUBLE()
example.
Note: While Apps Script supports standard JSDoc for autocomplete in the editor,
the Google Sheets UI has specific limitations
regarding which tags and syntax are displayed in the spreadsheet's formula helper.
/**
...
Apps Script services to perform
...
Notes
Cache
Works, but not particularly useful in custom functions
HTML
Can generate HTML, but can't display it (rarely useful)
JDBC
Language
Lock
Works, but not particularly useful in custom functions
Maps
Can calculate directions, but not display maps
Properties
getUserProperties() only gets the properties of the
spreadsheet owner. Spreadsheet editors can't set user properties in a
custom function.
Spreadsheet
...
URL Fetch
Access resources on the web by fetching URLs.
Utilities
XML
...
Publish the script as a Sheets Editor add-on .
