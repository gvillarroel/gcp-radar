---
title: "Create and manage tags \_|\_ Resource Manager \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/resource-manager/docs/tags/tags-creating-and-managing
...
that can be attached to a Google Cloud resource. You can use tags to
...
tag binding to a resource. These are called dynamic tag values( Preview ).
...
name, use the
tagValues.getNamespaced method:
...
resource using the Google Cloud console, the gcloud CLI, or with a
call to the API.
Console
To view all tags:
Open the Tags page in the Google Cloud console.
...
Google Cloud console, the gcloud CLI, or with a call to the API.
...
resources using the Google Cloud console. See
Required permissions for a list of the roles related to
...
the value to the Google Cloud resource. The following workflow describes how to
...
command in gcloud CLI or the locations.tagBindingCollections.patch
...
gcloud alpha resource-manager tags bindings update command.
gcloud
gcloud alpha resource-manager tags bindings update \
...
flag with the gcloud alpha resource-manager tags bindings update command.
gcloud
gcloud alpha resource-manager tags bindings update \
--resource-name= RESOURCE_NAME \
...
use the --replace-tags flag with the gcloud alpha resource-manager tags bindings update command.
gcloud
gcloud alpha resource-manager tags bindings update \
--resource-name= RESOURCE_NAME \
...
Resource type : the fully qualified name of the Google Cloud
...
Google Cloud REST resource that you want to enforce mandatory tags on,
...
Google Cloud project, you should omit this flag. If you are
...
hold, which can be found by using the list command .
For example:
...
that policy.
For example, you can
conditionally grant Identity and Access Management (IAM) roles
based on whether a resource has a specific tag.
Note: In general, any tag changes take effect within 2 minutes. However, changes
can potentially take 7 minutes or longer to fully propagate across the system.
Identity and Access Management conditions and tags
You can use tags and Identity and Access Management conditions
...
Important: Ensure that any roles you grant to your developers don't
include any setIamPolicy permissions, as these allow your developers to
modify the project's allow policy, where they could remove any conditional
restrictions you may have put into place.
...
System tags
System tags are Google-managed tags that Google Cloud services can bind to
