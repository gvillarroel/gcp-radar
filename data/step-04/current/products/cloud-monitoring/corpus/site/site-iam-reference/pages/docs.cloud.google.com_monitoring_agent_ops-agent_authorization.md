---
title: "Authorize the Ops Agent \_|\_ Cloud Monitoring \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/monitoring/agent/ops-agent/authorization
...
Ops Agent unless you are using very old
Compute Engine VMs or have to use a custom service
account instead of
the default service
account .
Compute Engine instances are pre-authorized to run the agent.
This guide explains how to ensure that the Ops Agent, which you install
...
the Ops Agent on a Compute Engine VM instance by using
...
The Ops Agent doesn't support authorization using gcloud auth .
To prevent unwanted behavior, you must
revoke
any credentials created by using gcloud auth .
...
Compute Engine VM instances are assigned access scopes when you create
them. New Compute Engine VMs have adequate access scopes for the
Ops Agent, but old VMs or those on which scopes have been modified
might not. For more information about access scopes and service accounts, see
Authorization
...
Ops Agent, then use the following steps to add the roles described in
Create a service account :
...
To authorize the Ops Agent installed on a Compute Engine VM instance
...
To authorize the Ops Agent installed on a VM instance by using
...
to point to the location of the service account key file, and the variable
must be visible to the agent's process. For configuration information, see
...
Note: The Ops Agent isn't the only service that uses the environment
