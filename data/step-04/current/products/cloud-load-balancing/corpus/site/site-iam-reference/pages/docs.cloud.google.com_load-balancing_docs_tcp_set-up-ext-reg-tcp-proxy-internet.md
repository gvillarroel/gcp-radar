---
title: "Set up a regional external proxy Network Load Balancer with an external backend\
  \ \_|\_ Cloud Load Balancing \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/load-balancing/docs/tcp/set-up-ext-reg-tcp-proxy-internet
...
source_metadata:
  url: https://docs.cloud.google.com/load-balancing/docs/tcp/set-up-ext-reg-tcp-proxy-internet
  title: "Set up a regional external proxy Network Load Balancer with an external\
    \ backend \_|\_ Cloud Load Balancing \_|\_ Google Cloud Documentation"
  fetched_via: browser_cdp
  cdp_url: http://127.0.0.1:9222
---

Set up a regional external proxy Network Load Balancer with an external backend | Cloud Load Balancing | Google Cloud Documentation
...
Test the load balancer
What's next
The guide shows you how to configure a regional external proxy Network Load Balancer that proxies
...
including the
limitations .
The following architecture diagram shows a regional external proxy Network Load Balancer frontend with
an external backend.
Figure 1. A regional external proxy Network Load Balancer with an external backend (click to enlarge).
Note: Regional external proxy Network Load Balancers support both the
Premium and Standard Network Service Tiers. This procedure demonstrates the setup
with Standard Tier.
Permissions
...
For Proxy or passthrough , select Proxy load balancer and
click Next .
For Public facing or internal , select Public facing
(external) and click Next .
For Global or single region deployment , select Best for regional
...
Enter a Name .
For Network Service Tier , select Standard .
For IP address , select the IP address reserved previously:
LB_IP_ADDRESS .
For Port number , specify a single port number between 1-65535. The
forwarding rule only forwards packets with a matching destination port.
Click Done .
...
For `--ports`, specify a single port number
from 1-65535. The forwarding rule only
forwards packets with a matching destination port.
...
Now that you have configured your load balancer, you can test sending
...
gcloud compute addresses describe LB_IP_ADDRESS
Send traffic to the load balancer on the IP address
and port specified when creating the load balancer forwarding rule. Testing
whether the internet NEG backends are responding to requests depends on the
service running on the external endpoints.
What's next
To set up logging and monitoring for your load balancer, see
Proxy Network Load Balancer logging and
monitoring .
To delete the resources that you created so you are not billed for them in the
future, see Clean up the load balancer setup .
Was this helpful?
Send feedback
