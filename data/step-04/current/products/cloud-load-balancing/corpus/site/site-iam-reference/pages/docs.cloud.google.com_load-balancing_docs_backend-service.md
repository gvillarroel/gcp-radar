...
Backend services overview | Cloud Load Balancing | Google Cloud Documentation
...
Balancing mode, target capacity, and capacity scaler Supported backends
Balancing modes for Application Load Balancers and Cloud Service Mesh
Balancing modes for Proxy Network Load Balancers
Target capacity specifications Connection balancing mode
Rate balancing mode
...
Backend subsetting Backend subsetting for regional internal Application Load Balancers
Backend subsetting for internal passthrough Network Load Balancer
Backend subsetting pricing
Session affinity
Backend service timeout
Health checks
Additional features enabled on the backend service resource Cloud CDN
Cloud Armor
IAP
...
Cloud Run, or Cloud Run functions resources, or
One global internet NEG for an external backend
EXTERNAL 4
...
One or more service bindings ( Preview )
...
A service binding is a backend that establishes a connection between a
backend service in Cloud Service Mesh and a service registered in
Service Directory. A backend service can reference several
service bindings. A backend service with a service binding cannot reference
any other type of backend. For more information, see
