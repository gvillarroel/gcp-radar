---
title: "Bare Metal HSM \_|\_ Cloud Key Management Service \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/kms/docs/bare-metal-hsm
...
This page provides an overview of the Bare Metal HSM solution.
Overview
Bare Metal HSM is an infrastructure-as-a-service offering that lets
you deploy customer-owned hardware security modules (HSMs) next to your
...
physical and network security, rack and shelf space, power, and network
integration for a monthly fee.
Bare Metal HSM lets you contract directly with Google for placement of
your HSMs. HSMs are placed within specified colocation facilities and connect to
Google Cloud.
The Bare Metal HSM solution is supported in colocation facilities with
...
Comparison with Bare Metal Rack HSM
...
accessibility
to the HSMs, test the solution, and sign off.
Support model
...
You must erase all data and initialize all HSM to factory default.
...
Bare Metal HSM solution is hosted in facilities that are fully
...
appropriate Google Cloud regions. The HSMs used are your choice, but they must
comply with the HSM equipment requirements.
Google pre-configures the racks, top-of-rack switches, and connectivity.
The switches are from different vendors for each pair of racks. For the
Bare Metal HSM solution, you have your own dedicated racks and
...
racks, switching, and connection. Google has no access to the data or keys
on your HSM.
You must deploy HSMs with complete remote management capability.
You can't physically access your HSMs while they are in the colocation
facility.
Google provides a remote hands service. Customer visits to the facility are
not permitted. You are responsible for your own compliance and audit
requirements.
...
available in the current model of the top-of-rack switch, the number of
rack units taken up the HSM model, and the power draw of the HSMs.
Note: You are provided with a rack that has 36 rack units available.
Power
...
Security
The HSMs cannot be equipped with cameras or wireless networks such as
Bluetooth.
...
Deploy a minimum of two HSMs per zone (at least one HSM per rack in at least
two racks).
...
Network topology
A pair of racks in a single zone is covered by a 99.9% SLA.
A full deployment across two zones provides a 99.99% SLA. This can be achieved
using two regions or, where available, two zones in the same region.
Applications should be designed to take advantage of this redundancy model.
An application should be able to fail over from zone 1 to zone 2 within a single
location, from HSM to HSM.
...
99.99% availability.
BGP between the top-of-rack switches and the Cloud Routers in your project
exchange reachability information to route between Google Cloud project
resources and the HSMs.
...
per zone using the Cloud Routers from the previous step. Create the
attachments with the pre-activate option enabled. There should be a total of
four attachments per zone. If the attachments were created without the
pre-activation option enabled, you can activate the connections
manually.
...
To achieve 99.99% availability, use the steps in
Establishing 99.99% Availability for
Partner Interconnect .
Deployments in a single zone have 99.9% availability until the second
zone is available. For this case, see
Establishing 99.9% Availability for
Partner Interconnect
Configure firewall rules as needed to allow traffic between your premises and
project resources.
Location compatibility
Bare Metal HSM is available in the following Cloud KMS locations:
Geographical area
Location name
...
If you're interested in Bare Metal HSM with Google, reach out to your
account representative for additional assistance.
Send feedback
