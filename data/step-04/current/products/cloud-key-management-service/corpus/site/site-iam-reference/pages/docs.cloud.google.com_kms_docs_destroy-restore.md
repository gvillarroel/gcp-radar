---
title: "Destroy and restore key versions \_|\_ Cloud Key Management Service \_|\_\
  \ Google Cloud Documentation"
url: https://docs.cloud.google.com/kms/docs/destroy-restore
...
Destroy and restore key versions | Cloud Key Management Service | Google Cloud Documentation
...
Destroy a key version Destroying external keys
...
state for a configurable time. During the scheduled for destruction duration,
...
creation . Your organization can enforce
...
data loss. Make sure that you understand the risks before
destroying a key version.
You can also temporarily disable a key version . We
...
View key usage tracking details for the key.
...
Disable the key version . Disabling the key
...
Warning: It is your responsibility to ensure that a key version is
safe to destroy. Google is not responsible for outages, loss of data, or
compliance issues that result from you destroying a key version.
...
scheduled for destruction duration has
...
material can remain in Google systems for up to 45 days from the
...
Note: Destroying all active key versions doesn't change the key's rotation
schedule if one is configured. If you want to discontinue the automatic creation
of new key versions, then you must also disable automatic
rotation on the key. If you don't disable automatic
rotation, new key versions continue to be created on schedule.
...
Caution: You can't recreate a Cloud EKM key version that has been
destroyed, even if you use the same external key URI or key path.
...
key version in Google Cloud. After the Cloud EKM key version is
...
destroys the key version in Google Cloud, and then sends a destruction
...
disabled. You must enable the key before it can be used.
Required IAM permissions
