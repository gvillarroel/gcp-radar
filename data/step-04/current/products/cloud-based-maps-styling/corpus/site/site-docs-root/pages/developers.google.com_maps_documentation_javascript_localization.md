...
url: https://developers.google.com/maps/documentation/javascript/localization
...
Utilize the language parameter in the script tag to display map information in a specific language, overriding browser settings if needed.
Apply the region parameter in the script tag to refine map tiles, bias application results, and ensure compliance with local laws.
For optimal results and legal adherence, consider setting both language and region parameters when localizing your map.
You can customize your map for a specific country or region in the following
ways:
Change the default language settings.
Specify a region code, which alters the map's behavior based on a given
...
consider setting the region too. This helps ensure
...
languages . Note that new languages are added often, so this list may not
be exhaustive.
Region localization
