---
title: "Libraries \_|\_ Maps JavaScript API \_|\_ Google for Developers"
...
Libraries | Maps JavaScript API | Google for Developers
...
Page Summary
outlined_flag
The Maps JavaScript API loads quickly by using libraries that are imported as needed with google.maps.importLibrary() .
You can include your API key and specify the API version in a bootstrap loader script to initially load the API.
Dynamic Library Import offers a modular approach, letting you load specific libraries like maps , places , marker , etc., on demand.
For backward compatibility, a legacy bootstrap script tag is also available, supporting libraries such as drawing , geometry , and places .
Both methods allow customization: Dynamic Import uses importLibrary() within an async function, while the legacy method uses the libraries parameter in the script tag's URL.
To load the JavaScript code for the Maps JavaScript API, you
...
dynamic library import and direct script loading tag :
...
drawing ( google.maps.DrawingLibrary ) (deprecated) block
