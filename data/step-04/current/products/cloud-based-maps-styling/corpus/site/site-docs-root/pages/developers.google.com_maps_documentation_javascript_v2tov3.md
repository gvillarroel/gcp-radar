---
title: "Upgrading Your Maps JavaScript API Application from V2 to V3 \_|\_ Google\
  \ for Developers"
...
Upgrading Your Maps JavaScript API Application from V2 to V3 | Google for Developers
...
Upgrade your application Your New Key
Load the API
...
Summary
Detailed Reference
Events New Event for MVC Support
Remove Event Listeners
Listen to DOM Events
Use Passed Arguments in Events
Controls Changes in Control Types
Add Controls to the Map
Position Controls on the Map
Custom Controls
Overlays Add and remove overlays
Draggable Markers
Icons
Polylines
Polygons
...
Was this helpful?
Send feedback
Upgrading Your Maps Java Script API Application from V2 to V3
...
Changes in V3 of the Maps Java Script API
...
additional quota when required.
...
client parameter.
(Google Maps Platform Premium Plan only) Verify that the
client parameter is supplied as
explained in the
Google Maps Platform Premium Plan Developer's Guide .
Remove the v parameter to request the
latest released version or change its value accordingly to the
v3 versioning
scheme.
(Optional) Replace the hl parameter with
language and preserve its value.
(Optional) Add a libraries parameter to
load optional
libraries .
...
tags.
Google Maps Platform Premium Plan customers will want to read through the
Google Maps Platform Premium Plan
Documentation .
The
Google Geo Developers
Blog is a great way to find out about the latest changes to the
API.
...
and are named using a property _changed
convention.
...
Polylines
A polyline consists of an array of LatLng s, plus a series
...
the v3 API supports multiple, concurrent InfoWindow s on
each map.
