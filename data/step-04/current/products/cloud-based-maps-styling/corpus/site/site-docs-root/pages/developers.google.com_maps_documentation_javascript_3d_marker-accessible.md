---
title: "Make markers accessible \_|\_ Maps JavaScript API \_|\_ Google for Developers"
url: https://developers.google.com/maps/documentation/javascript/3d/marker-accessible
...
Make markers accessible | Maps JavaScript API | Google for Developers
...
Text specified in the title option is readable by screen readers, and will
display when a user holds the mouse pointer over the marker.
Increasing the size of markers reduces the precision required to interact
with it across all devices — especially touchscreen devices — and improves
accessibility. Default markers meet the WCAG AA minimum
size
standard but for developers aiming to comply with the WCAG AAA
target
size standard the marker size should be increased.
See basic marker
customization to learn
...
Use the tab key to focus on the first marker; if there are multiple markers
on the same map, use the arrow keys to cycle through the markers.
If the marker is clickable, press the enter key to "click". If a marker has
an info window, you can open it by clicking, or by pressing the enter key or
space bar. When the info window closes, focus will return to the associated
marker.
Press tab again to continue moving through the rest of the map controls.
To enhance accessibility:
Set descriptive text for a marker using Marker3DInteractiveElement.title
option.
