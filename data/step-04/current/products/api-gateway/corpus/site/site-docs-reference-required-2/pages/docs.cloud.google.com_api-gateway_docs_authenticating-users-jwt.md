---
title: "Using JWT to authenticate users \_|\_ API Gateway \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/api-gateway/docs/authenticating-users-jwt
...
URL_PUBLIC_KEY is the URL to the public key.
...
ISSUER is the issuer of the token.
URL_PUBLIC_KEY is the URL to the public key.
CLLIENT_ID is a list of client IDs as strings.
Add a security section at either the API level to apply to the entire
API, or at the method level to apply to a specific method.
security :
- SCHEME_NAME : []
...
The x-google-jwks_uri (OpenAPI 2.0) or jwksUri (OpenAPI 3.x) field is required.
API Gateway supports two asymmetric public key formats defined
by this OpenAPI extension:
JWK set format .
For example:
OpenAPI 2.0
...
If you are using a symmetric key format, set the x-google-jwks_uri (OpenAPI 2.0) or jwksUri (OpenAPI 3.x) to
the URI of a file that contains the base64url-encoded key string.
Note : When API Gateway sends requests to the x-google-jwks_uri or jwksUri ,
the request will contain the x-forwarded-proto header.
