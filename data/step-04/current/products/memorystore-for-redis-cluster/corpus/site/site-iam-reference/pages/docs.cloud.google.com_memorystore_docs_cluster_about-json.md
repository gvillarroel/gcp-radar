---
title: "About JSON \_|\_ Memorystore for Redis Cluster \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/memorystore/docs/cluster/about-json
...
manipulate structured data in Memorystore for Redis Cluster. Using JSON provides you with the
following benefits:
...
ideal choice for you. Also, by using JSON, you can encode complex datasets
inside of clusters in Memorystore for Redis Cluster without using schemas. You can
store, access, and update data in the clusters, and you don't have to manage
custom code to serialize and deserialize the data.
...
in JSON text.
Availability
If you create a cluster in Memorystore for Redis Cluster, version 8.0 and later, then
...
JSON.DEBUG
Retrieve information about a JSON document. Memorystore for Redis Cluster
...
Performance .
How Memorystore for Redis Cluster interacts with JSON
This section describes how Memorystore for Redis Cluster interacts with the JSON data
type.
Multiple conditional expressions
If there are multiple conditional expressions for filtering, then
Memorystore for Redis Cluster evaluates them in the following order:
...
than 128, then Memorystore for Redis Cluster rejects the document and you receive an
...
they have the same data type. When Memorystore for Redis Cluster receives a JSON
...
Memorystore for Redis Cluster outputs a number as part of a JSON response,
Memorystore for Redis Cluster converts the number from the internal binary
...
Direct arrays
Memorystore for Redis Cluster filters array objects directly. For example, if you have
data such as [ 0 , 1 , 2 , 3 ] and a path query such as
$[?(@ , then Memorystore for Redis Cluster returns
[ 0,1,2 ]. For data such as { "my_valkey_key":[0,1,2,3] }
and a path query such as $.my_valkey_key[?(@ ,
Memorystore for Redis Cluster also returns [ 0,1,2 ].
Array indexes
Memorystore for Redis Cluster lets you use positive and negative indexes for arrays.
...
Memorystore for Redis Cluster doesn't round indexes up or down. If you have an array
with a length of 3, and you call either a positive index that's greater than 3
or a negative index that's less than -3, then Memorystore for Redis Cluster doesn't
return a result.
