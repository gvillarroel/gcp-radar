---
title: "Create custom organization policy constraints \_|\_ Cloud DNS \_|\_ Google\
  \ Cloud Documentation"
url: https://docs.cloud.google.com/dns/docs/create-custom-constraints
...
for Cloud DNS resources. For example, if a request to create or
update a Cloud DNS instance fails to satisfy custom
constraint validation as set by your organization policy, the request fails
and an error will be returned to the caller.
Limitations
...
numbers, for example custom.restrictManagedZoneWithDeniedDescription . This field can contain up to
...
or numbers, for example, custom.restrictManagedZoneWithDeniedDescription . This field can contain up to 70
...
dns.googleapis.com/ManagedZone . Most resource types support up to 20 custom
constraints. If you attempt to create more custom constraints, the operation fails.
...
resource.description.contains('denied') .
...
example, custom.restrictManagedZoneWithDeniedDescription .
...
Description
Constraint syntax
Restrict logging for Cloud DNS instances
...
Cloud DNS supported resources
The following table lists the Cloud DNS resources that you can reference
