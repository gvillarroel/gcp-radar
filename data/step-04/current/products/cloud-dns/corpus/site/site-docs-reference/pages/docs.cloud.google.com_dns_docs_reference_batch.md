---
title: "Sending batch requests \_|\_ Cloud DNS \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/dns/docs/reference/batch
...
Overview
Each HTTP connection your client makes results in a certain amount of overhead. The Cloud DNS API supports batching, to allow your client to put several API calls into a single HTTP request.
...
Note : The batch system for the Cloud DNS API uses the same syntax as the OData batch processing system, but the semantics differ.
Batch details
...
A batch request is a single standard HTTP request containing multiple Cloud DNS API calls, using the multipart/mixed content type. Within that main HTTP request, each of the parts contains a nested HTTP request.
...
Example
The following example shows the use of batching with a generic (fictional) demo API called the Farm API. However, the same concepts apply to the Cloud DNS API.
