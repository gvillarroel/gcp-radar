---
title: "Creating a basic access level \_|\_ Access Context Manager \_|\_ Google Cloud\
  \ Documentation"
...
requires an identity to interact with other Google Cloud services.
...
Save the file. In this example, the file is named CONDITIONS.yaml .
Create the access level, using the create command.
gcloud access-context-manager levels create NAME \
--title TITLE \
--basic-level-spec CONDITIONS.yaml \
--combine-function = OR \
