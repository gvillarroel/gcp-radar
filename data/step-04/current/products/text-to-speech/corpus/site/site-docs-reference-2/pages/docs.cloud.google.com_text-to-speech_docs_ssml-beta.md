---
title: "Speech Synthesis Markup Language (SSML) \_|\_ Cloud Text-to-Speech \_|\_ Google\
  \ Cloud Documentation"
url: https://docs.cloud.google.com/text-to-speech/docs/ssml-beta
...
source_metadata:
  url: https://docs.cloud.google.com/text-to-speech/docs/ssml-beta
  title: "Speech Synthesis Markup Language (SSML) \_|\_ Cloud Text-to-Speech \_|\_\
...
code samples.
Note: SSML characters count toward character limits. See the
quotas & limits page for more information.
...
Note: You can practice using SSML tags using the Text-to-Speech product demo
on the product overview page.
Click the ssml link under the bottom right corner of the "Convert your text
to speech right now" demo's text box to view your input in ssml format.
...
Use the START and END marks instead of adding custom
marks near the beginning or end of the SSML.
Do not add consecutive marks in your SSML. Marks in rapid succession might not generate events.
Additionally, if there is no audio generated between marks, then
events won't be generated. For example, avoid doing the following:
Sentence 1. <mark name=a><mark name=b> Sentence 2.
Note: For convenience, you automatically get START and
END events in your responses for Interactive Canvas .
These marks are not included in the W3 spec.
Attribute
Description
name
...
Semitones
Increase or decrease pitch by " N " semitones using "+ N st" or "- N st" respectively. Note that "+/-" and "st" are required.
Percentage
...
Note: Set the alphabet field to either ipa or x-sampa , depending on which
phonetic alphabet you're using. The only exceptions are Japanese, which must be
specified as yomigana , and the Chinese languages,
which must be specified as pinyin or jyutping . See the
phonemes page to see which phonetic alphabets
are available for you language.
Stress markers
...
owner</speak>
Note: When a <voice> tag specifies a voice name, the type of voice must be
compatible with the requested voice name .
