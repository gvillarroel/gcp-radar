---
title: "Build and save queries by using the Logging query language \_|\_ Google Cloud\
  \ Documentation"
url: https://docs.cloud.google.com/logging/docs/view/building-queries
...
source_metadata:
  url: https://docs.cloud.google.com/logging/docs/view/building-queries
  title: "Build and save queries by using the Logging query language \_|\_ Google\
    \ Cloud Documentation"
...
Logging .
Select the Google Cloud project or other Google Cloud
...
Note: Substring searches are slower than token-based searches. Therefore, we
recommend that you use token-based searches when possible.
...
to precedence rules . Parentheses are parsed like any other
character.
...
statement, the precedence rules specify that the OR operation
...
format. For more information, see Correlate log entries .
...
options:
Stream : To run the query and stream the results, choose this
option.
...
and enter the summary fields that you want to display.
...
can share it with other members of the Google Cloud project. Once you share
...
fields and enter the summary fields that you want
to display.
Truncate summary fields (Optional): Enable
Truncate summary fields and select the number of characters to
truncate to and whether truncation occurs at the beginning or end of the
...
share your query with other members of the Google Cloud project.
Click Save query . Your saved queries appear in a
list under the Saved tab.
To run a saved query, click Run . To run the query and stream
...
savedQueries.create .
You can execute the savedQueries.create method by using the
APIs Explorer widget on the method's reference page.
For Logs Explorer queries, you must specify the loggingQuery
field. The following example illustrates a sample request body, which
...
To share the query with other members of the Google Cloud project, specify
...
in the Google Cloud project by clicking the menu_book Query
library button:
Console
In the Google Cloud console, go to the
segment
...
Google Cloud project, such as the Google Cloud products you're using.
...
cases and Google Cloud products. These queries can help you efficiently
...
of available queries and subsets of queries based on Google Cloud
