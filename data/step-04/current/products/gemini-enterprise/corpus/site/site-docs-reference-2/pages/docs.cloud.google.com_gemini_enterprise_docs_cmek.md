---
title: "Customer-managed encryption keys \_|\_ Gemini Enterprise \_|\_ Google Cloud\
  \ Documentation"
url: https://docs.cloud.google.com/gemini/enterprise/docs/cmek
...
By default, Gemini Enterprise encrypts customer content at
rest. Gemini Enterprise handles encryption for you without any
additional actions on your part. This option is called Google default encryption .
If you want to control your encryption keys, then you can use customer-managed encryption keys
(CMEKs) in Cloud KMS with CMEK-integrated services including
Gemini Enterprise. Using Cloud KMS keys gives you control over their protection
...
Gemini Enterprise resources is similar to using Google default encryption.
...
You can't use Terraform to configure CMEK for Gemini Enterprise.
Before you begin
...
If you use third-party connectors, create three single-region symmetric
Cloud KMS keys. It is optional otherwise.
Set the rotation period to Never (Manual rotation) .
For Location , select region .
Select the single-regions in the following table from the location drop-down.
You need to have all three keys with you when you
Register your Cloud KMS keys for third-party connectors .
us single-regions
...
To determine if there is an active key in Gemini Enterprise for a
location, see View Cloud KMS keys .
You have the Discovery Engine Admin
( roles/discoveryengine.admin ) role.
Procedure
REST
To register your own key for Gemini Enterprise, follow these
steps:
...
To register your own key for Gemini Enterprise, follow these
steps:
Call the UpdateCmekConfig method with the key that
...
"https:// LOCATION -discoveryengine.googleapis.com/v1/projects/ PROJECT_ID /locations/ LOCATION /cmekConfigs/ CMEK_CONFIG_ID ?set_default= SET_DEFAULT "
Replace the following:
KMS_PROJECT_ID : the ID of your project that
contains the key. The project number won't work.
KMS_LOCATION : the multi-region of your key:
us or europe .
KEY_RING : the name of the key ring that holds
the key.
KEY_NAME : the name of the key.
...
CMEK_CONFIG_ID : Set a unique ID for the
CmekConfig resource, for example, default_cmek_config .
SET_DEFAULT : set to true to use the key as the
default key for subsequent data stores created in the multi-region.
Example command and result
$ curl -X PATCH
-H "Authorization: Bearer $(gcloud auth print-access-token)"
-H "Content-Type: application/json
...
protected by the key. For general information about creating data stores, see
About apps and data stores .
If you use third-party connectors and want to protect
your third-party data with your own key, then create
three additional single-region keys as follows:
curl -X PATCH \
-H "Authorization: Bearer $( gcloud auth print-access-token ) " \
-H "Content-Type: application/json" \
-d '{
...
REGION_1 : for the eu location, set this to
europe-west1 , and for the us location, set this to us-east1 .
REGION_2 : for the eu location, set this to
europe-west4 , and for the us location, set this to us-central1 .
REGION_3 : for the eu location, set this to
europe-north1 , and for the us location, set this to us-west1 .
KEY_RING : the name of the key ring that holds the
key.
KEY_NAME : the name of the multi-region key.
LOCATION : the multi-region of your data store:
us or eu .
PROJECT_ID : the ID of your project that contains
the data store.
CMEK_CONFIG_ID : Set to the ID for the
CmekConfig resource. Use the same value as in Step 1, for example,
default_cmek_config .
SET_DEFAULT : set to true to use the key as the
default key for subsequent data stores created in the multi-region.
Example command and result
...
method to unregister your Cloud KMS key .
Before you begin
Make sure that the regions aren't already protected by keys. The following
procedure fails if a key is already registered for the region through the REST
command.
To determine if there is an active key for a location, see View
Cloud KMS keys .
Procedure
To register your own key when using Gemini Enterprise third-party
...
default_cmek_config .
If you are connecting a third-party data source ,
click Set single region keys .
Note: You need to add all three single-region keys together. Therefore,
make sure that you have created the three single-region keys for your location.
Click Set single region keys.
Click the Select a cloud kms key drop-down, and select the key for
each region.
If the key is in a different project, click Switch project ,
click your project name, type the name of the key you
created, and select the key.
...
To unregister your key from Gemini Enterprise, follow these
steps:
...
Note: A CmekConfig resource can only be deleted if no Gemini Enterprise
...
of app-owned core information held by Gemini Enterprise, such as the
...
Gemini Enterprise data store. For general information about
...
been registered for use with Gemini Enterprise in the specified location.
