---
title: "Integration Connectors release notes \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/integration-connectors/docs/release-notes
...
This page documents production updates to Integration Connectors.
...
December 16, 2025
Feature
MCP connector is now available in preview
Integration Connectors support the Model Context Protocol (MCP) connector. You can use this connector to create and configure connections for secure data exchange and operations in your business applications. Within Application Integration, you can use MCP through a Connectors task .
October 30, 2025
Feature
Apigee API hub connector is now available in preview
Integration Connectors now support the Apigee API hub connector. You can use this connector to discover, manage, and govern your APIs in Apigee API hub.
September 29, 2025
Feature
HL7 Connectors now support new actions
The HL7 connector now supports the following new transformation actions:
Transform HL7 to XML action : This action converts a standard HL7 message into an XML representation.
Transform XML to HL7 action : This action converts an XML message back into the HL7 format.
September 16, 2025
Change
Salesforce connected app requires installation for OAuth 2.0 authentication
A new security requirement from Salesforce restricts the use of uninstalled connected apps. To ensure your connections remain functional, all users must install connected app in your Salesforce account.
When establishing a new Salesforce connection using OAuth 2.0 authentication, you are now required to install the connected app in your Salesforce account. This step is also necessary for existing connections using OAuth 2.0 authentication if the connected app is not already installed, as failure to do so may cause them to stop working.
For more information, see Install the OAuth connected app .
June 30, 2025
Announcement
The customer-managed encryption keys (CMEKs) feature is now generally available (GA) .
June 24, 2025
Announcement
You can use the following connectors that are now available in Google Cloud Marketplace to create connections and use them in Application Integration.
...
IP Geolocation Api
KAPSARC Data Portal
KSA Open Data Platform
Morocco Open Data Portal
Qatar Open Data Portal
Qatar National Address Service (QNAS)
Rest Countries
...
. This connector also supports event subscription , which you can leverage in your integrations by using the Jira Server (Data Center) trigger . This trigger is available in preview .
...
May 05, 2025
Announcement
The ZATCA connector is now available in Google Cloud Marketplace. You can use this connector to create connections and use them in Application Integration.
Announcement
The following connectors are now generally available (GA) :
LinkedIn
Okta
...
April 07, 2025
Feature
You can now use a Google-managed OAuth client instead of creating your own client for the following connectors, which are in preview :
Gmail
Google Docs
Google Drive
Google Forms
...
March 10, 2025
Feature
The Microsoft Exchange connector is now GA .
To view the list of all the GA connectors, see Connectors in GA .
February 21, 2025
Feature
The following connectors are now generally available (GA) :
...
February 05, 2025
Feature
The Azure AD connector now supports event subscription which you can leverage in your integrations by using the Azure AD trigger . This trigger is available in preview .
January 06, 2025
Feature
Integration Connectors now supports customer-managed encryption keys (CMEKs) . This feature is in preview .
Feature
The HL7 connector is now available in preview . The connector also supports event subscription , which you can leverage in your integrations by using the HL7 trigger .
December 19, 2024
Feature
You can now build and publish custom connectors for Google Cloud Marketplace . This feature is in preview .
The following connectors built by partners are available in Google Cloud Marketplace. You can use these connectors to create connections and use them in Application Integration.
Exact HR
...
November 18, 2024
Feature
The BigQuery connector now supports the Array data type.
October 18, 2024
Change
The private network connectivity documentation has been restructured to include information about the following networking patterns:
...
To view the list of all the GA connectors, see Connectors in GA .
Feature
The BigQuery connector now supports OAuth 2.0 - Authorization code .
Change
...
Feature
The Jira Cloud connector now supports OAuth 2.0 - Authorization code .
July 07, 2024
Announcement
...
Versions
Logo
File upload for OpenAPI specification
May 24, 2024
Feature
...
March 03, 2024
Announcement
The event subscription feature is now generally available (GA) .
Feature
...
January 22, 2024
Announcement
The VPC Service Controls feature for Integration Connectors is now generally available .
January 17, 2024
Feature
Terraform support
...
January 16, 2024
Announcement
The Webhook connector is now available in preview. This connector lets you listen to events from your backend, and is designed
to be used with the Webhook trigger in Application Integration .
January 08, 2024
Announcement
The following connectors are now generally available (GA) :
Amazon S3
...
Subscribe to the Integration Connector release notifications .
Regularly check for the presence of the Preview label next to the connector name in the List connections page . If a connector doesn't have the Preview label, it means that the connector is generally available.
...
May 29, 2023
Feature
Support for proxy configuration in the following connectors:
Apache CouchDB
BigCommerce
BigQuery
Box
...
Note: Rollouts of this release will begin today and may take four or more business days to be completed across all Google Cloud zones. You may not be able to provision Integration Connectors until the rollout is complete.
Send feedback
