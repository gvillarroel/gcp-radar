---
title: "Create custom organization policy constraints \_|\_ Dataform \_|\_ Google\
  \ Cloud Documentation"
url: https://docs.cloud.google.com/dataform/docs/create-custom-constraints
...
The use of custom organization policies adds an additional layer of security,
compliance, and governance. For example, you can define policies that allow
resource creation only in a particular region, or that forbid using Git remotes
...
If you change the policy, an existing resource remains valid, unless you
change the resource from a compliant to a non-compliant value using the
Google Cloud console or an RPC.
We recommend making all existing resources compliant with a policy before
enforcing a new policy. To check which existing resources won't be compliant
after you enforce a policy, use the
...
numbers, for example custom.restrictRepositoryLocation . This field can contain up to
...
or numbers, for example, custom.restrictRepositoryLocation . This field can contain up to 70
...
dataform.googleapis.com/Repository . Most resource types support up to 20 custom
constraints. If you attempt to create more custom constraints, the operation fails.
...
"resource.name.contains('/locations/us-central1/')" .
...
example, custom.restrictRepositoryLocation .
...
dataform.googleapis.com/Repository resources to be created in a particular region.
Before you begin, you must have the following:
Your organization ID
A project ID
Create the constraint
To create a custom constraint, follow these steps:
Save the following file as constraint-restrict-repository-location.yaml :
...
This file defines a constraint where for every create or update operation on
the dataform.googleapis.com/Repository resource, the operation is allowed only if the
selected region for the resource is us-central1 .
...
Save the following file as policy-restrict-repository-location.yaml :
...
Follow the steps in
Create a repository
to create a dataform.googleapis.com/Repository resource in your project. Choose a region that
is different from the region that you specified in the custom constraint.
...
Description
Constraint syntax
All resources must be created in the us-central1 region.
name : organizations/ ORGANIZATION_ID /customConstraints/custom.restrictRepositoryLocation
resourceTypes :
- dataform.googleapis.com/Repository
methodTypes :
...
displayName : Only us-central1 region is allowed.
description : All resources must be created in the us-central1 region.
Only allow configuration of a Git remote to connect using HTTPS to repositories in the Google organization on GitHub.
...
The following table lists the Dataform resources that you can reference
