---
title: "Enable customer-managed encryption keys \_|\_ Knowledge Catalog \_|\_ Google\
  \ Cloud Documentation"
...
This document describes how to encrypt Knowledge Catalog (formerly Dataplex Universal Catalog) data with
customer-managed encryption keys (CMEK).
Overview
By default, Knowledge Catalog encrypts customer content at
rest. Knowledge Catalog handles encryption for you without any
additional actions on your part. This option is called Google default encryption .
If you want to control your encryption keys, then you can use customer-managed encryption keys
(CMEKs) in Cloud KMS with CMEK-integrated services including
Knowledge Catalog. Using Cloud KMS keys gives you control over their protection
...
Knowledge Catalog resources is similar to using Google default encryption.
For more information about your encryption
options, see Customer-managed encryption keys (CMEK) .
Knowledge Catalog uses one CMEK
per location for all Knowledge Catalog resources.
...
keys.
How CMEK works with Knowledge Catalog
Knowledge Catalog encryption administrators in your Google Cloud project
can configure CMEK for Knowledge Catalog by providing the
Cloud KMS key. Then, Knowledge Catalog uses the specified
Cloud KMS key to encrypt all data, including existing data and any new
resources created within Knowledge Catalog.
Supported features
Knowledge Catalog supports CMEK encryption for the following features:
Auto data quality
Data profiling
Data discovery
Data insights
...
Assured Workloads customers can't use other Knowledge Catalog
...
The Organization Administrator can switch to CMEK in Knowledge Catalog
for any location.
Knowledge Catalog supports Cloud KMS keys, Cloud HSM keys,
...
Knowledge Catalog retains data backups for a maximum of 15 days. Any
...
After a Cloud KMS key is configured for Knowledge Catalog, it
cannot be updated or changed.
Knowledge Catalog supports organization-level encryption only. As a
result, the encryption configuration is set at the organization level for a
given location and is used to encrypt Knowledge Catalog data for all
...
Knowledge Catalog doesn't support CMEK in the global region.
When CMEK is enabled for Knowledge Catalog, the following features don't
work:
Cross-organizational metadata Search
Before you configure CMEK in Knowledge Catalog, deactivate the
Data Catalog (deprecated) API in all projects.
When you enable CMEK in Knowledge Catalog, don't migrate projects into
or out of the CMEK organization. Modifying a project's organizational
placement will result in a permanent loss of Knowledge Catalog-stored
metadata for that project.
Upgrade existing CMEK configurations
If you enabled CMEK in Knowledge Catalog before November 7, 2025, run
the following command to extend CMEK coverage to
Knowledge Catalog and Knowledge Catalog
search:
...
contains the key.
When the data is encrypted, you might get a Knowledge Catalog availability issue .
If you enable CMEK for the first time after November 7, 2025,
Knowledge Catalog Metastore and Search are included in the
...
or destroyed, Knowledge Catalog data becomes inaccessible. If the key is
...
Knowledge Catalog data will be permanently lost.
In cases where Cloud KMS is temporarily unavailable,
Knowledge Catalog continues to support full operations on a best-effort
...
Knowledge Catalog availability
The following sections outline the process and expected operational impact when
you enable CMEK for your Knowledge Catalog organization.
Initial infrastructure provisioning
After you save the encryption configuration, Knowledge Catalog sets up
the necessary infrastructure. This process typically takes 6 to 8 hours. During
this provisioning phase, you retain full access to all Knowledge Catalog
...
After infrastructure provisioning, Knowledge Catalog begins encrypting
...
Knowledge Catalog, all existing data is encrypted. This one-time operation
...
within Knowledge Catalog is automatically encrypted using the configured
...
Knowledge Catalog. You can use a key
...
specific Google-managed service account that Knowledge Catalog can use
...
A CMEK-specific service account is also created, formatted as service-org- ORG_ID @gcp-sa-dataplex-cmek.iam.gserviceaccount.com . The CMEK-specific service account is used for encrypting and decrypting data stored in Knowledge Catalog.
...
( roles/cloudkms.cryptoKeyEncrypterDecrypter ) to the Knowledge Catalog
...
access to the Knowledge Catalog service account:
...
Configure Knowledge Catalog to use your CMEK key.
Console
...
gcloud
Set the encryption config in Knowledge Catalog:
...
Audit Knowledge Catalog requests to Cloud KMS by enabling
audit logging
...
Knowledge Catalog resources that don't have CMEK protection.
...
Knowledge Catalog is restricted to using CMEK keys only from the
specified locations.
If a Cloud KMS key from a disallowed project is provided, requests
to create CMEK-protected resources in Knowledge Catalog will fail.
