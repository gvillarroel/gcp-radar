---
title: "Data quality tasks overview \_|\_ Knowledge Catalog \_|\_ Google Cloud Documentation"
...
offering.
Knowledge Catalog data quality tasks let you define and run
data quality checks across tables in BigQuery and
Cloud Storage. Knowledge Catalog data quality tasks also let you
apply regular data controls in BigQuery environments.
When to create Knowledge Catalog data quality tasks
Knowledge Catalog data quality tasks can help you with the following:
...
Serverless implementation. Knowledge Catalog does not need any
...
through the serverless scheduler in Knowledge Catalog, or use the
Dataplex API through external schedulers like Cloud Composer
for pipeline integration.
Managed experience. Knowledge Catalog uses an open source data
quality engine, CloudDQ ,
to run data quality checks. However, Knowledge Catalog provides a
seamless managed experience for performing your data quality checks.
How data quality tasks work
The following diagram shows how Knowledge Catalog data quality tasks work:
Input from users
...
Knowledge Catalog data quality task is used.
Tables to validate
...
inside or outside a Knowledge Catalog zone.
BigQuery and Cloud Storage tables that are validated
in a single run can belong to different projects.
Knowledge Catalog data quality task : A Knowledge Catalog data quality task is configured with a prebuilt,
...
the Knowledge Catalog data quality task runs on a serverless Spark
...
specification file.
Pricing
When you run Knowledge Catalog data quality tasks, you are charged for
BigQuery and Managed Service for Apache Spark (Batches) usage.
The Knowledge Catalog data quality task converts the specification file
to BigQuery queries and runs them in the user project. See
BigQuery pricing .
Knowledge Catalog uses Spark to run the prebuilt, Google-maintained open source CloudDQ
driver program to convert user specification to BigQuery
queries. See Managed Service for Apache Spark pricing .
There are no charges for using Knowledge Catalog to organize data or using the serverless
scheduler in Knowledge Catalog to schedule data quality checks. See
Knowledge Catalog pricing .
What's next
Create Knowledge Catalog data quality checks .
Send feedback
