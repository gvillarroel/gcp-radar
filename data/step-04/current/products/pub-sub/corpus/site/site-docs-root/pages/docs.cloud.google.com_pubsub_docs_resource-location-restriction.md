---
title: "Configure message storage policies \_|\_ Pub/Sub \_|\_ Google Cloud Documentation"
...
If you publish messages to the global Pub/Sub endpoint ,
Pub/Sub automatically stores the messages in the nearest
Google Cloud region. If you want to control the regions in which your
messages are stored and processed, you can configure
a message storage policy on your topic.
Note: Message storage policies can result in additional fees for outbound
messages. See
Resource location restriction implications
in the Pub/Sub pricing documentation.
...
Google Cloud regions that you specify, regardless of where the publish or
...
to where the published message enters the Google Cloud network.
...
error.
If a client runs within Google Cloud in one of the allowed regions
...
If a client runs within Google Cloud in a region that isn't allowed,
or runs outside of Google Cloud, it must use a locational or regional
...
based on the effective Resource Location Restriction organization policy .
...
more information, see Manage differences between organization and topic policies .
...
Set a message storage policy using an organization policy
Note: Making changes to your organization policy extends beyond Pub/Sub
resources. Before making any changes, ensure that the changes don't inadvertently
impact any other Google Cloud services that you're using.
Console
...
Click Google Cloud - Resource Location Restriction .
Click EDIT .
...
must run an update operation .
For more information about organization policies, see Manage your Google Cloud resources .
Configure a message storage policy when creating a topic
Console
When using the Google Cloud console, you can't configure a message storage policy
...
storage policies.
REST API reference: create
RPC API reference
Update message storage policies
Console
In the Google Cloud console, open the Topic details page.
Go to Topic details
...
To help you understand where message data is stored, Pub/Sub
offers metrics broken down by each Google Cloud region.
...
outside Google Cloud or in regions not allowed by the policy. Users who
...
Google Cloud are, as a rule, bound to the Pub/Sub
...
When publishing from outside of Google Cloud, an additional layer of routing is
involved to get the request to a nearby Google Cloud region where the
