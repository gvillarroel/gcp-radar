---
title: "Download Python packages using direct repository access \_|\_ Security Command\
  \ Center \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/security-command-center/docs/aoss-download-python-packages
...
Assured Open Source Software packages are
hosted in an Artifact Registry repository that is created in a project that you
control.
...
the free tier, see Download Python packages using direct repository access for
the free tier .
Before you begin
...
Authenticate with a Python keyring
...
To use the Python keyring to
authenticate to the Artifact Registry, see authenticating with
keyring and credential search
order . We recommend
that you use the Python keyring for authentication.
...
https://us-python.pkg.dev/ PROJECT_ID /assuredoss-python/simple with
the URL
...
selected when you set up Assured Open Source Software.
Install packages
...
To specify the Assured OSS Python packages that you want to
download, create two requirements.txt files. The sample files are the following:
...
packages and for all package versions in the requirements.txt file. We
recommend pinning dependencies with SHA-256 hashes in your
requirements.txt file to ensure security and reproducibility.
-v is optional. If specified, the command gives more
output .
...
The required script and its README.md file are available in a Cloud Storage bucket
( gs://cloud-aoss/utils/python-requirements-txt/v1.0 ) which can be downloaded
...
the credentials of the service account provided during the set up.
...
What's next
Download Java packages using direct repository access in the premium tier
