---
title: "Deploy a regional Google-managed certificate with Certificate Authority Service\
  \ \_|\_ Certificate Manager \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/certificate-manager/docs/deploy-google-managed-cas-regional
...
source_metadata:
  url: https://docs.cloud.google.com/certificate-manager/docs/deploy-google-managed-cas-regional
  title: "Deploy a regional Google-managed certificate with Certificate Authority\
...
regional Google-managed certificate with
Certificate Authority Service to a
regional external Application Load Balancer or to a regional internal Application Load Balancer.
If you want to deploy to global external load balancers or cross-region load
...
the certificate has been issued, even though it is not fully compatible
with Certificate Manager.
Google-managed TLS certificates that are issued from a regional CA pool are
global and can be be used in any region.
Required roles
...
For Region , select the same region as your certificate and CA pool.
...
resource.
LOCATION : the target Google Cloud location. You must specify the same location as the CA pool and the certificate.
...
gcloud
To create a regional Google-managed certificate with Certificate Authority Service,
use the certificate-manager certificates create command with the
issuance-config flag:
...
LOCATION : the target Google Cloud location. You must specify the same location as the CA pool,
certificate issuance configuration resource, and managed certificate.
API
...
Replace the following:
CERTIFICATE_NAME : the name of the certificate.
LOCATION : the target Google Cloud location where you created the Google-managed certificate.
...
To deploy the regional Google-managed certificate, attach it directly to the target proxy.
...
To attach certificate to an existing target HTTPS proxy, use the gcloud
...
referenced by a certificate issuance configuration, delete all certificate issuance
configurations that reference the CA pool. For more information, see Delete a CA
pool .
What's next
Manage certificates
