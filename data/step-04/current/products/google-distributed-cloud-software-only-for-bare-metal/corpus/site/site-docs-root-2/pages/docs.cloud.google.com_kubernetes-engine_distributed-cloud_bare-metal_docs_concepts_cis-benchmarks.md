---
title: "CIS Kubernetes benchmarks for Google Distributed Cloud (software only) on\
  \ bare metal \_|\_ Google Distributed Cloud (software only) for bare metal \_|\_\
  \ Google Cloud Documentation"
url: https://docs.cloud.google.com/kubernetes-engine/distributed-cloud/bare-metal/docs/concepts/cis-benchmarks
knowledge_key: corpus
source_id: site-docs-root-2
source_type: site
entrypoint: https://docs.cloud.google.com/kubernetes-engine/distributed-cloud/bare-metal/docs/concepts/about-bare-metal
source_metadata:
  url: https://docs.cloud.google.com/kubernetes-engine/distributed-cloud/bare-metal/docs/concepts/cis-benchmarks
  title: "CIS Kubernetes benchmarks for Google Distributed Cloud (software only) on\
    \ bare metal \_|\_ Google Distributed Cloud (software only) for bare metal \_\
...
Guides
Send feedback
CIS Kubernetes benchmarks for Google Distributed Cloud (software only) on bare metal
Stay organized with collections
Save and categorize content based on your preferences.
This document introduces the CIS Kubernetes Benchmark, explains how to audit
your compliance with the benchmark. For recommendations that you can't implement
yourself, the document explains what Google Distributed Cloud (software only) on
bare metal does to address the recommendation.
...
provides a set of recommendations for configuring Kubernetes to support a strong
security posture. The Benchmark is tied to a specific Kubernetes release. The
CIS Kubernetes Benchmark is written for the open source Kubernetes distribution
and intended to be as universally applicable across distributions as possible.
Versions
The following table lists the versions of Google Distributed Cloud, Kubernetes,
and the CIS Kubernetes Benchmark that were used to perform the assessment
...
CIS Kubernetes Benchmark
The following sections provide basic information to help you interpret the CIS
Kubernetes Benchmark status for Google Distributed Cloud.
...
supported as defined in the following table:
Scoring
Description
Automated
...
but other mechanisms exist to provide equivalent security controls.
Depends on Environment
Google Distributed Cloud doesn't configure items related to this
...
Google Distributed Cloud supports multiple cluster configurations and cluster
types. Every supported cluster type, admin, user, hybrid, and standalone, is
evaluated against the CIS benchmark. The following assessments and
justifications apply to all supported cluster types. For more information on the
supported cluster configurations, see Choosing deployment
models .
Status for Google Distributed Cloud
The following sections contain the assessments for new clusters created with the
specified version of Google Distributed Cloud. These assessments are
an indicator for how new clusters without user workloads perform against the CIS
Kubernetes Benchmark. If you deploy your own workloads before auditing
Benchmarks yourself , your results may be different.
Status of clusters:
1.34
#
Recommendation
...
L1
Warn
Failures and Equivalent Controls for a Google Distributed Cloud admin cluster:
1.34
#
Recommendation
Level
...
Fail
false
GDC (software only) for bare metal cluster nodes do not protect kernel defaults from Kubernetes, as customer workloads may want to modify these.
4.2.9
Ensure that the --tls-cert-file and --tls-private-key-file arguments are set as appropriate (Manual)
L2
Equivalent Control
...
Equivalent Control
755
Anthos clusters on bare metal Container Network Interface path is /opt/cni/bin , and its permission is set to 755 for the normal cluster operation.
1.1.11
Ensure that the etcd data directory permissions are set to 700 or more restrictive (Automated)
L1
Equivalent Control
...
Equivalent Control
2003:2003
The etcd data directory, /var/lib/etcd, is owned by 2003:2003 as the result of rootless control-plane for enhanced security.
1.1.16
Ensure that the scheduler . conf file ownership is set to root:root (Automated)
L1
Equivalent Control
2002:2002
Anthos clusters on bare metal, starting release 1.9.0, implements rootless control-plane for enhanced security.
1.1.18
Ensure that the controller-manager . conf file ownership is set to root:root (Automated)
L1
Equivalent Control
...
Warn
not set
Some Anthos clusters on bare metal operations, such as HA, require anonymous-auth be enabled.
1.2.3
Ensure that the -- DenyServiceExternalIPs is set (Manual)
L1
Equivalent Control
...
Warn
not set
Anthos clusters on bare metal does not enable Pod Security Policy. Pod Security Admission is enabled in Kubernetes 1.23 clusters by default, see https://kubernetes.io/docs/concepts/security/pod-security-admission/
1.2.13
Ensure that the admission control plugin ServiceAccount is set (Automated)
L1
Warn
...
Equivalent Control
not set
Anthos clusters on bare metal captures audit logs but does not use these flags for auditing, more on logging and monitoring .
4.1.1
Ensure that the kubelet service file permissions are set to 600 or more restrictive (Automated)
L1
Fail
...
Fail
false
Anthos on Bare Metal cluster machines do not protect kernel defaults from Kubernetes, as customer workloads may want to modify these.
4.2.9
Ensure that the --tls-cert-file and --tls-private-key-file arguments are set as appropriate (Manual)
L2
Equivalent Control
not set
Anthos clusters on bare metal manages kubelet server TLS using the --rotate-server-certificates flag.
4.2.10
Ensure that the --rotate-certificates argument is not set to false (Automated)
L1
Equivalent Control
...
Warn
not set
Anthos clusters on bare metal kubelet --pod-max-pids is not set by default, thus no explicit limit on the number of PIDs a pod can use. The pod inherits the host's overall PID limit, which is typically set by the operating system's pid_max kernel parameter. The operating system is user owned.
How to audit Benchmarks
Specific instructions for auditing each Recommendation is available as part of
the relevant CIS Benchmark. However, you may want to automate some of these
checks to simplify the verification of these controls in your environment.
Tools like the one listed in the following section can help with this.
