---
title: "Use custom constraints \_|\_ Live Stream API \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/livestream/docs/custom-constraints
...
numbers, for example custom.inputSRTAllow . This field can contain up to
...
or numbers, for example, custom.inputSRTAllow . This field can contain up to 70
...
livestream.googleapis.com/Input . Most resource types support up to 20 custom
constraints. If you attempt to create more custom constraints, the operation fails.
...
Inputs, in a specific project, to use only the SRT protocol for video ingestion.
Before you begin, you should know the following:
Your organization ID
A project ID
Create the constraint
Save the following file as constraint-restrict-livestream.yaml :
...
This defines a constraint where a request to create an Input resource will be rejected unless the type is SRT_PUSH.
...
Create the policy
Set PROJECT_ID to your project ID and save the following file as policy-restrict-livestream.yaml :
...
Test the policy
Try to create an Input in the project with the following payload:
{
"type": "RTMP_PUSH"
}
The output is similar to the following:
Operation denied by custom org policy: ["customConstraints/custom.inputSRTAllow": "All Inputs must have type=SRT_PUSH."]
Live Stream API supported resources
The following table lists the Live Stream API resources that you can reference
...
resource.video.uri
livestream.googleapis.com/Channel
resource.elementaryStreams.audioStream.bitrateBps
