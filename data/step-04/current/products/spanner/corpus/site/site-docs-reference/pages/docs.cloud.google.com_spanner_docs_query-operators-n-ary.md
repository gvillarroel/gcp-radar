---
title: "N-ary operators \_|\_ Spanner \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/spanner/docs/query-operators-n-ary
...
An N-ary operator has more than two relational children.
The following operators are N-ary operators:
Union all
...
no data.
Union all
A union all operator combines all row sets of its children without removing
duplicates. Union all operators receive their input from union
input operators that are distributed across multiple servers. The
union all operator requires that its inputs have the same schema (the same set of data types for each column).
...
The union all operator combines its input rows, and in this example it sends the
results to a serialize result operator.
The following query succeeds because the same set of data types
is used for each column, even though the children use different variables for
the column names:
SELECT 1 a ,
2 b
UNION ALL
SELECT 3 c ,
4 e ;
The following query fails because the children use
