---
title: "Cloud Storage JSON API overview \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/storage/docs/json_api
...
applications that consume web services through HTTP requests. If this does not
describe you, consider one of the following alternatives:
...
PATCH request .
Note: As with all query parameter values, the fields parameter value must be
URL encoded. For better readability, the examples in this page omit the
encoding.
Syntax summary
...
Example
In a normal JSON API request to retrieve object metadata ,
Cloud Storage returns the full object resource in the response.
...
Object Lifecycle Management configurations and custom metadata ,
...
What's next
Learn more about authenticating to the API .
Learn about request endpoints and URI path encoding .
Read the JSON API reference pages .
Try it for yourself
