---
title: "Upload file data \_|\_ Google Drive \_|\_ Google for Developers"
...
File . For information about how to create a
...
supported MIME types . Note: Users can upload any file type to Drive using the
Drive UI and Drive
...
To perform a simple upload, use the create
method on the files resource with
uploadType=media .
...
HTTP
Create a POST request to the method's /upload URI with the query
...
Send the request. If the request succeeds, the server returns the HTTP
...
create method on the files resource with uploadType=multipart .
Note: If you're using the older Drive API v2, use the
files.insert method. You can find code
samples in GitHub .
Learn how to migrate to Drive API v3 .
...
like "name": "photo.jpg" in the metadata. Subsequent calls to the get method return the read-only fileExtension
...
create method on the files resource with uploadType=resumable .
Note: If you're using the older Drive API v2, use the
files.insert method. You can find code
samples in GitHub .
Learn how to migrate to Drive API v3 .
...
HTTP
To request the upload status, create an empty PUT request to the
resumable session URI.
Add a Content-Range header to indicate that the current position in the
...
If you received a 308 Resume Incomplete response, process the
