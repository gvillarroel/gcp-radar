---
title: "Datasets tagged geophysical in Earth Engine \_|\_ Earth Engine Data Catalog\
  \ \_|\_ Google for Developers"
url: https://developers.google.com/earth-engine/datasets/tags/geophysical
...
Page Summary
outlined_flag
Global SRTM topographic diversity represents the variety of temperature and moisture conditions based on the 30m SRTM DEM.
The Global SRTM mTPI (Multi-Scale Topographic Position Index) distinguishes ridges from valleys using elevation data based on the 30m SRTM DEM.
GloFAS provides global daily flood forecasts up to 30 days and seasonal forecasts up to 4 months ahead at 0.05° resolution.
...
HydroSHEDS provides standard hydrologic derivatives like slope and aspect from the SRTM 30m DEM data globally (excluding Antarctica).
The JRC Global Surface Water Mapping Layers dataset maps the location and temporal distribution of surface water globally from 1984 to 2021 at 30 meter resolution using Landsat images.
MERIT Hydro is a new global hydrography dataset at 3 arc-second resolution, improving upon the hydro-enforcement of the MERIT DEM.
The MERIT DEM is a new global Digital Elevation Model at 3 arc-second resolution developed by removing multiple error components from existing DEMs.
The CSP Ecologically Relevant Geographic Information (ERGo) datasets, Landforms and Physiography, contain detailed, multi-scale data on landforms and physiographic types.
The US NED CHILI (Continuous Heat-Insolation Load Index) is a surrogate for the effects of insolation and topographic shading on evapotranspiration based on the USGS's 10m NED DEM.
The US NED Landforms dataset provides landform classes created by combining CHILI and mTPI datasets based on the USGS's 10m NED DEM.
...
The US NED Topographic Diversity dataset represents the variety of temperature and moisture conditions as local habitats based on the USGS's 10m NED DEM.
The US NED mTPI distinguishes ridge from valley forms based on the USGS's 10m NED DEM.
...
The USGS 3DEP 10m National Map Seamless dataset provides seamless DEM coverage for the U.S. and territories at 1/3 arc-second resolution.
The USGS 3DEP 1m National Map dataset is a tiled collection of 1m resolution DEM images for the U.S. from the 3D Elevation Program.
VIIRS Active Fire detection products from NOAA-20 and Suomi NPP provide active fire data at 375m resolution, offering improved response over small fires.
...
WWF HydroATLAS Basins provide a standardized compendium of hydro-environmental attribute information for global watersheds at high spatial resolution.
