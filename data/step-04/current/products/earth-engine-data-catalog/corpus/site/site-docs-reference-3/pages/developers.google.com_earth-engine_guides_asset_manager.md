---
title: "Manage assets \_|\_ Google Earth Engine \_|\_ Google for Developers"
url: https://developers.google.com/earth-engine/guides/asset_manager
...
Page Summary
outlined_flag
Earth Engine assets are project-owned geospatial data stored within the platform, which can include uploaded data or results from analyses.
Earth Engine supports various asset types including Image, ImageCollection, Table, Classifier, FeatureView, and Folder, each serving different data and organizational purposes.
Assets are organized in a hierarchical folder system rooted in a Cloud project, similar to common file systems, and referenced using unique asset IDs.
Assets can be created by ingesting files from local sources or Cloud Storage, exporting analysis results, or creating empty folders and ImageCollections, with different methods available for Python, Code Editor, and the command line.
Users can manage assets through Python, Code Editor, or command line, including listing, setting and checking permissions, copying, moving, renaming, deleting, viewing metadata, setting metadata, and checking quota.
...
Earth Engine as an ee.ImageCollection object which includes a
...
ee.FeatureCollection object, which includes a set of methods
for filtering and analysis.
Classifier
A trained Earth Engine machine learning model. It is represented by the
ee.Classifier object, which includes a set of
...
asset directory. For example, the root of my-project is
projects/my-project/assets .
All assets that belong to my-project are in the projects/my-project/assets
folder or a sub-folder (or ImageCollection) within it.
Note: Legacy user-owned assets use a different root format:
...
projects/my-project/assets/my-asset specifies an asset named
...
using batch functions Export.*.toAsset ).
Python client
Image
Use the ee.data.startIngestion
...
Table
Use the ee.data.startTableIngestion
...
Code Editor
Within the Asset Manager, click the NEW
...
ADD ACCESS button to confirm the change.
...
Rename
Within the Asset Manager, hold the pointer over an asset and click the
edit icon and type a new name
in the editable input field.
Command line
Use the earthengine mv
...
page.
Python client
Use the ee.data.getAssetRootQuota
...
data_usage icon. An information
dialog will appear.
Command line
