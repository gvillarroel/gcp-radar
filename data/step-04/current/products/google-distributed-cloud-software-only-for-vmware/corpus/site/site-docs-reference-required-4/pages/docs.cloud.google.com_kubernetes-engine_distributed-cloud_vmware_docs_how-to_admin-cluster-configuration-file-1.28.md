---
title: "Admin cluster configuration file 1.28 \_|\_ Google Distributed Cloud (software\
  \ only) for VMware \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/kubernetes-engine/distributed-cloud/vmware/docs/how-to/admin-cluster-configuration-file-1.28
...
Replace VERSION with the full version of a 1.28 patch
release. For example:
...
Required
Immutable
This section holds information about your vSphere environment and your
...
If your resource pool is nested under another resource pool, set this to the
full path to your resource pool.
Example:
vCenter:
...
vCenter:
folder: "my-folder"
If your VM folder is nested under another VM folder, set this to the
full path to your VM folder.
Example:
vCenter:
folder: "/my-datacenter/vm/vm-folder/nested-vm-folder"
Note that you must include the first "/" in your VM folder's full path.
vCenter.dataDisk
...
If you have an existing F5 BIG-IP load balancer and the cluster uses this
configuration, after upgrading to 1.29 or higher, we recommend that
you migrate to manual mode .
...
Immutable
String
The name of a BIG-IP partition
that you created for your admin cluster.
Example:
loadBalancer:
f5BigIP:
...
We still support Seesaw for clusters that are upgraded from 1.16 to 1.28. For
example, suppose you have a 1.16 non-HA admin cluster that uses Seesaw. Then
after you upgrade the cluster to 1.28, you can continue to use the non-HA
control plane and the Seesaw load balancer. For information about the Seesaw
load balancer, see the 1.16 documentation:
...
lifecycle of
user clusters using GKE On-Prem API clients .
...
If you specify ubuntu_cgv2 or cos_cgv2 , then you must run
gkectl prepare --extra-os-image-types=cgroupv2 before you create/update the cluster.
Example:
osImageType: "cos"
Send feedback
