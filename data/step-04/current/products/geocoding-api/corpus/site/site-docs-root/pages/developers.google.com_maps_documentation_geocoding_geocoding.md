---
title: "Geocode an address \_|\_ Geocoding API \_|\_ Google for Developers"
...
Or as a structured set of address components
...
Note: URLs must be
properly encoded
to be valid and are limited to 4096 characters for all web services.
Be aware of this limit when constructing your URLs. Note that different
browsers, proxies, and servers may have different URL character limits as well.
...
Query type
Example
Latitude and longitude coordinates. Use reverse geocoding instead.
...
— Use only with GeocodeAddress for forward geocoding.
...
Note: Generally, only one entry in the results array is returned for address
lookups, though the geocoder might return several results when address queries
are ambiguous.
...
Required parameters
address — The street address or
Plus Code
that you want to geocode. Note: Address geocoding does not resolve latitude and longitude coordinates,
or other unstructured strings that don't represent an address or a Plus
Code. See Pass an unstructured address string
for more details and examples of unsupported queries.
Specify addresses in accordance with the format
...
should be avoided. Street address elements should be delimited by spaces
URL-encoded to %20 . For example, pass the address "24 Sussex Drive Ottawa ON" as:
24 % 20 Sussex% 20 Drive% 20 Ottawa% 20 ON
Format Plus Codes as shown below. Plus signs are URL-encoded to %2B
and spaces are URL-encoded to %20 :
A global code is a 4 character area code and 6 character or longer
local code. For example, encode "849VCWC8+R9" as 849VCWC8%2BR9 .
A compound code is a 6 character or longer local code with an
explicit location. For example, encode "CWC8+R9 Mountain View, CA, USA"
as CWC8%2BR9%20Mountain%20View%20CA%20USA .
Optional parameters
locationBias
...
be overridden if the requested address contains
an explicit location such as Barcelona, Spain .
...
Note: This parameter only adds a bias
towards results within the viewport, and doesn't guarantee that
result(s) are contained by it. The
Geocoding best practices article
provides some guidance regarding which API method best fits which use-case.
...
two-character CLDR code value specifying the region bias. Most CLDR codes
