---
title: "Permissions supported in deny policies \_|\_ Identity and Access Management\
  \ (IAM) \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/iam/docs/deny-permissions-support
...
You can use some, but not all, Identity and Access Management (IAM) permissions in deny
policies .
Deny policies require the IAM v2 permission format, which is
SERVICE_FQDN / RESOURCE . ACTION . The
value of SERVICE_FQDN is typically the value of
SERVICE_ID from the v1 API, followed by
.googleapis.com . For example, the permission to delete a role is
iam.googleapis.com/roles.delete . Exceptions are documented on this page.
Supported permissions
The following table lists the permissions that can be used in deny policies.
Some permissions have aliases. If a permission has an alias, you can use either
the permission name or the alias in deny policies.
...
Note: In the IAM v1 API, this permission is
named serviceusage.apiKeys.regenerate .
apikeys.googleapis.com/apiKeys.revert
Note: In the IAM v1 API, this permission is
named serviceusage.apiKeys.revert .
...
telemetry.googleapis.com/logs.*
telemetry.googleapis.com/logs.write
telemetry.googleapis.com/metrics.*
telemetry.googleapis.com/metrics.write
