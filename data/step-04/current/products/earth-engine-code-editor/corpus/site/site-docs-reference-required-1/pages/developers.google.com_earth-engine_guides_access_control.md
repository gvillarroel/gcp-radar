---
title: "Control access to resources \_|\_ Google Earth Engine \_|\_ Google for Developers"
...
Page Summary
outlined_flag
Earth Engine assets and compute quota can be shared with individuals or groups at the project level.
...
Permissions can be set at the asset level using various tools or at the project level through IAM roles.
VPC Service Controls can enhance security but come with limitations on supported features like the Code Editor and exporting to Google Drive.
...
You can share the assets or compute quota of your Earth Engine enabled
project with other Earth Engine users at the
project level. Earth Engine assets or compute can be shared with another user or
group of users. If you want to share with a group of users, Create a new Google
Group and note its email
(available from the About link on the group page). This page describes how to
provide access to resources, for either an individual or group and the Roles
and Permissions required for different activities.
...
Additionally, the user must have at least serviceusage.services.use permission
...
You can share assets at the project level by assigning the appropriate Identity
and Access Management (IAM) role on
your project's IAM admin page. There are Predefined Earth Engine IAM
Roles for sharing Earth Engine assets and
resources. See Understanding
Roles for a more general
...
Open the IAM Page
Or hold the pointer over your project name on the Assets tab of the Code
Editor and click the
share icon.
Click select a project and choose your project (you should already be
there if you opened the IAM page from the Code Editor).
Click ADD at the top and add the individual or group email as the new
member, or click the edit icon next to the
existing member in the project.
In the Role drop down search for the Earth Engine Resource role you
want to grant. See Predefined Earth Engine IAM
Roles for details.
Click the SAVE button.
Note: the special user identifiers
allUsers and
allAuthenticatedUsers
are not supported project principal
types .
VPC Service Controls
Earth Engine supports VPC Service
Controls , a Google
...
Learn more about VPC-SC features and
configuration .
Limitations
...
Use
other available export destinations like
Google Cloud Storage and
BigQuery .
Save your resources in Earth Engine by exporting
raster or
tabular data to assets.
Earth Engine Apps are not supported for resources and clients inside a
service perimeter.
...
result in an error. To learn more about Earth Engine pricing visit official
documentation .
...
Note that you may set a primitive
or custom role
...
Noncommercial tier selection
The following permissions and suggested roles relate to noncommercial tier configuration .
Tier selection
Permissions needed
earthengine.config.update
Suggested roles
Earth Engine Resource Writer ( roles/earthengine.writer )
Associate a billing account with a project
The Contributor Tier requires a valid billing account associated with the
project. For more information on billing permissions and roles, see the
Cloud Billing Access page .
Permissions needed
Billing account
...
Suggested roles
Billing account
Billing Account Viewer ( roles/billing.viewer ), for creating a new Limited plan
...
role is earth-engine-public-apps@appspot.gserviceaccount.com
...
[[["Easy to understand","easyToUnderstand","thumb-up"],["Solved my problem","solvedMyProblem","thumb-up"],["Other","otherUp","thumb-up"]],[["Missing the information I need","missingTheInformationINeed","thumb-down"],["Too complicated / too many steps","tooComplicatedTooManySteps","thumb-down"],["Out of date","outOfDate","thumb-down"],["Samples / code issue","samplesCodeIssue","thumb-down"],["Other","otherDown","thumb-down"]],["Last updated 2026-02-04 UTC."],[],["Earth Engine resources are shared at project or asset levels, requiring specific permissions like `serviceusage.services.use` and Earth Engine roles (Viewer, Writer, Admin, Apps Publisher). Project-level permissions are set via IAM. VPC Service Controls enhance security but have limitations, requiring workarounds. Managing projects needs permissions like `resourcemanager.projects.get`. Commercial project registration involves billing and cloud project permissions. Plan management requires billing account permissions. App tasks such as app creation and deletion require specific permissions like `iam.serviceAccounts.create` and roles like Service Account Admin.\n"]]
