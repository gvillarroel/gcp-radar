---
title: "Admin settings - SAML authentication \_|\_ Looker \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/looker/docs/admin-panel-authentication-saml
...
Note: Customers who use Looker (Google Cloud core) should be aware of their responsibility for controlling access and permissions when using third-party identity providers. For details, refer to the Our shared security partnership page.
...
Your Looker (original) instance is enabled to use SAML. Contact your account executive to enable SAML. This page is enabled on Looker (Google Cloud core) instances by default.
...
The information to provide is the URL where you typically access your Looker instance using the browser, followed by /samlcallback . For example:
none
...
Getting started
To configure SAML authentication for your Looker instance, select SAML from the Authentication section of the Admin panel. On the SAML Authentication page, select Configure SAML to see the following configuration options.
Note: Changes to configuration options don't take effect until you test and save your settings at the bottom of the page.
SAML auth settings
...
Either paste the public URL of the XML document that contains the IdP information, or paste the document's text in its entirety here. Looker will parse that file to populate the required fields.
...
To pair SAML attributes with corresponding Looker user attributes, follow these steps:
Enter the name of the SAML attribute in the SAML Attribute field and the name of the Looker user attribute that you want to pair it with in the Looker User Attributes field.
Check Required if you want to require a SAML attribute value to allow a user to log in.
Click + and repeat these steps to add more attribute pairs.
To pair SAML user attributes with corresponding Looker user attributes, or manage existing pairings, follow these steps:
In the User Attribute Settings section, click the Manage Pairings button to open the Manage Pairings side panel.
In the Claim field, enter the name of the SAML attribute that you want to pair. Claim names can be entered in the list only once. If you want to associate a claim with multiple Looker attributes, you can enter multiple attributes in the Looker User Attributes field.
In the Looker User Attributes field, a drop-down list of Looker user attributes for the instance appears. Select the name of the Looker user attribute that you want to pair with the SAML attribute. Multiple Looker user attributes can be selected. At least one Looker user attribute must be selected.
Select the Required checkbox if you want to require a SAML attribute value to allow a user to sign in.
...
Verify your SAML configuration, and, at the bottom of the SAML Authentication page, select the I have verified the SAML configuration checkbox.
Click the Enable SAML or Update SAML Settings button to save your SAML user attribute pairings.
...
Groups and roles
You have the option for Looker to create groups that mirror your externally managed SAML groups, and then assign those users Looker roles that are based on their mirrored SAML groups. When you make changes to your SAML group membership, those changes will be applied to users of that Looker group when they next log in to Looker.
...
Some IdPs use a separate attribute for each group and then require a second attribute to determine whether a user is a member of a group. A sample SAML assertion showing this system follows:
none
...
The Manage Groups button: Click this button to open the Manage Groups side panel, where you can add, remove, or edit a custom group or the roles that are assigned to the corresponding group in Looker. The Manage Groups side panel displays the following fields:
...
The SAML Group Name field. Enter the SAML group in this field. For Okta users, enter the Okta group name as the SAML group ID. SAML users who are included in the SAML group will be added to the mirrored group within Looker.
...
If you want to stop mirroring your SAML groups within Looker, turn off the Mirror SAML Groups switch. Turning off the switch results in the following behavior:
Any mirror SAML group with no users is deleted immediately.
Any mirror SAML group that does contain users is marked as orphaned. If no users of this group log in within 31 days, the group is deleted. Users can no longer be added or removed from orphaned SAML groups.
Migration options
...
Note: Alternate login isn't available for Looker (Google Cloud core) . By default, the backup authentication method for Looker (Google Cloud core) is OAuth .
...
Note: If a user has authenticated into the Looker instance using only SAML, you can enable alternate login only by using the Looker API. To learn how to enable alternate login using the Looker API, see the Enabling the alternate login option documentation page.
...
OIDC
If you have more than one system in place, you can specify more than one system to merge by in this field. Looker will look up users from the systems listed in the order that they are specified . For example, assume you created some users using Looker email and password, then you enabled LDAP, and now you want to use SAML. Looker would merge by email and password first and then LDAP.
...
Condition 1 : Users are authenticating into Looker (Google Cloud core) using their Google identities through the SAML protocol.
...
Note: Be sure to verify that all tests are passing before clicking Update Settings . Saving incorrect SAML configuration information could lock yourself and others out of Looker.
...
The user tries to connect to a Looker URL (for example, instance_name.looker.com ).
Looker determines whether the user already has an active session enabled. To do this, Looker uses the cookie AUTH-MECHANISM-COOKIE to identify the authorization method used by the user in their last session. The value is always one of these: saml , ldap , oidc , google , or email .
If the user does have an active session enabled, the user is taken to the requested URL.
If the user doesn't have an active session enabled, they are redirected to the IdP. The IdP authenticates the user when they successfully sign in to the IdP. Looker then authenticates the user when the IdP sends the user back to Looker with information indicating that the user is authenticated with the IdP.
...
If the user is unable to sign in to the IdP, or if they aren't authorized by the IdP to use Looker, then depending on the IdP, they will either remain on the IdP's site, or be redirected to the Looker Log In page.
Troubleshooting
