---
title: "Looker (Google Cloud core) access control with IAM \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/looker/docs/looker-core-access-control
...
roles .
Note: IAM basic
roles might also contain permissions to assign Looker (Google Cloud core) IAM roles. You shouldn't grant basic roles in a
...
Note: To learn more about roles, see the Basic and predefined roles reference .
...
Looker Viewer
(roles/looker.viewer)
Read-only access to all Looker (Google Cloud core) resources in the Google Cloud console.
looker.backups.get
looker.backups.list
...
Access to sign in to a Looker (Google Cloud core) instance.
looker.instances.get
looker.instances.login
resourcemanager.projects.get
resourcemanager.projects.list
...
Access to Looker (Google Cloud core) schema data in Dataplex Universal Catalog.
looker.schemas.view
Looker Admin
roles/looker.admin
Full access to all Looker (Google Cloud core) resources.
...
Warning: The Looker Service Agent IAM role is intended only for the Looker service account and is automatically assigned to the service account when the Looker API is enabled . Don't grant service agent roles to any principals except service agents. Instead, choose a different predefined role , or create a custom role with the permissions that you need.
What's next
