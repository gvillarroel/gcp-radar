---
title: "Create an AI-optimized instance with A4X \_|\_ AI Hypercomputer \_|\_ Google\
  \ Cloud Documentation"
url: https://docs.cloud.google.com/ai-hypercomputer/docs/create/create-vm-a4x
...
Create an AI-optimized instance with A4X | AI Hypercomputer | Google Cloud Documentation
...
instances that use A4X machine types. To learn more about this machine type, see
A4X .
To learn about other ways to create VMs or clusters, see
Deployment options overview .
Limitations
When you create a standalone A4X VM, the following limitations apply:
Caution: The
...
method that is designed to create multiple instances with a single API request. To create A4X
...
If a sub-block lacks capacity for an A4X instance, then the request to create the
instance fails.
...
gcloud REST
More
To create the A4X instance, use the
...
Replace the following:
INSTANCE_NAME : the name of the A4X instance.
...
( STOP ) or deletes ( DELETE ) the A4X instance at the end of the reservation period.
POLICY_NAME : the name of the compact placement policy.
To create the A4X instance, make a POST request to the
