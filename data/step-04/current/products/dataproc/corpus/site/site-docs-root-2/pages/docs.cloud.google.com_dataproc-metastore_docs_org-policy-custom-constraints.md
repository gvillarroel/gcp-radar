---
title: "Create custom organization policy constraints \_|\_ Dataproc Metastore \_\
  |\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/dataproc-metastore/docs/org-policy-custom-constraints
...
Dataproc Metastore resources. For example, if a request to create
or update a metastore Service resource fails to satisfy custom constraint
validation as set by your organization policy, the request fails and an error
will be returned to the caller.
Limitations
Custom constraints and organization policies aren't supported on the following
resources:
...
numbers, for example custom.allowDataprocMetastoreService . This field can contain up to
...
or numbers, for example, custom.allowDataprocMetastoreService . This field can contain up to 70
...
metastore.googleapis.com/Service . Most resource types support up to 20 custom
constraints. If you attempt to create more custom constraints, the operation fails.
...
example, custom.allowDataprocMetastoreService .
...
Description
Constraint syntax
Restrict logging for Dataproc Metastore instances
name : organizations/ ORGANIZATION_ID /customConstraints/custom.allowDataprocMetastoreService
resourceTypes :
- metastore.googleapis.com/Service
methodTypes :
- CREATE
...
actionType : ALLOW
displayName : Allow only the creation of Enterprise DPMS instances
description : All DPMS instances should use the enterprise tier
Dataproc Metastore supported resources
The following table lists the Dataproc Metastore resources that you can reference
