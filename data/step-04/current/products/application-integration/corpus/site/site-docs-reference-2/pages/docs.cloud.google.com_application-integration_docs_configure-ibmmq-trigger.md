---
title: "IBM MQ trigger \_|\_ Application Integration \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/application-integration/docs/configure-ibmmq-trigger
...
IBM MQ trigger
The IBM MQ trigger is a Connector Event trigger that lets you invoke an integration based on the IBM MQ events that you've subscribed to in your IBM MQ connection.
...
Before you begin
If you plan to create or configure a new connection for the IBM MQ trigger , make sure that you have the following IAM role on the project:
Connector Admin ( roles/connectors.admin )
For information about granting roles, see Manage access .
Grant the following IAM roles to the service account that you want to use for the IBM MQ trigger :
...
Note: The IBM MQ trigger supports JMS (Java Message Service) protocol. Other protocols, such as MQTT and AMQP, are not supported.
Add the IBM MQ trigger
To add a IBM MQ trigger to your integration, perform the following steps:
...
Click and place the IBM MQ trigger element in the integration editor.
To configure the IBM MQ trigger , you can either use an existing IBM MQ connection available in Integration Connectors, or create a new IBM MQ connection using the inline connection creation option.
Configure IBM MQ trigger using an existing connection
You can configure the IBM MQ trigger using an existing IBM MQ connection in Integration Connectors. Note that the IBM MQ trigger is an Connector Event trigger , therefore you can only use a IBM MQ connection with event subscription enabled to configure the trigger.
For information about how to configure a IBM MQ trigger using a new IBM MQ connection, see Configure IBM MQ trigger using a new connection
To configure a IBM MQ trigger using an existing IBM MQ connection, perform the following steps:
Click the IBM MQ trigger element in the integration editor to open the trigger configuration pane.
Click Configure trigger .
Provide the following configuration details in the Connector Event Trigger Editor page:
Region: Select the region of your IBM MQ connection.
Connection: Select the IBM MQ connection that you want to use.
Application Integration only displays those IBM MQ connections that are active and have an event subscription enabled.
Select Queue or Topic as the JMS Source Type .
If you select Queue , provide the IBM MQ queue name in the JMS Source Name column.
If you select Topic , provide the IBM MQ topic name in the JMS Source Name column.
Service Account : Select a service account with the required IAM roles for the IBM MQ trigger.
Click Done to complete the trigger configuration and close the page.
Configure IBM MQ trigger using a new connection
To configure the IBM MQ trigger using a new IBM MQ connection, perform the following steps:
Click the IBM MQ trigger element in the integration editor to open the trigger configuration pane.
Click Configure trigger .
...
In the Location step, choose the location for the new IBM MQ connection:
Region: Select a region from the drop-down list.
Click Next .
In the Connection Details step, provide details about the new IBM MQ connection:
Connector version: Choose an available version of the IBM MQ connector from the drop-down list.
Connection Name: Enter a name for the IBM MQ connection.
...
Service Account : Select a service account with the required IAM roles for the IBM MQ connection.
Enable event subscription: Select this option to enable the event subscription for the IBM MQ connection.
...
For more information about the connection node settings for IBM MQ, see the IBM MQ connection documentation.
...
In the Destinations step, provide the IBM MQ instance details:
Destination Type: Select Host address .
Host: Enter the hostname or IP address of your IBM MQ instance.
Click Next .
In the Authentication step, provide the authentication details for the IBM MQ instance:
Select your desired authentication type and enter the relevant details to access the provided IBM MQ host URL.
To understand how to configure these authentication types, see Configure authentication .
Click Next .
Review : Review the provided IBM MQ connection details.
Click Create to complete creating a new IBM MQ connection.
...
Trigger output
The IBM MQ trigger takes a couple of minutes to complete the event subscription configuration. You can view the status
of your event subscription in the trigger configuration pane under Event Subscription details .
The IBM MQ trigger indicates the status of an event subscription using the following states:
restart_alt
...
The following image shows a sample layout of the IBM MQ trigger configuration pane:
Trigger output variable
For each event, the IBM MQ trigger generates a ConnectorEventPayload
output variable (in JSON format) which you can use in your downstream tasks. The output
variable will have the payload from your IBM MQ event. For example:
{
"message" : "Hello world" ,
...
Edit IBM MQ trigger
You can edit a IBM MQ trigger to change or update the connection configuration and event subscription details.
Important: When you edit or modify a IBM MQ trigger, you can either retain the previously configured event subscription that is attached to the trigger, or you can delete it. Deleting an event subscription will affect all the published integrations using that event subscription.
To edit a IBM MQ trigger, perform the following steps:
Click the IBM MQ trigger element in the integration editor to open the trigger configuration pane.
Click Configure IBM MQ trigger .
