---
title: "Introduction to BigQuery sharing \_|\_ Google Cloud Documentation"
...
or within your own organization in real time. Listings let you
...
built-in features of BigQuery .
When you subscribe to a listing, a linked dataset or
...
As an Analytics Hub Admin, you can create data exchanges
...
Table snapshots
Views
Shared datasets support column-level security
...
streaming data sharing in BigQuery .
...
public data exchanges can be
discovered
and
subscribed to
by
Google Cloud users ( allAuthenticatedUsers ) .
For more information about public data exchanges, see
...
Public listing. A public listing is shared with all
Google Cloud users ( allAuthenticatedUsers ) .
Listings in a public data exchange are public listings. These listings can be
references of a free public resource or a commercial resource . If the
listing is of a commercial resource, subscribers can either request access to the
listing directly from the data provider, or they can browse and purchase
...
groups. For example, a private listing can reference a marketing metrics dataset
that you share with other internal teams within your organization.
Subscriber workflow
...
and
writing to a destination table
are unavailable.
CREATE VIEW AS SELECT statements
...
A shared topic can have a maximum of 10,000
...
view metadata for your linked dataset .
...
Usage metrics for views are populated only for queries after April 22, 2024.
