---
title: "Work with pipe query syntax \_|\_ BigQuery \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/bigquery/docs/pipe-syntax-guide
...
syntax , or standard syntax —for instance, selection,
...
Standard syntax
Pipe syntax
Clauses must appear in a particular order.
Pipe operators can be applied in any order.
More complex queries, such as queries with multi-level aggregation,
usually require CTEs or nested subqueries.
More complex queries are usually expressed by adding pipe operators
...
For full syntax details, see the Pipe query syntax reference
documentation.
Basic syntax
...
Each pipe operator in pipe syntax consists of the pipe symbol, |> , an
operator name, and any arguments:
...
table-valued functions, and other contexts.
...
A pipe operator can see every alias that exists in the table preceding the
pipe.
A query can start with a FROM clause , and pipe operators
can optionally be added after the FROM clause.
...
operator , followed by any number of aggregate
...
You can use the JOIN pipe operator to join the results
...
The SELECT pipe operator doesn't perform aggregation. You must use the
AGGREGATE pipe operator instead.
Filtering is always done with the WHERE pipe
operator , which can be applied anywhere. The WHERE
pipe operator, which replaces HAVING and
QUALIFY , can filter the results of aggregation
or window functions.
For more details, see the complete list of pipe operators .
Use cases
...
query up to a pipe symbol |> is a valid query, which helps you view
intermediate results in a long query. The productivity gains can
speed up the development process across your organization.
Log analytics : There exist other types of pipe-like syntax that are
...
complete list of pipe operators .
EXTEND pipe operator
The EXTEND pipe operator lets you append computed
...
+---------+---------------+-----------------*/
SET pipe operator
The SET pipe operator lets you replace the value of
...
+---+---*/
DROP pipe operator
The DROP pipe operator lets you remove columns from the
...
The RENAME pipe operator lets you rename columns from
...
with a GROUP AND ORDER BY clause , which orders
...
+-------+-------------*/
JOIN pipe operator
The JOIN pipe operator lets you join the current table
with another table and supports the standard join operations ,
...
operator . You can optionally alias the joined table. You can
...
Find the average amount that repeat customers spend for each state and
item type.
Limitations
You can't include a differential privacy clause in a SELECT
statement following a pipe operator. Instead, use a differential privacy
clause in standard syntax and apply pipe operators following the query.
What's next
Analyze data using pipe syntax
Pipe query syntax reference
Standard query syntax reference
VLDB 2024 conference paper on pipe syntax
Send feedback
