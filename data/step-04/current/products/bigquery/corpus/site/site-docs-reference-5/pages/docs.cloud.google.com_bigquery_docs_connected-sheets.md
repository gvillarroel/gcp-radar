---
title: "Using Connected Sheets \_|\_ BigQuery \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/bigquery/docs/connected-sheets
...
You can use VPC Service Controls to restrict access to
...
the following:
An access level or ingress rule to allow requests from trusted IP addresses,
identities, and trusted client devices from outside of the perimeter.
An egress rule to allow query results to be copied to users' spreadsheets.
Learn about configuring ingress and egress
policies and
configuring access levels to
properly configure the rules .
...
cause scheduled refreshes to fail.
Before you begin
...
resources that you created. See Cleaning up for more detail.
...
Click Data , click Data connectors , and then click Connect to
BigQuery .
Note: If you don't see the Data connectors option, see
Before you begin .
...
conditions:
If the number of rows is less than or equal to 50,000, then there's no cell
limit.
If the number of rows is greater than 50,000 but less than or equal to
500,000, then the number of cells must be less than or equal to 5 million.
If the number of rows is greater than 500,000, then the data pull isn't
supported.
...
refresh Refresh .
Optional: To refresh the query results in Connected Sheets, click
refresh Refresh .
To view the query in BigQuery,
click info_outline
...
Get started with BigQuery data in Google Sheets topic.
