---
title: "Forecast a single time series with an ARIMA_PLUS univariate model \_|\_ BigQuery\
...
Creating a time series model to forecast site traffic by using the
...
Retrieving the forecasted site traffic information from the model by using the
