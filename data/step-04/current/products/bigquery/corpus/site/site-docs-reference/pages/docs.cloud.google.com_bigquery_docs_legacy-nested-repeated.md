---
title: "Querying nested and repeated fields in legacy SQL \_|\_ BigQuery \_|\_ Google\
  \ Cloud Documentation"
url: https://docs.cloud.google.com/bigquery/docs/legacy-nested-repeated
...
BigQuery supports loading
and exporting nested and repeated data in the
...
WITHIN RECORD : Aggregates data in the repeated values within
the record.
WITHIN node_name : Aggregates data in the repeated values
within the specified node, where a node is a parent node of the field in the
...
Performs a WITHIN RECORD on citiesLived.place and counts the number of
places each person has lived in
Performs a WITHIN on citiesLived.yearsLived and counts the number of
times each person has lived in each city (counting just across citiesLived ).
