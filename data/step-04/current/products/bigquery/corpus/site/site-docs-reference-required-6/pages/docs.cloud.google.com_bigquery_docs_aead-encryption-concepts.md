---
title: "AEAD encryption concepts \_|\_ BigQuery \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/bigquery/docs/aead-encryption-concepts
...
The key type determines the encryption mode to use with
that key.
...
Note: If you attempt to pass keysets in plaintext as part of queries, the query
text may be logged, and with them the plaintext
keyset. You can use BigQuery's
parameterized queries
to avoid logging the plaintext keyset.
Wrapped keysets
...
raw keysets.
As with keysets , wrapped keysets can, and should, be periodically
rotated. Wrapped keysets are used in
AEAD envelope encryption functions .
Here are some functions with wrapped keyset examples:
KEYS.NEW_WRAPPED_KEYSET : Create a new wrapped keyset.
KEYS.ROTATE_WRAPPED_KEYSET : Rotate a wrapped keyset.
KEYS.REWRAP_KEYSET : Rewrap a wrapped keyset with new
data.
KEYS.KEYSET_CHAIN : Get a Tink keyset that is
encrypted with a Cloud KMS key .
...
CBC mode isn't an
AEAD scheme in the cryptographic sense
...
key.
Cloud KMS keys
GoogleSQL supports AEAD encryption functions
with Cloud KMS keys to further secure your data. This
...
At query execution time, use the KEYS.KEYSET_CHAIN
