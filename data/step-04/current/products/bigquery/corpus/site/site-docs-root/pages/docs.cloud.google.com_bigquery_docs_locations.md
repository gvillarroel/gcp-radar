---
title: "BigQuery locations \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/bigquery/docs/locations
...
for locations, see BigQuery pricing . To learn
how to set the location for your dataset, see Create datasets . For
...
EU
Data centers in the United States 2
US
...
and custom models deployed to Vertex AI. The column name indicates the type of
remote model.
Region description
Region name
...
or other supported SQL dialects. These include the interactive SQL translator ,
the SQL translation API , and the batch SQL translator .
...
The BigQuery partitioning and clustering recommender
...
in the first statement of your query.
When you use the bq command-line tool, supply the --location
global flag and set
...
following ways:
Query Cloud Storage data using
...
otherwise the query incurs data transfer charges .
For example:
...
Multi-region bucket : Using multi-region
...
For more information about load data transfer charges, see the Query Cloud Storage data
...
Query Bigtable data
When you query data in Bigtable
...
(Oregon) region, which is within the United States.
...
the same region. For example, if your BigQuery dataset is in the asia-northeast1
(Tokyo) region, your Bigtable cluster must also be in the
asia-northeast1 (Tokyo) region.
Google Drive
Location considerations do not apply to Google Drive
external data sources.
Cloud SQL
When you query data in Cloud SQL
through a BigQuery federated query ,
...
Cloud SQL locations .
Spanner
When you query data in Spanner
through a BigQuery federated query ,
...
instance must be in the Belgium region.
Multi-region: If your BigQuery dataset is in the US
...
analysis tools :
Managed Service for Apache Spark : When you query
BigQuery datasets using a
BigQuery connector , your BigQuery dataset
should be colocated with your Managed Service for Apache Spark cluster.
Managed Service for Apache Spark is supported in all
Compute Engine locations .
Vertex AI Workbench : When you query
BigQuery datasets using
Jupyter notebooks in Vertex AI Workbench, your BigQuery dataset
...
a Cloud Storage bucket, develop a plan for
geographically managing your data .
Restrict locations
