---
title: "Functions (alphabetical) \_|\_ BigQuery \_|\_ Google Cloud Documentation"
...
Produces an object table with the original columns plus
...
SEARCH
Checks to see whether a table or other
search data contains a set of search terms.
SEC
...
TEXT_ANALYZE
Extracts terms (tokens) from text and converts them into a
tokenized document.
TF_IDF
Evaluates how relevant a term (token) is to a tokenized document in a set
...
VECTOR_SEARCH
Performs a vector search on embeddings to find semantically similar
entities.
