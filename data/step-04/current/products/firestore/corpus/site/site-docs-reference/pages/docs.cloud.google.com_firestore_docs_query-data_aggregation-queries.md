---
title: "Summarize data with aggregation queries \_|\_ Firestore in Native mode \_\
  |\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/firestore/docs/query-data/aggregation-queries
...
sum()
average()
Firestore calculates the aggregation and transmits only the result
...
Firestore Security Rules work the same on aggregation queries as on
...
how Firestore Security Rules interact with queries .
...
response. Queries are served only by the Firestore backend, skipping
the local cache and any buffered updates. This behavior is identical to
operations that are performed inside Firestore
transactions .
...
possible workaround is to use counters
for large datasets.
...
What's next
Learn how to query and filter data .
Save reads when you want to only count results
Send feedback
