...
Best practices for managing service account keys | Identity and Access Management (IAM) | Google Cloud Documentation
...
SDK, languages, frameworks, and tools
On this page
Protecting against credential leakage Provide alternatives to creating service account keys
...
Protecting against privilege escalation Avoid storing keys on a file system
...
Protecting against information disclosure threats Avoid disclosing confidential information in uploaded X.509 certificates
Protecting against non-repudiation threats Use a dedicated service account for each application
Use a dedicated key for each machine that runs an application
Protecting against malicious credential configurations Validate keys acquired from an external source before using them to authenticate to Google APIs
What's next
Home
Documentation
Security
