---
title: "About apps and data stores \_|\_ Vertex AI Search \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/generative-ai-app-builder/docs/create-datastore-ingest
...
Custom search apps have a many-to-many relationship with data stores. When
multiple data stores are connected to a single custom search app, this is
referred to as blended search . For
information about limitations of connecting a search app to more than one
data store, see About blended search .
A custom recommendations app has a one-to-one connection with its data
store.
...
healthcare data, data is imported into an entity , which is a type of data
...
or you can let Vertex AI Search derive the schema from the ingested
data.
Structured data for media . A document is a row in a table or a JSON
...
Unstructured data . A document is a file in any of the following formats:
TXT, PDF, HTML, DOCX, PPTX, XLSX, and XLSM.
Healthcare FHIR data . A document is a supported FHIR R4
...
In Vertex AI Search, there are various kinds of data stores.
...
You can also provide URL patterns for portions of websites that you want
excluded, for example, example.com/events/members-only/* or
example.com/events/past-* . Excluded URLs take priority over included ones.
...
Index and refresh web pages using sitemaps . This
...
information, see Refresh web pages .
...
form of meta tags, PageMap attributes, and schema.org data to your
...
recommendations.
Vertex AI Search auto-detects the schema from the data that you
...
more information about the schema, see About media documents and data
stores .
...
for media apps. For information about user events, see Record real-time user
events .
What's next
...
and images.
Unstructured data stores support documents in TXT, PDF, HTML, DOCX, PPTX, XLSX, and XLSM
formats.
...
supports, see Healthcare FHIR R4 data schema reference .
...
how to prepare healthcare FHIR data for ingestion.
What's next
...
to a single custom search app. This feature lets you use one app to search
across multiple sources and types of data.
To make a blended search app, select multiple data stores when creating a new
custom search app. If you don't select multiple data stores during creation,
