---
title: "Develop a LangChain agent \_|\_ Vertex AI Agent Builder \_|\_ Google Cloud\
  \ Documentation"
url: https://docs.cloud.google.com/agent-builder/agent-engine/develop/langchain
...
The LangchainAgent template uses ChatVertexAI by default, because it provides access to all
...
It is used in LangchainAgent when you omit the model_builder
...
Finally, use it in LangchainAgent with the following code:
from vertexai import agent_engines
agent = agent_engines . LangchainAgent (
...
To use the tool inside LangchainAgent , you will add it to the
...
Next, use the tool inside LangchainAgent :
from vertexai import agent_engines
agent = agent_engines . LangchainAgent (
model = model ,
...
Finally, use the tool inside LangchainAgent :
from vertexai import agent_engines
agent = agent_engines . LangchainAgent (
model = model ,
...
which provide input and output schemas for orchestration. The LangchainAgent class requires a runnable to be built for it to respond to queries. By default,
LangchainAgent will build such a runnable by binding the model with tools
...
have to override the default runnable when creating LangchainAgent by
...
be used (see Define and use a tool ),
