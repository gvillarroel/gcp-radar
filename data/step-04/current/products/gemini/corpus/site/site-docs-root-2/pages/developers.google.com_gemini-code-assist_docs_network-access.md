...
url: https://developers.google.com/gemini-code-assist/docs/network-access
...
Page Summary
outlined_flag
This document explains how network administrators can restrict Gemini Code Assist access to specific user domains using a custom HTTP header and a proxy.
The restriction is enforced by injecting the X-GeminiCodeAssist-Allowed-Domains header with a list of permitted domains into requests via a Person-in-the-Middle (PITM) proxy.
Instructions are provided for configuring a proxy within both VS Code and IntelliJ IDEs, as well as for setting up the PITM proxy to intercept and modify requests.
Proper SSL/TLS interception and header validation are crucial for the domain restrictions to be effectively applied.
Users attempting to access Gemini Code Assist from a disallowed domain will receive a restriction message.
