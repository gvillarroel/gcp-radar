---
title: "Cloud Asset Inventory release notes \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/asset-inventory/docs/release-notes
...
Resources
Send feedback
Cloud Asset Inventory release notes
Stay organized with collections
Save and categorize content based on your preferences.
This page documents production updates to Cloud Asset Inventory. You can check this
...
January 24, 2025
Feature
The following resource types are now publicly available through the ExportAssets , ListAssets , BatchGetAssetsHistory , QueryAssets , Feed , Search ( SearchAllResources , SearchAllIamPolicies ), and analyze policy ( AnalyzeIamPolicy and AnalyzeIamPolicyLongrunning ) APIs.
Cloud Storage
...
July 27, 2023
Feature
The following resource types are now publicly available through the ExportAssets , ListAssets , BatchGetAssetsHistory , QueryAssets , Feed API .
...
osconfig.googleapis.com/PatchDeployment
OS inventory
compute.googleapis.com/Instance (OS inventory is attached on VM instance)
Artifact Registry
artifactregistry.googleapis.com/DockerImage
artifactregistry.googleapis.com/Repository
Cloud Logging
...
July 01, 2021
Feature
Full resource metadata is available in SearchAllResources . A new request parameter read_mask is available to allow you to select which fields to be returned in the search result. Please visit our user guide for more information.
Feature
Attached asset is available in SearchAllResources . Attached assets are the assets attached to a primary resource, for example the OS Inventory contains runtime information of Compute Instance. It allows customers to search and filter by the attributes of the primary resource (e.g., Compute Instance) and the attached assets (e.g., OS Inventory) in a single query.
Feature
New resource types now available
The following resource types are now publicly available through the resource search API ( SearchAllResources ) and policy search API ( SearchAllIamPolicies ):
+ Secret Manager
...
March 22, 2021
Feature
The following searchable field is now publicly available through the resource search API ( SearchAllResources ):
...
Compute Engine (Note that Compute Engine types cover
...
November 20, 2020
Feature
Cloud Composer resource type now available
The following Cloud Composer resource type is now publicly available through the Cloud Asset APIs.
composer.googleapis.com/Environment
October 30, 2020
Feature
Policy Analyzer API GA Release
You can now use the Policy Analyzer API to analyze IAM policies in your project, folder, or organization, and then write the results to a BigQuery table . Note that the Policy Analyzer in console remains in Preview release.
October 22, 2020
Feature
Secret Manager resource types now available
The following Secret Manager resource types are now publicly available through the Cloud Asset APIs.
secretmanager.googleapis.com/Secret
secretmanager.googleapis.com/SecretVersion
September 09, 2020
Feature
Compute resource types now available
The following Compute resource types are now publicly available through the Cloud Asset APIs.
compute.googleapis.com/NodeGroup
compute.googleapis.com/NodeTemplate
compute.googleapis.com/ResourcePolicy
August 31, 2020
Feature
List Assets API Beta Release
You can now list assets within an organization, project, or folder and receive the results as a List API response with pagination support.
August 27, 2020
Feature
Cloud Functions resource type now available
The following Cloud Functions resource type is now publicly available through the Cloud Asset APIs.
cloudfunctions.googleapis.com/CloudFunction
August 26, 2020
Feature
Cloud Dataflow resource type now available
The following Cloud Dataflow resource type is now publicly available through the Cloud Asset APIs.
dataflow.googleapis.com/Job
Feature
Game Servers resource types now available
The following Game Servers resource types are now publicly available through the Cloud Asset APIs.
gameservices.googleapis.com/GameServerCluster
gameservices.googleapis.com/Realm
gameservices.googleapis.com/GameServerConfig
gameservices.googleapis.com/GameServerDeployment
August 14, 2020
Feature
Pub/Sub resource type now available
The following Pub/Sub resource type is now publicly available through the Cloud Asset APIs.
pubsub.googleapis.com/Snapshot
August 11, 2020
Feature
Compute resource types now available
...
August 10, 2020
Feature
Managed Service for Microsoft Active Directory resource type now available
The following Managed Microsoft AD resource type is now publicly available through the Cloud Asset APIs.
...
July 27, 2020
Feature
Conditional real-time notifications
You can now configure your feeds so that you only receive notifications which match your customized conditions .
July 24, 2020
Feature
Resource and policy search GA release
You can now search resource metadata and IAM policies in your project, folder or organization.
July 21, 2020
Feature
Cloud Run resource types now available
The following Cloud Run resource types are now publicly available through the Cloud Asset APIs.
run.googleapis.com/DomainMapping
run.googleapis.com/Revision
run.googleapis.com/Service
Feature
Network Intelligence resource type now available
The following Network Intelligence resource type is now publicly available through the Cloud Asset APIs.
networkmanagement.googleapis.com/ConnectivityTest
June 25, 2020
Feature
Cloud Spanner resource type now available
The following Cloud Spanner resource type is now publicly available through the Cloud Asset APIs.
spanner.googleapis.com/Backup
Feature
Cloud Key Management Service resource type now available
The following Cloud KMS resource type is now publicly available through the Cloud Asset APIs.
cloudkms.googleapis.com/ImportJob
June 17, 2020
Feature
Cloud OS Config resource type now available
The following Cloud OS Config resource type is now publicly available through the Cloud Asset APIs.
osconfig.googleapis.com/PatchDeployment
April 15, 2020
Feature
Policy Analyzer Beta Release
You can now analyze IAM policies in your folder or organization, and then write the results to Cloud Storage .
April 03, 2020
Feature
...
Cloud Asset Inventory is a storage service that keeps a five week history of Google Cloud asset metadata. It allows you to export all asset metadata at a certain timestamp or timeframe.
Send feedback
