---
title: "Connect to GitLab \_|\_ Developer Connect \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/developer-connect/docs/connect-gitlab
...
This page describes how to create connections to GitLab and create links to
GitLab repositories.
...
security managers who want to use GitLab source code repositories with Google.
Specifically, you can use GitLab repositories with
Gemini Code Assist .
...
Secret Manager API .
Ensure that you have access to an account on GitLab.
...
a personal account.
Ensure that you own a GitLab repository, or have admin-level permissions on
a shared repository.
...
and GitLab. If you're using the Google Cloud console, then you can also start
adding links to repositories as you finish setting up your connection.
To create a new GitLab connection, select one of the following options:
Console
...
management provider. Click Connect on the GitLab card.
...
creates.
Optional: By default, Developer Connect can act as a proxy for Git calls to GitLab.
...
following command, where WEBHOOK_SECRET_NAME
...
PROJECT_NAME : your Google Cloud
project ID .
...
Developer Connect to act as a proxy for Git calls to GitLab.
This capability is in Preview .
Developer Connect completes the connection to GitLab. Next,
link to repositories .
Link to repositories using an existing connection
Once you have established a connection to GitLab, you can link to
repositories. You can repeat these steps later to link additional repositories
as needed.
To create repository links on an existing GitLab connection, select one of the
following options:
Console
