---
title: "Connect to Bitbucket Data Center \_|\_ Developer Connect \_|\_ Google Cloud\
  \ Documentation"
url: https://docs.cloud.google.com/developer-connect/docs/connect-bitbucket-dc
...
This page describes how to create connections to Bitbucket Data Center and create links to
...
security managers who want to use Bitbucket Data Center source code repositories with Google.
Specifically, you can use Bitbucket Data Center repositories with
Gemini Code Assist .
...
a personal account.
Ensure that you own a Bitbucket Data Center repository, or have admin-level permissions on
a shared repository.
...
Sign in to Bitbucket Data Center .
Follow the instructions in the Bitbucket Data Center documentation to
create two HTTP access tokens and grant the following
scopes.
Developer Connect supports user, project, and repository access tokens.
Create one token with the repository admin scope. In
Developer Connect, this token is called the admin access token .
Create one token with the repository read scope. In
Developer Connect, this token is called the read access token .
You are responsible for managing Bitbucket Data Center token lifetimes in Bitbucket Data Center. To help
protect your source code, you can set an expiry date during token creation.
If you don't set an expiry date, then the token remains valid until you
revoke it in Bitbucket Data Center.
...
and Bitbucket Data Center. If you're using the Google Cloud console, then you can also start
...
management provider. Click Connect on the Bitbucket Data Center card.
...
For Host URL , enter the URL for the Bitbucket Data Center host you want to connect
to.
...
Optional: By default, Developer Connect can act as a proxy for Git calls to Bitbucket Data Center.
...
creates.
Click the Continue button. Developer Connect creates your
connection. This process can take a few seconds.
...
following command, where WEBHOOK_SECRET_NAME is a name
for your webhook secret:
cat /proc/sys/kernel/random/uuid | tr -d '\n' | gcloud secrets create WEBHOOK_SECRET_NAME
Store your access tokens in Secret Manager by running the
following commands:
...
ADMIN_SECRET_NAME : a name for the secret that
stores your admin access token.
...
Run the
gcloud developer-connect connections create command to create a connection to Bitbucket Data Center:
...
project ID .
ADMIN_SECRET_NAME : the name of the
Secret Manager secret that contains your admin
access token.
READ_SECRET_NAME : the name of the
Secret Manager secret that contains your read access
token.
...
Developer Connect to act as a proxy for Git calls to Bitbucket Data Center.
This capability is in Preview .
Developer Connect completes the connection to Bitbucket Data Center. Next,
link to repositories .
Link to repositories using an existing connection
Once you have established a connection to Bitbucket Data Center, you can link to
repositories. You can repeat these steps later to link additional repositories
as needed.
To create repository links on an existing Bitbucket Data Center connection, select one of the
following options:
Console
...
Developer Connect and your Bitbucket Data Center repository.
REPO_URI : the URL of your repository in Bitbucket Data Center,
including the https:// prefix and the .git suffix.
CONNECTION_NAME : the name of your connection to Bitbucket Data Center.
