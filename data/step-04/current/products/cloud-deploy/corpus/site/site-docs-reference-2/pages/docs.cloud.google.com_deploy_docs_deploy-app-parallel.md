---
title: "Quickstart: Deploy an app to multiple targets at the same time \_|\_ Google\
  \ Cloud Documentation"
url: https://docs.cloud.google.com/deploy/docs/deploy-app-parallel
...
to two targets at the same time—a
parallel deployment.
In this quickstart, you'll do the following:
Create two GKE clusters or two Cloud Run
services.
You can deploy in parallel to GKE attached clusters too, but this
...
targets.
This pipeline will have only one target, but that target will be a
multi-target —a target that represents more than one deployment target.
This multi-target will comprise two actual targets, delivering your app to
the two clusters or services.
...
which is used at deploy time to create your Cloud Run
service. The container image to deploy is set here as a placeholder,
...
GKE
In the deploy-gke-parallel-quickstart directory, create a new file:
...
Cloud Run
In the deploy-run-parallel-quickstart directory, create a new file:
clouddeploy.yaml , with the following contents:
apiVersion: deploy.cloud.google.com/v1
kind: DeliveryPipeline
metadata:
...
Note: In this file, targets are included with the delivery pipeline,
but you can define targets in a separate file or multiple separate files.
Notice that this file includes three targets: a multi-target and two
child targets. You can also configure targets in separate file, rather than
with your delivery pipeline.
Notice also that the delivery pipeline includes deployParameters , with
labels, and the child targets include labels to match those parameters. This
allows you to pass separate values to separate child targets, if you want.
Learn more .
...
file.
Notice that the only target listed is the multi-target qsprod-multi . Child
...
with the specific, SHA-qualified image. Google recommends that you templatize your
...
first target in your pipeline (or, as in this case, a specific target specified
using --to-target= ). In this quickstart, this target is a multi-target, so the
