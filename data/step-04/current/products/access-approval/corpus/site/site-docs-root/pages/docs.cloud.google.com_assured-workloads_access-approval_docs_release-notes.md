---
title: "Release notes \_|\_ Access Approval \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/assured-workloads/access-approval/docs/release-notes
knowledge_key: corpus
source_id: site-docs-root
source_type: site
entrypoint: https://docs.cloud.google.com/assured-workloads/access-approval/docs
source_metadata:
  url: https://docs.cloud.google.com/assured-workloads/access-approval/docs/release-notes
  title: "Release notes \_|\_ Access Approval \_|\_ Google Cloud Documentation"
  fetched_via: browser_cdp
  cdp_url: http://127.0.0.1:9222
---

Release notes | Access Approval | Google Cloud Documentation
Skip to main content
Technology areas
close
AI and ML
Application development
Application hosting
Compute
Data analytics and pipelines
Databases
Distributed, hybrid, and multicloud
Industry solutions
Migration
Networking
Observability and monitoring
Security
Storage
Cross-product tools
close
Access and resources management
Costs and usage management
Infrastructure as code
SDK, languages, frameworks, and tools
More
/
Console
English
Deutsch
Español – América Latina
Français
Português – Brasil
中文 – 简体
日本語
한국어
Google Developer Program View your saved pages and finish your Google Developer Profile setup here.
Cloud provider access management
Access Approval
Start free
Overview
Guides
Reference
Resources
More
Technology areas
More
Overview
Guides
Reference
Resources
Cross-product tools
More
Console
Access Approval
Release notes
Pricing
Service Level Agreement
Getting support
AI and ML
Application development
Application hosting
Compute
Data analytics and pipelines
Databases
Distributed, hybrid, and multicloud
Industry solutions
Migration
Networking
Observability and monitoring
Security
Storage
Access and resources management
Costs and usage management
Infrastructure as code
SDK, languages, frameworks, and tools
On this page
March 19, 2026
February 25, 2026
February 23, 2026
February 19, 2026
February 05, 2026
January 23, 2026
January 05, 2026
December 22, 2025
December 16, 2025
December 05, 2025
November 19, 2025
November 17, 2025
October 31, 2025
October 30, 2025
October 29, 2025
October 17, 2025
October 14, 2025
October 13, 2025
October 08, 2025
October 03, 2025
October 01, 2025
September 26, 2025
September 25, 2025
September 24, 2025
September 10, 2025
September 08, 2025
September 02, 2025
August 18, 2025
August 07, 2025
August 04, 2025
July 18, 2025
June 16, 2025
May 30, 2025
April 04, 2025
March 28, 2025
March 14, 2025
February 21, 2025
February 06, 2025
December 20, 2024
November 29, 2024
November 27, 2024
November 18, 2024
October 25, 2024
September 27, 2024
September 24, 2024
September 13, 2024
September 12, 2024
September 09, 2024
August 31, 2024
August 12, 2024
August 09, 2024
August 06, 2024
July 12, 2024
July 05, 2024
July 01, 2024
June 24, 2024
June 07, 2024
June 06, 2024
May 31, 2024
April 24, 2024
April 18, 2024
March 15, 2024
March 14, 2024
March 06, 2024
January 11, 2024
November 23, 2023
November 16, 2023
November 10, 2023
October 12, 2023
October 05, 2023
September 29, 2023
September 18, 2023
September 15, 2023
September 08, 2023
August 31, 2023
August 28, 2023
August 25, 2023
August 05, 2023
August 04, 2023
July 21, 2023
July 14, 2023
June 30, 2023
June 29, 2023
June 26, 2023
June 20, 2023
June 07, 2023
May 26, 2023
March 31, 2023
March 24, 2023
March 10, 2023
March 06, 2023
February 24, 2023
February 13, 2023
February 10, 2023
December 15, 2022
November 11, 2022
September 29, 2022
September 14, 2022
August 25, 2022
August 03, 2022
May 25, 2022
January 31, 2022
December 13, 2021
November 11, 2021
October 07, 2021
August 30, 2021
July 26, 2021
June 21, 2021
May 17, 2021
April 27, 2021
April 15, 2021
March 24, 2021
March 15, 2021
January 21, 2021
October 19, 2020
August 06, 2020
April 15, 2020
January 27, 2020
March 29, 2019
Home
Documentation
Security
Cloud provider access management
Access Approval
Resources
Was this helpful?
Send feedback
Release notes
Stay organized with collections
Save and categorize content based on your preferences.
On this page
March 19, 2026
February 25, 2026
February 23, 2026
February 19, 2026
February 05, 2026
January 23, 2026
January 05, 2026
December 22, 2025
December 16, 2025
December 05, 2025
November 19, 2025
November 17, 2025
October 31, 2025
October 30, 2025
October 29, 2025
October 17, 2025
October 14, 2025
October 13, 2025
October 08, 2025
October 03, 2025
October 01, 2025
September 26, 2025
September 25, 2025
September 24, 2025
September 10, 2025
September 08, 2025
September 02, 2025
August 18, 2025
August 07, 2025
August 04, 2025
July 18, 2025
June 16, 2025
May 30, 2025
April 04, 2025
March 28, 2025
March 14, 2025
February 21, 2025
February 06, 2025
December 20, 2024
November 29, 2024
November 27, 2024
November 18, 2024
October 25, 2024
September 27, 2024
September 24, 2024
September 13, 2024
September 12, 2024
September 09, 2024
August 31, 2024
August 12, 2024
August 09, 2024
August 06, 2024
July 12, 2024
July 05, 2024
July 01, 2024
June 24, 2024
June 07, 2024
June 06, 2024
May 31, 2024
April 24, 2024
April 18, 2024
March 15, 2024
March 14, 2024
March 06, 2024
January 11, 2024
November 23, 2023
November 16, 2023
November 10, 2023
October 12, 2023
October 05, 2023
September 29, 2023
September 18, 2023
September 15, 2023
September 08, 2023
August 31, 2023
August 28, 2023
August 25, 2023
August 05, 2023
August 04, 2023
July 21, 2023
July 14, 2023
June 30, 2023
June 29, 2023
June 26, 2023
June 20, 2023
June 07, 2023
May 26, 2023
March 31, 2023
March 24, 2023
March 10, 2023
March 06, 2023
February 24, 2023
February 13, 2023
February 10, 2023
December 15, 2022
November 11, 2022
September 29, 2022
September 14, 2022
August 25, 2022
August 03, 2022
May 25, 2022
January 31, 2022
December 13, 2021
November 11, 2021
October 07, 2021
August 30, 2021
July 26, 2021
June 21, 2021
May 17, 2021
April 27, 2021
April 15, 2021
March 24, 2021
March 15, 2021
January 21, 2021
October 19, 2020
August 06, 2020
April 15, 2020
January 27, 2020
March 29, 2019
This document lists production updates to Access Approval. We recommend
that Access Approval developers periodically check this list for any
new announcements.
You can see the latest product updates for all of Google Cloud on the
Google Cloud page, browse and filter all release notes in the
Google Cloud console ,
or programmatically access release notes in
BigQuery .
To get the latest product updates delivered to you, add the URL of this page to your
feed
reader , or add the
feed URL directly.
March 19, 2026
v1
Feature
Flow Analyzer is generally available
(GA) .
February 25, 2026
v1
Feature
Vertex AI Agent Engine is generally available
(GA) .
February 23, 2026
v1
Feature
Secure Web Proxy is generally available (GA) .
February 19, 2026
v1
Feature
Unified Maintenance is generally available (GA) .
February 05, 2026
v1
Feature
Google Cloud Managed Lustre is generally available (GA) .
January 23, 2026
v1
Feature
Automotive AI Agent is generally available (GA) .
January 05, 2026
v1
Feature
Workflows is generally available (GA) .
December 22, 2025
v1
Feature
Backup and DR Service is generally available
(GA) .
December 16, 2025
v1
Feature
The new access insights
feature is generally available (GA) ,
allowing you to generate and download a single, filtered, organization-wide
report of Google administrative access to your data.
December 05, 2025
v1
Feature
Artifact Analysis is generally available
(GA) .
November 19, 2025
v1
Feature
Cloud Billing is available in Preview .
November 17, 2025
v1
Feature
Backup and DR Service is available in Preview .
October 31, 2025
v1
Feature
Vertex AI model tuning is generally available
(GA) .
v1
Feature
Cloud Scheduler is generally available
(GA) .
October 30, 2025
v1
Feature
IAM System for Cross-domain Identity Management (SCIM) Service is generally
available (GA) .
October 29, 2025
v1
Feature
Text-to-Speech is generally available
(GA) .
October 17, 2025
v1
Feature
Cloud Domains is generally available (GA) .
October 14, 2025
v1
Feature
Google Cloud Managed Service for Apache Kafka is generally available
(GA) .
October 13, 2025
v1
Feature
IAM System for Cross-domain Identity Management (SCIM) Service is available in
Preview .
October 08, 2025
v1
Feature
NCC Gateway is generally available (GA) .
v1
Feature
Cloud Domains is available in Preview .
October 03, 2025
v1
Feature
NCC Gateway is available in Preview .
October 01, 2025
v1
Feature
Certificate Manager is generally available (GA) .
September 26, 2025
v1
Feature
Memorystore for Redis Cluster is generally available (GA) .
v1
Feature
Memorystore for Valkey is generally available (GA) .
September 25, 2025
v1
Feature
Cloud TPU is generally available (GA) .
September 24, 2025
v1
Feature
Google Cloud NetApp Volumes is generally available (GA) .
v1
Feature
VM Manager is generally available (GA) .
September 10, 2025
v1
Feature
Access Approval supports Chrome Enterprise Premium secure gateway in the GA stage.
September 08, 2025
v1
Feature
Image streaming is generally available (GA) .
September 02, 2025
v1
Feature
Access Approval supports Web Risk in the GA stage.
August 18, 2025
v1
Feature
Access Approval supports Firebase Data Connect in the GA stage.
August 07, 2025
v1
Feature
Access Approval supports Security Command Center Premium tier in the GA stage.
August 04, 2025
v1
Feature
Access Approval supports Firebase Data Connect in the Preview stage.
July 18, 2025
v1
Feature
Access Approval supports Cloud Deploy in the GA stage.
v1
Feature
Access Approval supports Cloud Asset Inventory in the GA stage.
v1
Feature
Access Approval supports Network Connectivity Center in the GA stage.
June 16, 2025
v1
Feature
Access Approval supports Cloud Trace in the GA stage.
May 30, 2025
v1
Feature
Access Approval supports App Hub in the GA stage.
April 04, 2025
v1
Feature
Access Approval supports Document AI in the GA stage.
v1
Feature
Access Approval supports Storage Intelligence in the GA stage.
March 28, 2025
v1
Feature
Access Approval supports Org Lifecycle API in the GA stage.
v1
Feature
Access Approval supports Integration Connectors in the GA stage.
March 14, 2025
v1
Feature
Access Approval supports Access Transparency in the GA stage.
February 21, 2025
v1
Feature
Access Approval supports Cloud Workstations in the GA stage.
February 06, 2025
v1
Feature
Access Approval supports Looker Studio in the GA stage.
December 20, 2024
v1
Feature
Access Approval supports Virtual Private Cloud (VPC) in the GA stage.
v1
Feature
Access Approval supports regional internal proxy Network Load Balancer in the GA stage.
v1
Feature
Access Approval supports Looker (Google Cloud core) in the GA stage.
v1
Feature
Access Approval supports regional external Application Load Balancer in the GA stage.
v1
Feature
Access Approval supports regional internal Application Load Balancer in the GA stage.
v1
Feature
Access Approval supports regional external proxy Network Load Balancer in the GA stage.
v1
Feature
Access Approval supports internal passthrough Network Load balancer in the GA stage.
v1
Feature
Access Approval supports external passthrough Network Load Balancer in the GA stage.
November 29, 2024
v1
Feature
Access Approval supports Cloud Storage in the GA stage.
November 27, 2024
v1
Feature
Access Approval now supports Cloud Healthcare API in the GA stage.
November 18, 2024
v1
Feature
Access Approval now supports Cloud Healthcare API in the Preview stage.
October 25, 2024
v1
Feature
Access Approval supports Database Center in the GA stage.
September 27, 2024
v1
Feature
Access Approval supports Speech-to-Text in the GA stage.
v1
Feature
Access Approval supports Dataplex in the GA stage.
September 24, 2024
v1
Change
Access Approval supports Vertex AI Search in the GA stage.
September 13, 2024
v1
Feature
Access Approval supports Cloud Interconnect in the GA stage.
September 12, 2024
v1
Feature
Access Approval supports Database Center in the Preview stage.
September 09, 2024
v1
Feature
Access Approval supports Cloud Data Fusion in the GA stage.
August 31, 2024
v1
Feature
Access Approval supports Filestore in the GA stage.
August 12, 2024
v1
Change
Access Approval supports Cloud Armor in the GA stage.
August 09, 2024
v1
Change
Access Approval supports Storage Transfer Service in the GA stage.
August 06, 2024
v1
Feature
Access Approval supports Looker (Google Cloud core) in the Preview stage.
July 12, 2024
v1
Feature
Access Approval supports Dataform in the GA stage.
July 05, 2024
v1
Feature
Access Approval supports Cloud Armor in the Preview stage.
v1
Feature
Access Approval supports Cloud DNS in the GA stage.
July 01, 2024
v1
Feature
Access Approval supports Storage Transfer Service in the Preview stage.
June 24, 2024
v1
Change
Access Approval supports Apigee in the GA stage.
June 07, 2024
v1
Change
Access Approval supports Cloud Build in the GA stage.
June 06, 2024
v1
Change
Access Approval supports Cloud Service Mesh in the GA stage.
May 31, 2024
v1
Feature
Access Approval supports Cloud Service Mesh in the Preview stage.
v1
Change
Access Approval supports Resource Manager in the GA stage.
v1
Feature
Access Approval supports Apigee in the Preview stage.
April 24, 2024
v1
Feature
Access Approval supports Cloud Build in the Preview stage.
April 18, 2024
v1
Change
Access Approval supports Serverless VPC Access in the GA stage.
March 15, 2024
v1
Feature
Access Approval supports Google Distributed Cloud Edge in the GA stage.
March 14, 2024
v1
Change
Access Approval supports Workforce identity federation in the GA stage.
March 06, 2024
v1
Feature
Access Approval supports Serverless VPC Access in the Preview stage.
January 11, 2024
v1
Change
Access Approval supports Identity-Aware Proxy in the GA stage.
November 23, 2023
v1
Change
Access Approval supports Application Integration in the GA stage.
November 16, 2023
v1
Change
Access Approval supports Secure Source Manager in the GA stage.
November 10, 2023
v1
Change
Access Approval supports Cloud Monitoring in the GA stage.
October 12, 2023
v1
Change
Access Approval supports Access Context Manager in the GA stage.
October 05, 2023
v1
Change
Access Approval supports Firebase Security Rules in the GA stage.
September 29, 2023
v1
Feature
Access Approval supports Vertex AI Search in the Preview stage.
September 18, 2023
v1
Feature
Access Approval supports Cloud Monitoring in the Preview stage.
September 15, 2023
v1
Feature
Access Approval supports Access Context Manager in the Preview stage.
September 08, 2023
v1
Feature
Access Approval supports Firebase Security Rules in the Preview stage.
August 31, 2023
v1
Change
Access Approval supports the following services in the GA stage:
GKE Connect
GKE Hub
Cloud Tasks
Cloud Functions
August 28, 2023
v1
Change
Access Approval supports Anthos Identity Service in the GA stage.
August 25, 2023
v1
Change
Access Approval supports Memorystore for Redis in the GA stage.
August 05, 2023
v1
Change
Access Approval supports AlloyDB for PostgreSQL in the GA stage.
August 04, 2023
v1
Feature
Access Approval supports Cloud Run in the GA stage. For the complete list of supported services, see Supported services .
July 21, 2023
v1
Feature
Access Approval supports AlloyDB for PostgreSQL in the Preview stage.
July 14, 2023
v1
Feature
Access Approval supports Cloud Tasks in the Preview stage.
June 30, 2023
v1
Feature
Access Approval supports Memorystore for Redis in the Preview stage.
June 29, 2023
v1
Feature
Access Approval supports Application Integration in the Preview stage.
June 26, 2023
v1
Feature
Access Approval supports Eventarc in the GA stage.
June 20, 2023
v1
Feature
Access Approval supports Cloud Run in the Preview stage.
June 07, 2023
v1
Feature
Access Approval supports Anthos Identity Service in the Preview stage.
May 26, 2023
v1
Change
Access Approval supports Firestore in the GA stage.
March 31, 2023
v1
Change
Access Approval supports Cloud Composer in the GA stage.
March 24, 2023
v1
Change
Access Approval supports Certificate Authority Service in the GA stage.
v1
Feature
Access Approval supports Firestore in the Preview stage.
March 10, 2023
v1
Feature
Access Approval supports Certificate Authority Service in the Preview stage.
March 06, 2023
v1
Change
Access Approval supports Cloud NAT in the GA stage.
February 24, 2023
v1
Feature
Access Approval supports Cloud Composer in the Preview stage. For the complete list of supported services, see Supported services .
February 13, 2023
v1
Change
Access Approval supports Artifact Registry in the GA stage.
February 10, 2023
v1
Change
Access Approval supports Cloud NAT in the Preview stage. For the complete list of supported services, see Supported services .
December 15, 2022
v1
Change
Access Approval supports Artifact Registry in the Preview stage. For the complete list of services that Access Approval supports, see Supported services .
November 11, 2022
v1
Feature
Access Approval lets you revoke active access requests using the Google Cloud console.
September 29, 2022
v1
Change
Access Approval supports Dataproc in the GA stage.
September 14, 2022
v1
Change
Access Approval supports Secret Manager in the GA stage.
August 25, 2022
v1
Change
Access Approval supports Dataproc in the Preview stage.
August 03, 2022
v1
Feature
Access Approval supports Secret Manager in the Preview stage.
May 25, 2022
v1
Feature
Access Approval lets you know if the notification emails for access requests don't get delivered to you because you provided an incorrect email address while setting up the notification configurations.
January 31, 2022
v1
Announcement
Access Approval provides a public issue tracker that you can use for suggesting product features, providing product and documentation feedback, and reporting issues. For more information, see File bugs or feature requests .
December 13, 2021
v1
Feature
Access Approval provides a preview feature that uses a cryptographic key to sign an access request. Access Approval uses this signature to verify the integrity of the approval.
You can either use the default Google-managed signing key or bring your own signing key. To bring your own signing key, you must meet certain requirements . Using the default Google-managed signing key doesn't require any additional configuration.
For more information about how signing of an access request works, see Overview of Access Approval .
November 11, 2021
v1
Feature
Access Approval supports Organization Policy Service in the Preview stage.
October 07, 2021
v1
Change
Access Approval supports the following services in GA stage:
Cloud SQL
Google Kubernetes Engine
Speaker ID
August 30, 2021
v1
Feature
Access Approval supports Speaker ID in the Preview stage.
July 26, 2021
v1
Change
Access Approval supports the following Google Cloud services in the GA stage:
Cloud Data Loss Prevention
Cloud External Key Manager
Cloud HSM
Cloud Logging
June 21, 2021
v1
Feature
Access Approval supports Cloud External Key Manager in the Preview stage.
v1
Feature
Access Approval supports Cloud Data Loss Prevention in the Preview stage.
v1
Feature
Access Approval supports Cloud HSM in the Preview stage.
May 17, 2021
v1
Feature
Access Approval supports Cloud SQL in Preview stage.
April 27, 2021
v1
Feature
Access Approval supports Cloud Spanner in GA stage.
v1
Feature
Access Approval supports Google Kubernetes Engine in Preview stage.
April 15, 2021
v1
Feature
You can view Access Transparency logs associated with an approval request on the Access Approval UI.
March 24, 2021
v1
Feature
Access Transparency logs contain a new field called accessApprovals . This field lists the approvals that granted access to a resource that is enrolled in Access Approval . Access Transparency logs published before March 24, 2021 will not have this field populated. This feature is subject to Access Approval exclusions and only available for the services supported by Access Approval .
March 15, 2021
v1
Feature
Access Approval supports Cloud Logging and Cloud Spanner in Preview stage.
January 21, 2021
v1
Change
VPC Service Controls supports Access Approval in GA stage. For more information, see the list of VPC Service Controls supported products .
October 19, 2020
v1
Change
The UpdateAccessApprovalSettings API now accepts product names (as listed here ) in enrolledServices.cloudProduct, and the return values from GetAccessApprovalSettings and UpdateAccessApprovalSettings will contain product names as well.
v1
Feature
History view is now available on the Access Approval UI. This page shows the access reason, request status, response/expiration times (if applicable) of each request which has been previously acted upon, as well as details and audit logs associated with the request.
v1
Feature
Cloud IAM and BigQuery are in General Availability (GA) support in Access Approval API.
August 06, 2020
v1
Feature
Added 'implicit' field to DismissDecision structure to indicate if an approval request was dismissed implicitly by timeout rather than by user action.
v1
Feature
Added 'history' filter value to ListApprovalRequests method.
April 15, 2020
v1
Feature
Access Approval API reached General Availability (GA) status.
January 27, 2020
v1beta1
Feature
The v1 endpoint is available and functionally equivalent to v1beta1. The v1beta1 endpoint will stop accepting requests on August 1, 2020.
v1beta1
Feature
AccessApprovalSettings now includes the configuration of enrollment at the project, folder or organization level. Project and folder enrollment can be done for 'all' cloud products only, while organization enrollment can be done for either of individual or 'all' products.
v1beta1
Feature
DeleteAccessApprovalSettings API has been added.
March 29, 2019
v1beta1
Feature
Beta release of Access Approval API.
Was this helpful?
Send feedback
Except as otherwise noted, the content of this page is licensed under the Creative Commons Attribution 4.0 License , and code samples are licensed under the Apache 2.0 License . For details, see the Google Developers Site Policies . Java is a registered trademark of Oracle and/or its affiliates.
Last updated 2026-04-02 UTC.
Need to tell us more?
[[["Easy to understand","easyToUnderstand","thumb-up"],["Solved my problem","solvedMyProblem","thumb-up"],["Other","otherUp","thumb-up"]],[["Hard to understand","hardToUnderstand","thumb-down"],["Incorrect information or sample code","incorrectInformationOrSampleCode","thumb-down"],["Missing the information/samples I need","missingTheInformationSamplesINeed","thumb-down"],["Other","otherDown","thumb-down"]],["Last updated 2026-04-02 UTC."],[],[]]
Products and pricing
See all products
Google Cloud pricing
Google Cloud Marketplace
Contact sales
Support
Community forums
Support
Release Notes
System status
Resources
GitHub
Getting Started with Google Cloud
Code samples
Cloud Architecture Center
Training and Certification
Engage
Blog
Events
X (Twitter)
Google Cloud on YouTube
Google Cloud Tech on YouTube
About Google
Privacy
Site terms
Google Cloud terms
Manage cookies
Our third decade of climate action: join us
Sign up for the Google Cloud newsletter
Subscribe
English
Deutsch
Español – América Latina
Français
Português – Brasil
中文 – 简体
日本語
한국어
close
Welcome to Cloud Shell
Cloud Shell is a development environment that you can use in the browser:
Activate Cloud Shell to explore Google Cloud with a terminal and an editor
Start a free trial to get $300 in free credits
Activate Cloud Shell
Start a free trial
