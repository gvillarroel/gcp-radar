---
title: "Use the quota adjuster \_|\_ Cloud Quotas \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/docs/quotas/quota-adjuster
...
When you enable the quota adjuster , it monitors all
...
You can view requests made by the
quota adjuster in the Quotas & System Limits page of
Google Cloud console. You can also set up alerts to monitor changes
...
available in the Google Cloud console. To access quota adjuster
settings at the folder or organization level, use the Cloud Quotas API
( Preview ) or the
Google Cloud CLI Cloud Quotas commands
(available at the beta release level ).
The quota adjuster isn't available for all quotas. To learn
more, see Availability in this document.
Availability
...
Compute Engine API
Unique Cloud Router dynamic route prefixes from other regions per region per VPC Network
Compute Engine API
...
Note: To see the full list of available dimensions, expand this entry in the Google Cloud console.
Vertex AI API
...
gcloud beta quotas adjuster settings update command .
...
adjuster is denied, set up quota adjuster alerts .
Send feedback
