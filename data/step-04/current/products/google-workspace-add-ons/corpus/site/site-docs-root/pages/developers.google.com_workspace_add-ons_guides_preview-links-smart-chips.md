---
title: "Preview links with smart chips \_|\_ Google Workspace add-ons \_|\_ Google\
  \ for Developers"
url: https://developers.google.com/workspace/add-ons/guides/preview-links-smart-chips
...
A Google Workspace
account.
A Google Workspace add-on. To build an
add-on, follow this
quickstart .
Node.js
A Google Workspace
...
add-on, follow this
quickstart .
Note: The Node.js code samples in this guide are written to run as a
Cloud
Function
Python
A Google Workspace
...
Configure link previews in your
add-on's manifest.
...
add-on's manifest:
Under the addOns section, add the docs field to extend Docs,
the sheets field to extend Sheets, and the slides
field to extend Slides.
In each field, implement the linkPreviewTriggers trigger
that includes a runFunction (You define this function in the following
section, Build the smart chip and card ).
...
Note: For Slides,
labelText and localizedLabelText don't render, but you must include
labelText for when Slides starts
supporting third-party smart chips. localizedLabelText is optional.
{
"oauthScopes" : [
...
components for preview cards .
To build the smart chip and card for a link preview:
Implement the function that you specified in the linkPreviewTriggers
section of your add-on's
manifest:
The function must accept an event object containing
EDITOR_NAME .matchedUrl.url as an argument and return
a single Card object.
If your service requires authorization, the function must
also invoke the authorization flow .
For each preview card, implement any callback functions that provide
widget interactivity for the interface. For example, if you include a
button that says "View link," you can create an action that specifies a
callback function to open the link in a new window. To learn more about
widget interactions, see Add-on actions .
Note: For link previews, the card response is cached for each user for 5
minutes. During that time, if a user previews a link, they receive the cached
response. The cache is cleared when the user refreshes the file, or after 5
minutes. If the title of the returned card has changed, the app then
prompts the user to refresh the title.
...
https://www.example.com/support/cases/1234 . The smart chip displays a
support icon, and the preview card includes the case ID and a description.
If the user's locale is set to Spanish, the smart chip localizes its
labelText into Spanish.
Manifest
Apps Script
...
JSON
Note: To use the following manifest, replace the
URL value with the URL of the deployed function. You
can deploy the function using the code sample .
{
"oauthScopes" : [
"https://www.googleapis.com/auth/workspace.linkpreview"
],
...
[[["Easy to understand","easyToUnderstand","thumb-up"],["Solved my problem","solvedMyProblem","thumb-up"],["Other","otherUp","thumb-up"]],[["Missing the information I need","missingTheInformationINeed","thumb-down"],["Too complicated / too many steps","tooComplicatedTooManySteps","thumb-down"],["Out of date","outOfDate","thumb-down"],["Samples / code issue","samplesCodeIssue","thumb-down"],["Other","otherDown","thumb-down"]],["Last updated 2026-04-03 UTC."],[],["Google Workspace add-ons allow link previews within Docs, Sheets, and Slides. Users input URLs, which are replaced with smart chips (Docs/Sheets) or text links (Slides) that display brief summaries. Hovering reveals a preview card with detailed content. Add-ons require manifest configuration, including URL patterns, and a callback function (`runFunction`) to generate card objects. Authentication is handled via OAuth or custom methods. Card responses are cached for 5 minutes. Add-on examples are provided for various languages, showcasing a support case link preview implementation.\n"]]
