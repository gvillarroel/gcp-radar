---
title: "Authenticate to Google Cloud APIs from GKE workloads \_|\_ GKE security \_\
...
source_metadata:
  url: https://docs.cloud.google.com/kubernetes-engine/docs/how-to/workload-identity
  title: "Authenticate to Google Cloud APIs from GKE workloads \_|\_ GKE security\
...
Autopilot
Standard
This page shows you how to more securely access Google Cloud APIs from
...
Ensure that the Google Cloud APIs that you want to access are supported
...
Ensure that you understand the limitations of Workload Identity Federation for GKE .
...
Configure applications to use Workload Identity Federation for GKE section.
In Standard, you enable Workload Identity Federation for GKE on clusters and node
pools using the Google Cloud CLI or the Google Cloud console.
...
using the gcloud CLI or the Google Cloud console. Existing node pools
...
Caution: Modifying the node pool immediately enables Workload Identity Federation for GKE for
any workloads running in the node pool. This prevents the workloads from using
the service account that your nodes use and might result in disruptions.
gcloud
In the Google Cloud console, activate Cloud Shell.
Activate Cloud Shell
At the bottom of the Google Cloud console, a
...
To let your GKE applications authenticate to Google Cloud
...
Google Cloud resources that your application needs to access. You must
...
Workload Identity Federation for GKE. However, this federated identity has specific limitations for each
supported Google Cloud API. If these limitations apply to you, use the
...
IAM service account.
For information on authorizing IAM service accounts to access Google Cloud
APIs, see Understanding service accounts .
...
name. For example, serviceAccount:example-project.svc.id.goog[example-namespace/example-serviceaccount] .
...
server . When you use Workload Identity Federation for GKE, your
...
from a different Google Cloud project when making calls to
the GenerateAccessToken and the GenerateIdToken methods in the IAM Service Account Credentials API . This lets you avoid using the entire quota in
...
in the Google Cloud console for your quota project:
Go to APIs
