...
title: "Configure your Google Cloud resources \_|\_ Cloud Interconnect \_|\_ Google\
  \ Cloud Documentation"
url: https://docs.cloud.google.com/network-connectivity/docs/interconnect/how-to/cci/oci/configure-google-resources
...
network and a peer network (in this case, your OCI network).
...
For more information about the ASNs that OCI reserves for itself, see the
OCI documentation .
Enter a VLAN ID that's between 100 and 4094 and unique among
the VLAN attachments associated with this connection. Typically, in Google Cloud, you
can enter a broader range of possible IDs. However, you must use this value again in OCI, and
OCI has more narrow requirements than Google Cloud.
...
We recommend leaving the IPv4 subnet mask set
to /29 .
Click Next .
...
Enter a VLAN ID for the redundant attachment. We recommend that you use the same value that
...
We recommend that you do not set a value for
--subnet-length but accept the default value of 29 .
...
VPC network and your OCI network. To that end, configure a BGP session
for each of your VLAN attachments. The sessions aren't active until you configure your
OCI resources, but you can configure the Google Cloud side of the sessions
now.
Console gcloud
More
...
OCI side of the peering. Use 31898 .
...
key. Later, when you configure peering in OCI, you must use the same key on the OCI side of
peering. OCI supports only alphanumeric characters for the key. For
more information about Google Cloud support for MD5 authentication, see
...
Advertised prefixes and priorities .
Optional: Set MD5 Authentication to Enabled , and enter your secret MD5 authentication
key. Later, when you configure peering in OCI, you must use the same key
on the OCI side of peering. OCI supports only alphanumeric characters for the key.
Click Save and continue .
...
located
YOUR_KEY :
the secret key to use for MD5 authentication; later, when you configure peering in OCI,
you must use the same key (OCI supports only alphanumeric characters for the key)
