---
title: "Custom organization policy constraints \_|\_ Storage Transfer Service \_|\_\
  \ Google Cloud Documentation"
url: https://docs.cloud.google.com/storage-transfer/docs/custom-constraints
...
source_metadata:
  url: https://docs.cloud.google.com/storage-transfer/docs/custom-constraints
  title: "Custom organization policy constraints \_|\_ Storage Transfer Service \_\
...
Benefits
Security and governance : Enforce centralized control over data transfer
operations to mitigate security risks associated with broad user permissions.
Fine-grained control : Restrict transfer jobs to specific allowed source
or destination buckets, or limit transfers to specific service pairs (e.g.,
only Cloud Storage to Cloud Storage transfers).
Before you begin
...
numbers, for example custom.enforceCloudStorageToCloudStorageOnly . This field can contain up to
...
or numbers, for example, custom.enforceCloudStorageToCloudStorageOnly . This field can contain up to 70
...
storagetransfer.googleapis.com/TransferJob . Most resource types support up to 20 custom
constraints. If you attempt to create more custom constraints, the operation fails.
...
"has(resource.transferSpec.gcsDataSource) && has(resource.transferSpec.gcsDataSink)" .
...
example, custom.enforceCloudStorageToCloudStorageOnly .
...
newly created transfer jobs to have logging enabled. You
can test the policy by trying to create a job that the policy should prevent
(i.e., one with logging disabled).
Create the constraint
Save the following file as constraint-transfer-job.yaml :
...
...
Create the policy
Save the following file as policy-transfer-job.yaml :
...
Test the policy
Try to create a transfer job where the logging configuration is completely
omitted or set to an empty configuration:
...
Description
Constraint syntax
Restrict transfers to only allow Cloud Storage to Cloud Storage
...
Storage Transfer Service supported resources
The following table lists the Storage Transfer Service resources that you can reference
