---
title: "Create an A4X Max Slurm cluster \_|\_ Cluster Toolkit \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/cluster-toolkit/docs/deploy/slurm/create-a4x-max-cluster
...
source_metadata:
  url: https://docs.cloud.google.com/cluster-toolkit/docs/deploy/slurm/create-a4x-max-cluster
  title: "Create an A4X Max Slurm cluster \_|\_ Cluster Toolkit \_|\_ Google Cloud\
    \ Documentation"
...
Guides
Send feedback
Create an A4X Max Slurm cluster
Stay organized with collections
Save and categorize content based on your preferences.
This document describes how to deploy an A4X Max Slurm cluster. These clusters
provide direct access to the host server's CPU and memory, optimizing your
environment for compute-intensive, network-bound machine learning (ML) training
and high performance computing (HPC) workloads.
The A4X Max machine series runs on an exascale platform based on NVIDIA's
rack-scale architecture and is optimized for compute and memory-intensive,
network-bound ML training and HPC workloads. A4X Max offers bare metal
instances, which provide direct access to the host server's CPU and memory,
...
A4X Max limitations
The following limitations apply to A4X Max bare metal instances in your Slurm cluster:
Caution: The
...
A4X Max : requires a minimum of 10 TiB
...
Cluster Toolkit directory by using the cd command:
cd cluster-toolkit
This cluster deployment requires Cluster Toolkit v1.62.0 or
later. Check your installed Cluster Toolkit version:
./gcluster --version
...
Cluster Toolkit directory. This process can take about 20 to 30
minutes.
./gcluster deploy -d a4xmax-bm-slurm-deployment.yaml examples/machine-learning/a4x-maxgpu-4g-metal/a4xmax-bm-slurm-blueprint.yaml
...
To access your cluster, you must sign in to the Slurm login node. To sign in,
you can use either Google Cloud console or Google Cloud CLI.
Console
...
do so, use NCCL/gIB tests , which are
...
./gcluster deploy -d a4xmax-bm-slurm-deployment.yaml examples/machine-learning/a4x-maxgpu-4g-metal/a4xmax-bm-slurm-blueprint.yaml --only cluster-env,cluster -w
...
By default, the A4X Max blueprint turns on deletion protection on the
Filestore instance. To delete the Filestore instance when you
destroy the Slurm cluster, you must turn off deletion protection before you use
the destroy command. For instructions, see set or remove deletion protection on
an existing instance .
If you are connected to the cluster, then disconnect from it.
Before you use the destroy command, go to the root of the
