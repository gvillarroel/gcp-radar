...
GPUDirect-TCPXO.
Additionally, you must install a NCCL net plugin into the execution
environment of the workload. A PyTorch Docker image distributes both the RxDM
and the plugin.
...
To turn on optimized NCCL communication tuning on your cluster, you must sign in
to the Slurm login node. To sign in, you can use either Google Cloud console or
Google Cloud CLI.
Console
Go to the Compute Engine > VM instances page.
Go to VM instances
Locate the login node. The node has a name similar to a3mega-login-001 .
...
To run any job on an A3 Mega cluster, you must set several environment variables
to turn on high-performance networking with GPUDirect-TCPXO. Because you use
enroot containers in this procedure to launch workloads, you must set these
variables in the container environment instead of the host environment.
