---
title: "Create an A4X Max GKE cluster \_|\_ Cluster Toolkit \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/cluster-toolkit/docs/deploy/gke/create-a4x-max-cluster
...
Guides
Send feedback
Create an A4X Max GKE cluster
Stay organized with collections
Save and categorize content based on your preferences.
This document describes how to deploy a GKE cluster that uses A4X
Max accelerator-optimized machine types by using Cluster Toolkit.
...
A4X Max compute instances cannot be provisioned by using flex-start.
You must use the
reservation-bound
...
Note: When you request A4X Max capacity, you obtain it in all capacity
mode . This mode is the only supported reservation operational mode for
A4X Max machine types. For more information about all capacity mode, see
Reservation operational
mode .
Requirements
The following requirements apply to an AI-optimized GKE cluster
that uses A4X Max instances:
Verify that you use GKE version 1.35.0-gke.2745000 or later
(for 1.35), or version 1.34.3-gke.1318000 or later (for 1.34). These versions
help ensure that A4X Max uses the following:
R580.95.05, which is the minimum GPU driver version for the GB300 GPUs in
A4X Max bare metal instances. This driver is turned on by default.
...
Software
Support .
GPUDirect RDMA, which is recommended to let A4X Max node pools use the
networking capabilities of A4X Max.
To use GPUDirect RDMA, the GKE nodes must use a
Container-Optimized OS node image. Ubuntu and Windows node images are
not supported.
Create an A4X Max GKE cluster
To create an A4X Max GKE cluster, follow these steps:
Note: If you create multiple clusters by using these same cluster blueprints,
help ensure that all VPC network and subnet names are unique
per project to prevent errors.
...
deployment.
In the examples/gke-a4x-max-bm/gke-a4x-max-bm-deployment.yaml
...
The default value is gke-a4x-max-bm .
...
pool of A4X Max machines. This zone should match the zone
where machines are available in your reservation.
NODE_COUNT : the number of A4X Max
...
A4X Max node pools. The disk size that you need depends on
...
A4X_MAX_NODE_POOL_DISK_SIZE_GB : the size of the boot
disk for each node of the A4X Max node pool. The smallest
allowed disk size is 10 .
...
make the following changes in the examples/gke-a4x-max-bm/gke-a4x-max-bm-deployment.yaml
file:
...
by using A4X Max machine types:
cd ~/cluster-toolkit
./gcluster deploy -d \
