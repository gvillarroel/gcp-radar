---
title: "Restore an instance overview \_|\_ Cloud SQL for PostgreSQL \_|\_ Google Cloud\
  \ Documentation"
...
source_metadata:
  url: https://docs.cloud.google.com/sql/docs/postgres/backup-recovery/restore
  title: "Restore an instance overview \_|\_ Cloud SQL for PostgreSQL \_|\_ Google\
    \ Cloud Documentation"
...
All Cloud SQL Enterprise Plus edition instances store their write-ahead logs
...
January 9, 2023
store logs used for PITR
in Cloud Storage.
If your instance uses Cloud Storage to store write-ahead logs,
...
check where transaction logs are stored for your instance .
For instances that store write-ahead logs only on disk, you can
...
Although an instance stores the write-ahead logs used for
PITR in Cloud Storage, the instance also keeps a smaller number of duplicate
write-ahead logs on disk to allow for replication of the
logs to Cloud Storage. By default, when you create an instance with PITR
enabled, the instance stores its write-ahead logs for PITR
...
For PITR write-ahead logs that are stored on disk , that
...
write-ahead logs are stored on disk, are being switched to
...
Note: If your instance has a large number of write-ahead logs on
...
doesn't purge all of the write-ahead logs simultaneously.
For
