---
title: "About read pools \_|\_ Cloud SQL for PostgreSQL \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/sql/docs/postgres/about-read-pools
...
To see how to configure your read pool to automatically resize to adapt to
your application's changing workload needs, see
Read pool autoscaling .
...
If you want to scale the number of read replicas beyond a total count of
10, then you must set reset the values for the max_wal_senders
and max_replication_slots database flags on the primary
instance to a value greater than or equal to the number of read replicas you
want to use. For example, if your primary instance has two read replicas and
two read pools of ten replicas each, then you must reset the max_wal_senders
and max_replication_slots configuration flags to 22
or greater. If your instance uses PostgreSQL 18, then you must also
set the max_active_replication_origins flag to 22
or greater.
Limitations
...
update your read pool to a new major database version.
...
cascading replica.
If you want to convert a regional read replica to a read pool, then you must
first convert the regional read replica to a zonal read replica.
...
If your application must connect to a dedicated read replica—for
example, because of secondary indexes present on a specific read replica—create
a Cloud SQL read replica and use it instead.
What's next
