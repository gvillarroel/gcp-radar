---
title: "Quickstart: Build and deploy a Java Spring Boot web app to Google Cloud with\
  \ Cloud Run \_|\_ Google Cloud Documentation"
...
source_metadata:
  url: https://docs.cloud.google.com/run/docs/quickstarts/build-and-deploy/deploy-java-service
  title: "Quickstart: Build and deploy a Java Spring Boot web app to Google Cloud\
    \ with Cloud Run \_|\_ Google Cloud Documentation"
...
From the console, create a new empty web project using the curl and unzip
commands:
...
To use this curl command on Microsoft Windows, you'll need one of
following command lines, or optionally use the Spring Initializr (configuration preloaded) to generate the project:
WSL (preferred)
cygwin
Update the HelloworldApplication class in
src/main/java/com/example/helloworld/HelloworldApplication.java by adding
a @RestController to handle the / mapping and also add a @Value field
to provide the NAME environment variable:
...
Set the server port to be defined by the PORT environment variable in src/main/resources/application.properties :
server . port = $ { PORT : 8080 }
This code creates a basic web server that listens on the port defined by
the PORT environment variable .
Create a project.toml file in the application root directory
to specify a Java version:
[[build.env]]
name = "GOOGLE_RUNTIME_VERSION"
value = "25"
Your app is finished and ready to be deployed.
Note: To deploy Java to Cloud Run with other frameworks, review the
Knative sample for Spark .
Deploy to Cloud Run from source
Deploy from source automatically builds a container image from source code
and deploys it.
To deploy from source:
In the directory that
contains the pom.xml file, deploy the app using the following command:
...
Success: You deployed a Java Spring Boot web app to Cloud Run.
Cloud Run locations
