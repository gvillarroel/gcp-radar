---
title: "Quickstart: Deploy a Python (FastAPI) web app to Google Cloud with Cloud Run\
...
source_metadata:
  url: https://docs.cloud.google.com/run/docs/quickstarts/build-and-deploy/deploy-python-fastapi-service
  title: "Quickstart: Deploy a Python (FastAPI) web app to Google Cloud with Cloud\
...
Quickstart: Build and deploy a Python (FastAPI) web app to Cloud Run
...
then you already have the required permissions.
Verify that billing is enabled for your Google Cloud project .
To set the default project for your Cloud Run service, run the
following command:
gcloud config set project PROJECT_ID
...
This code creates a web service that responds to HTTP GET requests. HTTP
handling is done by a FastAPI web framework in the container.
Create a file named requirements.txt and paste the following code into it:
fastapi [ standard ]~= 0.0
