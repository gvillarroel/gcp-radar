---
title: "Google Drive API overview \_|\_ Google for Developers"
...
Google , it handles the OAuth 2.0 flow
...
and upload files to Drive.
Search for files and folders stored in
Drive. Create complex search queries that return any of the
file metadata fields in the files resource.
Let users share files, folders, and drives
to collaborate on content.
Combine with the
Google Picker API to search all
files in Drive, then return the filename, URL, last modified
date, and user.
Create third-party shortcuts that
...
application-specific data so the app cannot access
...
using the Google Drive UI . It's Google's standard web UI that you can
