---
title: "Cloud Storage Text to BigQuery template with Python UDF \_|\_ Cloud Dataflow\
  \ \_|\_ Google Cloud Documentation"
url: https://docs.cloud.google.com/dataflow/docs/guides/templates/provided/cloud-storage-to-bigquery-python-udf
...
The Cloud Storage Text to BigQuery with Python UDF pipeline is a batch pipeline that
reads text files stored in Cloud Storage, transforms them using a Python
user-defined function (UDF), and appends the result to a BigQuery table.
Note: If you want to overwrite data in the BigQuery
table instead of appending, update WriteDisposition in the
template source code from WRITE_APPEND to
WRITE_TRUNCATE .
Pipeline requirements
Create a JSON file that describes your BigQuery schema.
Ensure that there is a top-level JSON array titled BigQuery Schema and that its
...
Create a Python ( .py ) file with your UDF function that supplies the logic
...
The Cloud Storage URI of the Python code file that defines the user-defined function (UDF)
you want to use. For example, gs://my-bucket/my-udfs/my_file.py .
pythonExternalTextTransformFunctionName
The name of the Python user-defined function (UDF) that you want to use.
inputFilePattern
...
Optionally, you can extend this template by writing a user-defined function
(UDF). The template calls the UDF for each input element. Element payloads are
serialized as JSON strings. For more information, see
Create
user-defined functions for Dataflow templates .
Function specification
The UDF has the following specification:
Input : a line of text from a Cloud Storage input
file.
Output : a JSON string that matches the schema of the
...
the Text Files on Cloud Storage to BigQuery with Python UDF (Batch) template.
...
PATH_TO_PYTHON_UDF_FILE :
The Cloud Storage URI of the Python code file that defines the user-defined function (UDF)
you want to use. For example, gs://my-bucket/my-udfs/my_file.py .
PATH_TO_TEXT_DATA : your Cloud Storage path to your
text dataset
BIGQUERY_TABLE : your BigQuery table name
PATH_TO_TEMP_DIR_ON_GCS : your Cloud Storage path to the
temp directory
API
