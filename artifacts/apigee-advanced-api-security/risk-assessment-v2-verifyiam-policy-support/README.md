# Risk Assessment v2 VerifyIAM policy support

Product: Apigee Advanced API Security
Feature slug: `risk-assessment-v2-verifyiam-policy-support`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

Risk Assessment v2 now supports the VerifyIAM policy in assessments.

## Lifecycle

- Latest feature date: 2025-12-17
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- access (evidence: [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores), [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security), [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts))
- allow (evidence: [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores), [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security), [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts))
- audit (evidence: [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores), [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security), [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts))
- auth (evidence: [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores), [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security), [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts))
- iam (evidence: [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores), [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security), [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts))
- key (evidence: [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores), [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security), [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts))
- logging (evidence: [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores), [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security), [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts))
- policy (evidence: [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores), [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security), [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts))
- role (evidence: [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores), [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security), [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts))
- security (evidence: [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores), [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security), [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts))

## Official Evidence

- [https://docs.cloud.google.com/apigee/docs/api-security](https://docs.cloud.google.com/apigee/docs/api-security)
- [https://docs.cloud.google.com/apigee/docs/api-security/security-alerts](https://docs.cloud.google.com/apigee/docs/api-security/security-alerts)
- [https://docs.cloud.google.com/apigee/docs/api-security/security-scores](https://docs.cloud.google.com/apigee/docs/api-security/security-scores)
