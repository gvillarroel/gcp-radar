# Network Connectivity Center

Service card: [card.json](./card.json)

Generated from Step 08 card: `data/step-08/current/products/network-connectivity-center/card.json`

## Summary

- Promoted features: 18
- Step 07 product status: PASS
- Corpus health: healthy
- Latest feature date: 2026-03-25
- Official source links: 18

## Official Service Evidence

- [https://docs.cloud.google.com/network-connectivity/docs/concepts/key-terms](https://docs.cloud.google.com/network-connectivity/docs/concepts/key-terms)
- [https://docs.cloud.google.com/network-connectivity/docs/how-to/choose-product](https://docs.cloud.google.com/network-connectivity/docs/how-to/choose-product)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/access-control](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/access-control)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/asn-requirements](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/asn-requirements)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/audit-logging](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/audit-logging)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/connect-vpc-networks](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/connect-vpc-networks)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/connectivity-topologies](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/connectivity-topologies)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/data-transfer](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/data-transfer)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/data-transfer-topology](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/data-transfer-topology)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/dynamic-route-exchange-with-vpc-spokes](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/dynamic-route-exchange-with-vpc-spokes)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/ncc-gateway-overview](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/ncc-gateway-overview)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/overview](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/overview)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/producer-vpc-spokes-overview](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/producer-vpc-spokes-overview)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/producer-vpc-spokes-supported-services](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/producer-vpc-spokes-supported-services)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/psc-propagated-connection-overview](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/concepts/psc-propagated-connection-overview)
- [https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/tutorials/connecting-two-offices-with-vpns](https://docs.cloud.google.com/network-connectivity/docs/network-connectivity-center/tutorials/connecting-two-offices-with-vpns)
- [https://docs.cloud.google.com/network-connectivity/docs/reference/networkconnectivity/rest](https://docs.cloud.google.com/network-connectivity/docs/reference/networkconnectivity/rest)

## Features

- [Cross-Cloud Interconnect](./cross-cloud-interconnect/README.md)
- [Custom organization policy constraints for Network Connectivity Center resources](./custom-organization-policy-constraints-for-network-connectivity-center-resources/README.md)
- [Hybrid spoke export and import filters](./hybrid-spoke-export-and-import-filters/README.md)
- [Include export filters](./include-export-filters/README.md)
- [IPv4 address range filtering for VPC spokes](./ipv4-address-range-filtering-for-vpc-spokes/README.md)
- [IPv4 and IPv6 address range filtering for VPC spokes](./ipv4-and-ipv6-address-range-filtering-for-vpc-spokes/README.md)
- [IPv6 subnet exchange](./ipv6-subnet-exchange/README.md)
- [Preset topologies](./preset-topologies/README.md)
- [Private Service Connect connection propagation](./private-service-connect-connection-propagation/README.md)
- [Privately used public IPv4 address exchange](./privately-used-public-ipv4-address-exchange/README.md)
- [Producer VPC spokes](./producer-vpc-spokes/README.md)
- [Producer VPC spokes for Google Cloud NetApp Volumes](./producer-vpc-spokes-for-google-cloud-netapp-volumes/README.md)
- [Route exchange with VPC spokes](./route-exchange-with-vpc-spokes/README.md)
- [Router appliance spoke](./router-appliance-spoke/README.md)
- [Site-to-site data transfer](./site-to-site-data-transfer/README.md)
- [Static routes for Network Connectivity Center](./static-routes-for-network-connectivity-center/README.md)
- [VPC network connectivity](./vpc-network-connectivity/README.md)
- [VPC spoke updates for IPv6 subnet exchange](./vpc-spoke-updates-for-ipv6-subnet-exchange/README.md)
