# JupyterLab Cell Tags extension

Product: Deep Learning VM Images
Feature slug: `jupyterlab-cell-tags-extension`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

Deep Learning VM Images preinstall the jupyterlab-celltags extension.

## Lifecycle

- Latest feature date: 2019-04-26
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- access (evidence: [https://docs.cloud.google.com/deep-learning-vm/docs/jupyter](https://docs.cloud.google.com/deep-learning-vm/docs/jupyter), [https://docs.cloud.google.com/deep-learning-vm/docs/troubleshooting](https://docs.cloud.google.com/deep-learning-vm/docs/troubleshooting), [https://docs.cloud.google.com/deep-learning-vm/docs/cloud-marketplace](https://docs.cloud.google.com/deep-learning-vm/docs/cloud-marketplace))

## Official Evidence

- [https://docs.cloud.google.com/deep-learning-vm/docs/cloud-marketplace](https://docs.cloud.google.com/deep-learning-vm/docs/cloud-marketplace)
- [https://docs.cloud.google.com/deep-learning-vm/docs/jupyter](https://docs.cloud.google.com/deep-learning-vm/docs/jupyter)
- [https://docs.cloud.google.com/deep-learning-vm/docs/troubleshooting](https://docs.cloud.google.com/deep-learning-vm/docs/troubleshooting)

## Validation Findings

- `warn:thin_token_overlap` Supporting pages only weakly overlap with informative feature tokens.
