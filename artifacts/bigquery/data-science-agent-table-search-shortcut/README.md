# Data Science Agent table search shortcut

Product: BigQuery
Feature slug: `data-science-agent-table-search-shortcut`
Coverage: `LOW`
IAM mapping: `unknown`

## Technical Summary

The Data Science Agent in BigQuery supports @ syntax to search project tables.

## Lifecycle

- Latest feature date: 2025-09-08
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- permission

## Official Evidence

- [https://docs.cloud.google.com/bigquery/docs/ai-introduction](https://docs.cloud.google.com/bigquery/docs/ai-introduction)
- [https://docs.cloud.google.com/bigquery/docs/colab-data-science-agent](https://docs.cloud.google.com/bigquery/docs/colab-data-science-agent)
- [https://docs.cloud.google.com/bigquery/docs/hadoop-metadata](https://docs.cloud.google.com/bigquery/docs/hadoop-metadata)

## Validation Findings

- `warn:coverage_low` The feature ended with coverage LOW.
