# Dialogflow CX credit card enrollment prebuilt component

Product: Dialogflow
Feature slug: `dialogflow-cx-credit-card-enrollment-prebuilt-component`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

Dialogflow CX provides a prebuilt component for credit card enrollment flows.

## Lifecycle

- Latest feature date: 2023-10-23
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- identity (evidence: [https://docs.cloud.google.com/dialogflow/cx/docs/concept/agents-prebuilt](https://docs.cloud.google.com/dialogflow/cx/docs/concept/agents-prebuilt), [https://docs.cloud.google.com/java/docs/reference/google-cloud-dialogflow/latest/com.google.cloud.dialogflow.v2](https://docs.cloud.google.com/java/docs/reference/google-cloud-dialogflow/latest/com.google.cloud.dialogflow.v2), [https://docs.cloud.google.com/dialogflow/cx/docs/tutorials/deploy/prebuilt](https://docs.cloud.google.com/dialogflow/cx/docs/tutorials/deploy/prebuilt))

## Official Evidence

- [https://docs.cloud.google.com/dialogflow/cx/docs/concept/agents-prebuilt](https://docs.cloud.google.com/dialogflow/cx/docs/concept/agents-prebuilt)
- [https://docs.cloud.google.com/dialogflow/cx/docs/tutorials/deploy/prebuilt](https://docs.cloud.google.com/dialogflow/cx/docs/tutorials/deploy/prebuilt)
- [https://docs.cloud.google.com/java/docs/reference/google-cloud-dialogflow/latest/com.google.cloud.dialogflow.v2](https://docs.cloud.google.com/java/docs/reference/google-cloud-dialogflow/latest/com.google.cloud.dialogflow.v2)
