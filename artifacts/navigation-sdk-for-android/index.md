# Navigation SDK for Android

Service card: [card.json](./card.json)

Generated from Step 08 card: `data/step-08/current/products/navigation-sdk-for-android/card.json`

## Summary

- Promoted features: 41
- Step 07 product status: PASS
- Corpus health: healthy
- Latest feature date: 2026-03-16
- Official source links: 33

## Official Service Evidence

- [https://developers.google.com/maps/documentation/navigation/android-sdk/android-auto](https://developers.google.com/maps/documentation/navigation/android-sdk/android-auto)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/android-studio-setup](https://developers.google.com/maps/documentation/navigation/android-sdk/android-studio-setup)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/camera](https://developers.google.com/maps/documentation/navigation/android-sdk/camera)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/controls](https://developers.google.com/maps/documentation/navigation/android-sdk/controls)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/customize-map-styles](https://developers.google.com/maps/documentation/navigation/android-sdk/customize-map-styles)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/customize-route](https://developers.google.com/maps/documentation/navigation/android-sdk/customize-route)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/events](https://developers.google.com/maps/documentation/navigation/android-sdk/events)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/intro-custom-nav](https://developers.google.com/maps/documentation/navigation/android-sdk/intro-custom-nav)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/intro-google-nav](https://developers.google.com/maps/documentation/navigation/android-sdk/intro-google-nav)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/intro-route-experience](https://developers.google.com/maps/documentation/navigation/android-sdk/intro-route-experience)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/multi-destination](https://developers.google.com/maps/documentation/navigation/android-sdk/multi-destination)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/navigation-points](https://developers.google.com/maps/documentation/navigation/android-sdk/navigation-points)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/overview](https://developers.google.com/maps/documentation/navigation/android-sdk/overview)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/real-time-disruptions](https://developers.google.com/maps/documentation/navigation/android-sdk/real-time-disruptions)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference](https://developers.google.com/maps/documentation/navigation/android-sdk/reference)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.CancelableCallback](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.CancelableCallback)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnCameraChangeListener](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnCameraChangeListener)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnCameraMoveCanceledListener](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnCameraMoveCanceledListener)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnCameraMoveStartedListener](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnCameraMoveStartedListener)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnIndoorStateChangeListener](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnIndoorStateChangeListener)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnInfoWindowClickListener](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnInfoWindowClickListener)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnInfoWindowCloseListener](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnInfoWindowCloseListener)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnInfoWindowLongClickListener](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnInfoWindowLongClickListener)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnMapCapabilitiesChangedListener](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnMapCapabilitiesChangedListener)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnMapClickListener](https://developers.google.com/maps/documentation/navigation/android-sdk/reference/com/google/android/gms/maps/GoogleMap.OnMapClickListener)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/route](https://developers.google.com/maps/documentation/navigation/android-sdk/route)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/set-up-project](https://developers.google.com/maps/documentation/navigation/android-sdk/set-up-project)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/setup-overview](https://developers.google.com/maps/documentation/navigation/android-sdk/setup-overview)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/speed-alerts](https://developers.google.com/maps/documentation/navigation/android-sdk/speed-alerts)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/support](https://developers.google.com/maps/documentation/navigation/android-sdk/support)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/tbt-feed](https://developers.google.com/maps/documentation/navigation/android-sdk/tbt-feed)
- [https://developers.google.com/maps/documentation/navigation/android-sdk/try-demo](https://developers.google.com/maps/documentation/navigation/android-sdk/try-demo)

## Features

- [3D buildings in turn-by-turn navigation](./3d-buildings-in-turn-by-turn-navigation/README.md)
- [AdvancedMarker.addMarker(AdvancedMarkerOptions)](./advancedmarker-addmarker-advancedmarkeroptions/README.md)
- [Alternate route preview and selection](./alternate-route-preview-and-selection/README.md)
- [Android Auto support](./android-auto-support/README.md)
- [Arrival triggering](./arrival-triggering/README.md)
- [Cloud-based maps styling for navigation and road maps](./cloud-based-maps-styling-for-navigation-and-road-maps/README.md)
- [Consecutive duplicate destination waypoints](./consecutive-duplicate-destination-waypoints/README.md)
- [Custom control footer position](./custom-control-footer-position/README.md)
- [Destination and entrance highlighting](./destination-and-entrance-highlighting/README.md)
- [Destination request quota tracking persistence](./destination-request-quota-tracking-persistence/README.md)
- [DisplayOptions.showStopSigns API](./displayoptions-showstopsigns-api/README.md)
- [DisplayOptions.showTrafficLights API](./displayoptions-showtrafficlights-api/README.md)
- [Enhanced location context for Waypoint API](./enhanced-location-context-for-waypoint-api/README.md)
- [GpsAvailabilityEnhancedLocationListener.onGpsAvailabilityUpdate callback](./gpsavailabilityenhancedlocationlistener-ongpsavailabilityupdate-callback/README.md)
- [Guidance notifications disable on app dismissal](./guidance-notifications-disable-on-app-dismissal/README.md)
- [Guidance rendering performance](./guidance-rendering-performance/README.md)
- [Incident reporting panel APIs](./incident-reporting-panel-apis/README.md)
- [Maneuver and lane icons](./maneuver-and-lane-icons/README.md)
- [Map background color while tiles load](./map-background-color-while-tiles-load/README.md)
- [Map color scheme selection](./map-color-scheme-selection/README.md)
- [MAP_TYPE_TERRAIN navigation map type](./map-type-terrain-navigation-map-type/README.md)
- [My Location button enabled setting](./my-location-button-enabled-setting/README.md)
- [Narrow road callouts](./narrow-road-callouts/README.md)
- [Navigation voice guidance stop sign references](./navigation-voice-guidance-stop-sign-references/README.md)
- [NavigationMapStyle API](./navigationmapstyle-api/README.md)
- [NavigationMapStyle DEFAULT and HIGH_DETAIL constants](./navigationmapstyle-default-and-high-detail-constants/README.md)
- [navigator.fetchRouteInfo()](./navigator-fetchrouteinfo/README.md)
- [Prompt Visibility API](./prompt-visibility-api/README.md)
- [Railroad crossing callouts](./railroad-crossing-callouts/README.md)
- [Real-time disruptions](./real-time-disruptions/README.md)
- [Road closure callouts on alternate routes](./road-closure-callouts-on-alternate-routes/README.md)
- [Road Level Details](./road-level-details/README.md)
- [Route-aware road label decluttering](./route-aware-road-label-decluttering/README.md)
- [Route-relevant orientation landmark POIs](./route-relevant-orientation-landmark-pois/README.md)
- [RouteChangedListener trigger events](./routechangedlistener-trigger-events/README.md)
- [Traffic light and stop sign display](./traffic-light-and-stop-sign-display/README.md)
- [Traffic light and stop sign route icons](./traffic-light-and-stop-sign-route-icons/README.md)
- [Traffic-light-aware voice guidance](./traffic-light-aware-voice-guidance/README.md)
- [Turn and ETA navigation card header layout](./turn-and-eta-navigation-card-header-layout/README.md)
- [Turn card design update](./turn-card-design-update/README.md)
- [Unpaved road callouts](./unpaved-road-callouts/README.md)
