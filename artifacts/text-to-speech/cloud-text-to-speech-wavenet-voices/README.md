# Cloud Text-to-Speech WaveNet voices

Product: Text-to-Speech
Feature slug: `cloud-text-to-speech-wavenet-voices`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

WaveNet voices were added for Danish (Denmark), Polish (Poland), Portuguese (Brazil), Russian (Russia), Slovak (Slovakia), and Turkish (Turkey).

## Lifecycle

- Latest feature date: 2019-02-05
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- access (evidence: [https://docs.cloud.google.com/text-to-speech/docs/wavenet](https://docs.cloud.google.com/text-to-speech/docs/wavenet), [https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types](https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types), [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices))
- auth (evidence: [https://docs.cloud.google.com/text-to-speech/docs/wavenet](https://docs.cloud.google.com/text-to-speech/docs/wavenet), [https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types](https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types), [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices))
- authorization (evidence: [https://docs.cloud.google.com/text-to-speech/docs/wavenet](https://docs.cloud.google.com/text-to-speech/docs/wavenet), [https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types](https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types), [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices))
- key (evidence: [https://docs.cloud.google.com/text-to-speech/docs/wavenet](https://docs.cloud.google.com/text-to-speech/docs/wavenet), [https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types](https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types), [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices))
- token (evidence: [https://docs.cloud.google.com/text-to-speech/docs/wavenet](https://docs.cloud.google.com/text-to-speech/docs/wavenet), [https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types](https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types), [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices))

## Official Evidence

- [https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types](https://docs.cloud.google.com/text-to-speech/docs/list-voices-and-types)
- [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices)
- [https://docs.cloud.google.com/text-to-speech/docs/wavenet](https://docs.cloud.google.com/text-to-speech/docs/wavenet)
