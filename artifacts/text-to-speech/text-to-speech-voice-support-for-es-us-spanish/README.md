# Text-to-Speech voice support for es-US (Spanish

Product: Text-to-Speech
Feature slug: `text-to-speech-voice-support-for-es-us-spanish`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

Text-to-Speech added a supported voice for the es-US locale (Spanish, US).

## Lifecycle

- Latest feature date: 2021-04-09
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- access (evidence: [https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice](https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice), [https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd](https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd), [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices))
- auth (evidence: [https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice](https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice), [https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd](https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd), [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices))
- authorization (evidence: [https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice](https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice), [https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd](https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd), [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices))
- key (evidence: [https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice](https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice), [https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd](https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd), [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices))
- token (evidence: [https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice](https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice), [https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd](https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd), [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices))

## Official Evidence

- [https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd](https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd)
- [https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice](https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice)
- [https://docs.cloud.google.com/text-to-speech/docs/voices](https://docs.cloud.google.com/text-to-speech/docs/voices)
