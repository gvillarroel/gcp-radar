# Studio in Looker report recovery after re-enable

Product: Looker
Feature slug: `studio-in-looker-report-recovery-after-re-enable`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

Reports saved within the previous 30 days remain available after Studio in Looker is disabled and then re-enabled.

## Lifecycle

- Latest feature date: 2025-03-17
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- access
- auth
- key
- permission
- role

## Official Evidence

- [https://docs.cloud.google.com/bigquery/docs/google-ads-transformation](https://docs.cloud.google.com/bigquery/docs/google-ads-transformation)
- [https://docs.cloud.google.com/looker/docs/admin-panel-users-roles](https://docs.cloud.google.com/looker/docs/admin-panel-users-roles)
- [https://docs.cloud.google.com/looker/docs/studio/connect-to-google-bigquery](https://docs.cloud.google.com/looker/docs/studio/connect-to-google-bigquery)
