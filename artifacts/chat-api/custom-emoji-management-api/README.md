# Custom emoji management API

Product: Chat API
Feature slug: `custom-emoji-management-api`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

The Chat API can create, delete, get, and list custom emoji when custom emoji is enabled for the Workspace organization.

## Lifecycle

- Latest feature date: 2024-10-10
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- auth (evidence: [https://developers.google.com/workspace/chat/delete-custom-emoji](https://developers.google.com/workspace/chat/delete-custom-emoji), [https://developers.google.com/workspace/chat/create-custom-emoji](https://developers.google.com/workspace/chat/create-custom-emoji), [https://developers.google.com/workspace/chat/api/reference/rest/v1/customEmojis/delete](https://developers.google.com/workspace/chat/api/reference/rest/v1/customEmojis/delete))
- authorization (evidence: [https://developers.google.com/workspace/chat/delete-custom-emoji](https://developers.google.com/workspace/chat/delete-custom-emoji), [https://developers.google.com/workspace/chat/create-custom-emoji](https://developers.google.com/workspace/chat/create-custom-emoji), [https://developers.google.com/workspace/chat/api/reference/rest/v1/customEmojis/delete](https://developers.google.com/workspace/chat/api/reference/rest/v1/customEmojis/delete))
- credential (evidence: [https://developers.google.com/workspace/chat/delete-custom-emoji](https://developers.google.com/workspace/chat/delete-custom-emoji), [https://developers.google.com/workspace/chat/create-custom-emoji](https://developers.google.com/workspace/chat/create-custom-emoji), [https://developers.google.com/workspace/chat/api/reference/rest/v1/customEmojis/delete](https://developers.google.com/workspace/chat/api/reference/rest/v1/customEmojis/delete))

## Official Evidence

- [https://developers.google.com/workspace/chat/api/reference/rest/v1/customEmojis/delete](https://developers.google.com/workspace/chat/api/reference/rest/v1/customEmojis/delete)
- [https://developers.google.com/workspace/chat/create-custom-emoji](https://developers.google.com/workspace/chat/create-custom-emoji)
- [https://developers.google.com/workspace/chat/delete-custom-emoji](https://developers.google.com/workspace/chat/delete-custom-emoji)
