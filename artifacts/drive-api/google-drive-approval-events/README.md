# Google Drive approval events

Product: Drive API
Feature slug: `google-drive-approval-events`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

Google Drive events can now report approval lifecycle and reviewer changes on a file.

## Lifecycle

- Latest feature date: 2025-12-17
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- access (evidence: [https://developers.google.com/workspace/drive/api/guides/events-overview](https://developers.google.com/workspace/drive/api/guides/events-overview), [https://developers.google.com/workspace/drive/api/guides/approvals](https://developers.google.com/workspace/drive/api/guides/approvals), [https://developers.google.com/workspace/drive/api/guides/about-changes](https://developers.google.com/workspace/drive/api/guides/about-changes), [https://developers.google.com/workspace/drive/api/guides/about-shareddrives](https://developers.google.com/workspace/drive/api/guides/about-shareddrives))
- permission (evidence: [https://developers.google.com/workspace/drive/api/guides/events-overview](https://developers.google.com/workspace/drive/api/guides/events-overview), [https://developers.google.com/workspace/drive/api/guides/approvals](https://developers.google.com/workspace/drive/api/guides/approvals), [https://developers.google.com/workspace/drive/api/guides/about-changes](https://developers.google.com/workspace/drive/api/guides/about-changes), [https://developers.google.com/workspace/drive/api/guides/about-shareddrives](https://developers.google.com/workspace/drive/api/guides/about-shareddrives))

## Official Evidence

- [https://developers.google.com/workspace/drive/api/guides/about-changes](https://developers.google.com/workspace/drive/api/guides/about-changes)
- [https://developers.google.com/workspace/drive/api/guides/about-shareddrives](https://developers.google.com/workspace/drive/api/guides/about-shareddrives)
- [https://developers.google.com/workspace/drive/api/guides/approvals](https://developers.google.com/workspace/drive/api/guides/approvals)
- [https://developers.google.com/workspace/drive/api/guides/events-overview](https://developers.google.com/workspace/drive/api/guides/events-overview)
