# Cloud Product Registry API

Service card: [card.json](./card.json)

Generated from Step 08 card: `data/step-08/current/products/cloud-product-registry-api/card.json`

## Summary

- Promoted features: 1
- Step 07 product status: PASS
- Corpus health: healthy
- Latest feature date: 2026-02-27
- Official source links: 4

## Features

- [Cloud Product Registry API](./cloud-product-registry-api/README.md)
