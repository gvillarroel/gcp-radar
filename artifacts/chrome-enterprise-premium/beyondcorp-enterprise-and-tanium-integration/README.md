# BeyondCorp Enterprise and Tanium integration

Product: Chrome Enterprise Premium
Feature slug: `beyondcorp-enterprise-and-tanium-integration`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

The BeyondCorp Enterprise and Tanium integration has been officially shut down; deprecated on 2024-01-03.

## Lifecycle

- Latest feature date: 2024-01-03
- Deprecation date: 2024-01-03
- Status: deprecation_noted

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- access (evidence: [https://docs.cloud.google.com/chrome-enterprise-premium/docs/integration-docs](https://docs.cloud.google.com/chrome-enterprise-premium/docs/integration-docs), [https://docs.cloud.google.com/chrome-enterprise-premium/docs/cs-overview](https://docs.cloud.google.com/chrome-enterprise-premium/docs/cs-overview), [https://docs.cloud.google.com/chrome-enterprise-premium/docs/intune-overview](https://docs.cloud.google.com/chrome-enterprise-premium/docs/intune-overview))
- security (evidence: [https://docs.cloud.google.com/chrome-enterprise-premium/docs/integration-docs](https://docs.cloud.google.com/chrome-enterprise-premium/docs/integration-docs), [https://docs.cloud.google.com/chrome-enterprise-premium/docs/cs-overview](https://docs.cloud.google.com/chrome-enterprise-premium/docs/cs-overview), [https://docs.cloud.google.com/chrome-enterprise-premium/docs/intune-overview](https://docs.cloud.google.com/chrome-enterprise-premium/docs/intune-overview))

## Official Evidence

- [https://docs.cloud.google.com/chrome-enterprise-premium/docs/cs-overview](https://docs.cloud.google.com/chrome-enterprise-premium/docs/cs-overview)
- [https://docs.cloud.google.com/chrome-enterprise-premium/docs/integration-docs](https://docs.cloud.google.com/chrome-enterprise-premium/docs/integration-docs)
- [https://docs.cloud.google.com/chrome-enterprise-premium/docs/intune-overview](https://docs.cloud.google.com/chrome-enterprise-premium/docs/intune-overview)
