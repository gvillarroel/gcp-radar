# EmbeddedChartBuilder chart type helpers

Product: Apps Script
Feature slug: `embeddedchartbuilder-chart-type-helpers`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

EmbeddedChartBuilder now provides chart-specific helper methods as easier alternatives to setChartType().

## Lifecycle

- Latest feature date: 2012-11-06
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- auth
- authorization
- key
- permission

## Official Evidence

- [https://developers.google.com/apps-script/reference/spreadsheet](https://developers.google.com/apps-script/reference/spreadsheet)
- [https://developers.google.com/apps-script/reference/spreadsheet/data-source-chart](https://developers.google.com/apps-script/reference/spreadsheet/data-source-chart)
- [https://developers.google.com/apps-script/releases](https://developers.google.com/apps-script/releases)
