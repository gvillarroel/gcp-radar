# Embedded Charts in Google Sheets

Product: Apps Script
Feature slug: `embedded-charts-in-google-sheets`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

Apps Script can now create, modify, and remove embedded charts in Google Sheets.

## Lifecycle

- Latest feature date: 2012-06-14
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- key
- permission

## Official Evidence

- [https://developers.google.com/apps-script/guides/sheets](https://developers.google.com/apps-script/guides/sheets)
- [https://developers.google.com/apps-script/reference/spreadsheet](https://developers.google.com/apps-script/reference/spreadsheet)
- [https://developers.google.com/apps-script/releases](https://developers.google.com/apps-script/releases)
