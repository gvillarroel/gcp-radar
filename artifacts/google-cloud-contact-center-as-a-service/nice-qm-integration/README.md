# NICE QM integration

Product: Google Cloud Contact Center as a Service
Feature slug: `nice-qm-integration`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

The platform provides an out-of-the-box data export integration for NICE QM; The platform provides an out-of-the-box data export integration for NICE QM.

## Lifecycle

- Latest feature date: 2023-09-25
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- access
- token

## Official Evidence

- [https://docs.cloud.google.com/contact-center/ccai-platform/docs](https://docs.cloud.google.com/contact-center/ccai-platform/docs)
- [https://docs.cloud.google.com/contact-center/ccai-platform/docs/kustomer-integration-guide](https://docs.cloud.google.com/contact-center/ccai-platform/docs/kustomer-integration-guide)
- [https://docs.cloud.google.com/contact-center/ccai-platform/docs/salesforce-integration-guide](https://docs.cloud.google.com/contact-center/ccai-platform/docs/salesforce-integration-guide)
