# Authentication context for CRM SAML SSO

Product: Google Cloud Contact Center as a Service
Feature slug: `authentication-context-for-crm-saml-sso`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

You can now select the authentication context when setting up SAML single sign-on for supported CRMs.

## Lifecycle

- Latest feature date: 2024-04-22
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- auth (evidence: [https://docs.cloud.google.com/contact-center/ccai-platform/docs/agent-desktop-set-desktop-layouts](https://docs.cloud.google.com/contact-center/ccai-platform/docs/agent-desktop-set-desktop-layouts), [https://docs.cloud.google.com/contact-center/ccai-platform/docs/mid-session-authentication-apps-api](https://docs.cloud.google.com/contact-center/ccai-platform/docs/mid-session-authentication-apps-api), [https://docs.cloud.google.com/contact-center/ccai-platform/docs/agent-desktop-create-desktop-layouts](https://docs.cloud.google.com/contact-center/ccai-platform/docs/agent-desktop-create-desktop-layouts))

## Official Evidence

- [https://docs.cloud.google.com/contact-center/ccai-platform/docs/agent-desktop-create-desktop-layouts](https://docs.cloud.google.com/contact-center/ccai-platform/docs/agent-desktop-create-desktop-layouts)
- [https://docs.cloud.google.com/contact-center/ccai-platform/docs/agent-desktop-set-desktop-layouts](https://docs.cloud.google.com/contact-center/ccai-platform/docs/agent-desktop-set-desktop-layouts)
- [https://docs.cloud.google.com/contact-center/ccai-platform/docs/mid-session-authentication-apps-api](https://docs.cloud.google.com/contact-center/ccai-platform/docs/mid-session-authentication-apps-api)
