# Channel events

Product: Live Stream API
Feature slug: `channel-events`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

The API adds channel events for mute, unmute, return to program, and switch input actions.

## Lifecycle

- Latest feature date: 2023-02-21
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- access (evidence: [https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events](https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events), [https://docs.cloud.google.com/livestream/docs/overview](https://docs.cloud.google.com/livestream/docs/overview), [https://docs.cloud.google.com/livestream/docs/how-to/create-channels](https://docs.cloud.google.com/livestream/docs/how-to/create-channels), [https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls](https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls))
- audit (evidence: [https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events](https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events), [https://docs.cloud.google.com/livestream/docs/overview](https://docs.cloud.google.com/livestream/docs/overview), [https://docs.cloud.google.com/livestream/docs/how-to/create-channels](https://docs.cloud.google.com/livestream/docs/how-to/create-channels), [https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls](https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls))
- auth (evidence: [https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events](https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events), [https://docs.cloud.google.com/livestream/docs/overview](https://docs.cloud.google.com/livestream/docs/overview), [https://docs.cloud.google.com/livestream/docs/how-to/create-channels](https://docs.cloud.google.com/livestream/docs/how-to/create-channels), [https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls](https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls))
- authorization (evidence: [https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events](https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events), [https://docs.cloud.google.com/livestream/docs/overview](https://docs.cloud.google.com/livestream/docs/overview), [https://docs.cloud.google.com/livestream/docs/how-to/create-channels](https://docs.cloud.google.com/livestream/docs/how-to/create-channels), [https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls](https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls))
- encrypt (evidence: [https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events](https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events), [https://docs.cloud.google.com/livestream/docs/overview](https://docs.cloud.google.com/livestream/docs/overview), [https://docs.cloud.google.com/livestream/docs/how-to/create-channels](https://docs.cloud.google.com/livestream/docs/how-to/create-channels), [https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls](https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls))
- iam (evidence: [https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events](https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events), [https://docs.cloud.google.com/livestream/docs/overview](https://docs.cloud.google.com/livestream/docs/overview), [https://docs.cloud.google.com/livestream/docs/how-to/create-channels](https://docs.cloud.google.com/livestream/docs/how-to/create-channels), [https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls](https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls))
- key (evidence: [https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events](https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events), [https://docs.cloud.google.com/livestream/docs/overview](https://docs.cloud.google.com/livestream/docs/overview), [https://docs.cloud.google.com/livestream/docs/how-to/create-channels](https://docs.cloud.google.com/livestream/docs/how-to/create-channels), [https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls](https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls))
- token (evidence: [https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events](https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events), [https://docs.cloud.google.com/livestream/docs/overview](https://docs.cloud.google.com/livestream/docs/overview), [https://docs.cloud.google.com/livestream/docs/how-to/create-channels](https://docs.cloud.google.com/livestream/docs/how-to/create-channels), [https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls](https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls))

## Official Evidence

- [https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events](https://docs.cloud.google.com/livestream/docs/how-to/create-channel-events)
- [https://docs.cloud.google.com/livestream/docs/how-to/create-channels](https://docs.cloud.google.com/livestream/docs/how-to/create-channels)
- [https://docs.cloud.google.com/livestream/docs/overview](https://docs.cloud.google.com/livestream/docs/overview)
- [https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls](https://docs.cloud.google.com/livestream/docs/quickstarts/quickstart-hls)
