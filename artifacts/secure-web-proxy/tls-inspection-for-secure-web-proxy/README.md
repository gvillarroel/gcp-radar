# TLS inspection for Secure Web Proxy

Product: Secure Web Proxy
Feature slug: `tls-inspection-for-secure-web-proxy`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

Secure Web Proxy can intercept and inspect encrypted TLS traffic and enforce security policies.

## Lifecycle

- Latest feature date: 2023-03-14
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- allow (evidence: [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview), [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection), [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation), [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules))
- certificate (evidence: [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview), [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection), [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation), [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules))
- encrypt (evidence: [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview), [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection), [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation), [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules))
- iam (evidence: [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview), [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection), [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation), [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules))
- key (evidence: [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview), [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection), [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation), [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules))
- permission (evidence: [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview), [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection), [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation), [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules))
- policy (evidence: [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview), [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection), [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation), [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules))
- private (evidence: [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview), [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection), [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation), [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules))
- role (evidence: [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview), [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection), [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation), [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules))
- security (evidence: [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview), [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection), [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation), [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules))

## Official Evidence

- [https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules](https://docs.cloud.google.com/secure-web-proxy/docs/configure-rules)
- [https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection](https://docs.cloud.google.com/secure-web-proxy/docs/enable-tls-inspection)
- [https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview](https://docs.cloud.google.com/secure-web-proxy/docs/tls-inspection-overview)
- [https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation](https://docs.cloud.google.com/secure-web-proxy/docs/tls-rule-evaluation)
