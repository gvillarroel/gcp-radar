# Ubuntu 24 LTS base image support for source deployment

Product: Buildpacks
Feature slug: `ubuntu-24-lts-base-image-support-for-source-deployment`
Coverage: `MEDIUM`
IAM mapping: `unknown`

## Technical Summary

Cloud Run source deployment supports Ubuntu 24 LTS base images through the google-24 builder.

## Lifecycle

- Latest feature date: 2026-02-17
- Deprecation date: none recorded
- Status: feature_observed

## IAM

No defensible IAM mapping was found in the current evidence.

## Security Capabilities

- access
- allow
- armor
- audit
- auth
- certificate
- key

## Official Evidence

- [https://docs.cloud.google.com/docs/buildpacks/base-images](https://docs.cloud.google.com/docs/buildpacks/base-images)
- [https://docs.cloud.google.com/docs/buildpacks/osonly](https://docs.cloud.google.com/docs/buildpacks/osonly)
- [https://docs.cloud.google.com/docs/buildpacks/stacks](https://docs.cloud.google.com/docs/buildpacks/stacks)
