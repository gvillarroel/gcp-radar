...
promoted cards disagree with their artifact paths, including missing or
mismatched product slugs, service-card paths, promoted feature counts,
duplicate promoted feature slugs, skipped feature counts, duplicate skipped
feature slugs, promoted/skipped feature overlap, feature artifact paths,
promoted or skipped feature names, embedded feature-card slugs, and
...
Every entry in those arrays must carry a non-empty `feature_slug`; Step 10
must reject missing feature slugs before rewriting reports instead of ignoring
malformed manifest entries during aggregation.
...
warning-rule policy must be an array of non-empty string rule IDs, sorted,
duplicate-free, and must match the `accepted_warning_rules` array embedded in
every product promotion manifest. Its latest-run
...
Step 08 product card timestamp, because the index summarizes the completed
card-construction run. Generated Step 08 product cards must also use the
current Step 08 card-construction schema version so stale card payloads cannot
pass under a current index. Each generated Step 08 product `card.md` must also
keep its rendered product title, schema version, generation timestamp, Step 07
summary, corpus health, IAM summary, service-card summary, and feature rows
aligned with the matching Step 08 `card.json` so the reviewable card layer
cannot silently drift from its canonical JSON card.
...
name, rendered feature link label, mapping status, explicit roles, explicit
permissions, derived roles, and derived permissions aligned with the promoted
feature card values rendered by Step 10. Each product report's feature rows
must also keep each promoted feature's rendered feature link label, IAM mapping
status, explicit roles, explicit permissions, derived roles, derived
permissions, coverage status, and rendered source links aligned with the
promoted feature card values. Each product report must also keep its
...
Each services report row must keep the rendered service link label, service
status, promoted feature count, latest feature date, explicit IAM count,
derived IAM count, unknown IAM count, and rendered official source links
aligned with the promoted service card and promotion manifest.
...
counts aligned with the promoted artifact inventory. It must not duplicate
valid promoted product-report or service-card links. Each root index product row
must also keep the product name, promoted feature count, latest feature date,
...
keep the promoted feature's product name, rendered feature link label,
capability list, and rendered evidence links aligned with the promoted feature
card.
