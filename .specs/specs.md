...
The root radar index and coverage report must also render the same
`generated_at` timestamp recorded in `data/step-10/current/index.json` so final
report metadata cannot drift from the Step 10 run metadata.
